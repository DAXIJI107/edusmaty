...
class AgentRuntime {
    ...
    async ensureSchema() {
        ...
        await this.pool
            .query(
                `CREATE TABLE IF NOT EXISTS ai_course_designs (
            id BIGINT PRIMARY KEY AUTO_INCREMENT,
            user_id BIGINT NOT NULL,
    ...
    inferSubject(message = "", fallback = "all") {
        ...
        return (
            subjects.find(
                subject => String(message).includes(subject.replace("系统", "")) || String(message).includes(subject)
            ) ||
            fallback ||
            "all"
        );
    ...
    createExecutionPlan({ intent, message, context, sourceName }) {
        ...
        const needsPublicSource =
            !!sourceName || /开源|公开|github|资料|rag|nexusrag|oatutor|openmaic|agentscope|hello-agents/.test(text);
        ...
        const steps = [{ tool: "profile", reason: "读取学习画像、掌握度、近期行为和笔记，作为个性化依据。" }];
        if (needsPublicSource)
            steps.push({ tool: "publicSource", reason: "从公开网页或 GitHub README 抓取资料并写入 RAG 知识库。" });
        ...
        if (needsCourse || needsPractice || intent === "next_action")
            steps.push({ tool: "path", reason: "把 agent 决策写回今日学习任务。" });
        if (needsNote || needsRag || intent === "next_action")
            steps.push({ tool: "note", reason: "生成复盘卡或资料沉淀任务。" });
        return steps;
    }
    ...
    async run({ userId, message = "", intent = "", context = {} }) {
        ...
        if (shouldRun("publicSource")) {
            publicSource = await this.tools.publicSource.run({
                sourceName: sourceName || "all",
                limit: sourceName ? 1 : 4
            });
            traces.push(
                await this.recordTrace({
                    userId,
        ...
        if (shouldRun("courseDesign")) {
            courseDesign = await this.tools.courseDesign.run({
                userId,
                goal,
                subject,
                durationDays,
        ...
        if (shouldRun("path")) {
            path = await this.tools.path.run({
                userId,
                goal,
                subject,
                intensity,
        ...
        await this.recordDecision({
            ...
            decisionDetail: {
                ...
                publicSource: publicSource
                    ? { documents: publicSource.documents, chunkCount: publicSource.chunkCount }
                    : null,
                executionPlan
            },
            confidence: 0.82
    ...
    async recordPracticeCompletion(userId, { knowledgeId, score, total, durationMs, questionIds, answers }) {
        ...
        await this.recordDecision({
            ...
            toolsCalled: [
                {
                    tool: "MasteryTool.recordCompletion",
                    result: { mastery: masteryResult.mastery, trend: masteryResult.trend }
                },
