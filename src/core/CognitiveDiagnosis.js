...
class CognitiveDiagnosis {
    constructor() {
        ...
        this.bloomWeights = {
            ...
            '理解': { weight: 0.20, keywords: ['解释', '说明', '归纳', '比较', '转换', '推断', '区别'] },
            ...
            '创造': { weight: 0.10, keywords: ['设计', '构建', '开发', '规划', '创作', '生成', '提出'] }
        };
    ...
    async diagnose(userId, examRecordId, pool) {
        ...
        return {
            ...
            recommendations: this.generateSmartRecommendations(
                dinaResult, bloomResult, misconceptions, skillMastery
            ),
    ...
    dinaDiagnose(answers) {
        ...
        for (const [attr, scores] of Object.entries(attributeScores)) {
            ...
            const slipEstimate = scores.hardTotal > 0
                ? 1 - (scores.hardCorrect / scores.hardTotal)
                : 0.1;
            ...
            attributeMastery[attr] = {
                ...
                state: masteryProb >= 0.75 ? 'mastered' :
                       masteryProb >= 0.5 ? 'developing' :
                       masteryProb >= 0.25 ? 'beginner' : 'weak',
        ...
        return {
            attributeMastery,
            overallProbability: Object.values(attributeMastery).reduce((s, v) => s + v.probability, 0) /
    ...
    bloomAnalyze(answers) {
        ...
        for (const [level, count] of Object.entries(blooms)) {
            levels[level] = {
                ...
                mastery: count > 0
                    ? Math.min(100, Math.round((correctBlooms[level] || 0) / Math.max(count, 1) * 100))
                    : 0,
    ...
    detectMisconceptions(answers) {
        ...
        const categories = Object.entries(misconceptionMap)
            ...
            .map(([category, data]) => ({
                ...
                severity: data.count / wrongAnswers.length >= 0.3 ? 'critical' :
                          data.count / wrongAnswers.length >= 0.15 ? 'moderate' : 'minor',
    ...
    advancedMisconceptionDetect(ans, content, userAnswer, correctAnswer) {
        ...
        if (/^[\d.\-+]+$/.test(userAnswer) && /^[\d.\-+]+$/.test(correctAnswer)) {
            ...
            if (!isNaN(userNum) && !isNaN(correctNum)) {
                ...
                if (ratio > 0.9 && ratio < 1.1) {
                    return { category: '运算错误', hint: `数值接近但略有偏差 (比率=${ratio.toFixed(3)})，检查计算精度或舍入` };
                }
                if (ratio > 9 || ratio < 0.11) {
                    return { category: '单位/符号错误', hint: `数值偏差约${Math.round(Math.abs(Math.log10(ratio)))}个数量级，检查单位换算` };
                }
                return { category: '运算错误', hint: `正确答案 ${correctAnswer}，你的答案是 ${userAnswer}，检查计算步骤` };
            }
        }
        ...
        if (/^[a-d]$/.test(userAnswer) && /^[a-d]$/.test(correctAnswer)) {
            return { category: '概念混淆', hint: `选项选错，可能混淆了相关概念 | 你的答案:${userAnswer.toUpperCase()}，正确:${correctAnswer.toUpperCase()}` };
        }
    ...
    buildSkillMatrix(answers) {
        ...
        for (const [skill, items] of Object.entries(skillQuestions)) {
            ...
            const pMasteryGivenCorrect = (rawRate * (1 - slipProb)) /
                (rawRate * (1 - slipProb) + (1 - rawRate) * guessProb);
    ...
    analyzeRootCause(answers, skillMastery, misconceptions) {
        ...
        // 分析深度: 是概念层还是应用层
        const depth = primary && primary.mastery.mastery < 40 ? 'deep' :
                      primary && primary.errorCount >= 3 ? 'moderate' : 'shallow';
    ...
    parseSkills(skillCodes) {
        ...
        try {
            ...
            return String(skillCodes).split(',').map(s => s.trim()).filter(Boolean);
        }
    }
        ...
        try {
            ...
            for (const [subject, subjectAnswers] of Object.entries(bySubject)) {
                results[subject] = {
                    ...
                    accuracy: Math.round(subjectAnswers.filter(a => a.is_correct).length / subjectAnswers.length * 100)
                };
            }
