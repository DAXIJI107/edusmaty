...
class DiagnosticEngine {
    ...
    buildLearningPatterns(answers) {
        ...
        return {
            易错知识点: [],
            学习速度:
                answers.q_path_prefer === "intensive" ? "fast" : answers.q_path_prefer === "guided" ? "slow" : "medium",
        ...
        return {
            ...
            summary: {
                ...
                recommendedDailyMinutes:
                    dailyMinutes >= 60 ? Math.round(dailyMinutes * 1.3) : Math.round(dailyMinutes * 1.6),
    ...
    classifyPersona(answers, discipline, dailyMinutes) {
        if (answers.q_path_prefer === "intensive" && discipline >= 4)
            return { primary: "冲刺突破型", tag: "高投入·快节奏" };
        if (answers.q_motivation === "career" && ["computer", "engineering", "finance"].includes(answers.q_major))
            return { primary: "职业导向型", tag: "目标明确·技能驱动" };
    ...
    generateRecommendations(profile, analysis, answers) {
        ...
        if (Number(answers.q_self_discipline || 3) <= 2) {
            recs.push({
                type: "habit",
                title: "习惯养成计划",
                description: "从每天专注15分钟开始，逐步建立学习节奏",
                action: "开始 21 天挑战",
                target: "study_habit",
                priority: "high",
                reason: "自评自律性偏低"
    ...
    async getLatestResult(userId) {
        try {
            const [rows] = await this.pool.query(
                "SELECT * FROM diagnostic_results WHERE user_id = ? ORDER BY id DESC LIMIT 1",
                [userId]
            );
        ...
        try {
            await this.pool.query(
                ...
                [
                    userId,
                    `subject_${subject}`,
                    JSON.stringify({ subject, answers }),
                    JSON.stringify(analysis.profile || {}),
                    JSON.stringify(analysis),
                    JSON.stringify(analysis.recommendations || []),
                    JSON.stringify(analysis.radarData || [])
                ]
            );
        } catch (error) {
    ...
    analyzeSubjectResults(subject, gradedQuestions, accuracy) {
        ...
        return {
            ...
            weakNodes,
            strongNodes,
            rootCause: weakNodes.length
                ? `在${subject}学科中，${weakNodes.map(n => n.nodeName).join("、")}等知识点掌握不足`
                : `在${subject}学科中表现良好，各知识点掌握较为均衡`,
            recommendations,
            radarData,
            profile,
            gradedQuestions
    ...
    classifySubjectProfile(subject, accuracy, nodeResults) {
        ...
        if (accuracy >= 85 && strongRatio >= 0.7)
            return {
                subject,
                personaLabel: "学科先锋",
                personaDescription: `在${subject}中表现优异，多个知识点掌握牢固，具备较强的学科能力`,
                level: 4
            };
        if (accuracy >= 70 && strongRatio >= 0.5)
            return {
                subject,
                personaLabel: "稳健进取型",
                personaDescription: `${subject}基础扎实，部分知识点还需巩固`,
                level: 3
            };
        if (accuracy >= 50)
            return {
                subject,
                personaLabel: "均衡发展型",
                personaDescription: `${subject}各知识点发展较为均衡，持续练习可稳步提升`,
                level: 2
            };
        if (accuracy >= 30)
            return {
                subject,
                personaLabel: "基础建设型",
                personaDescription: `${subject}基础需要加强，建议从核心知识点开始系统学习`,
                level: 1
            };
        return {
            subject,
            personaLabel: "起步探索型",
            personaDescription: `${subject}刚刚起步，需要从头建立知识体系`,
            level: 0
        };
    }
}
