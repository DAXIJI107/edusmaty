...
class PathTool {
    ...
    async run({ userId, goal, subject = "all", intensity = "normal", weakPoints = [], resources = {}, practice = {} }) {
        ...
        await this.pool.query(
            'DELETE FROM study_tasks WHERE user_id = ? AND task_date = CURDATE() AND source = "agent-runtime"',
            [userId]
        ).catch(() => {});
        ...
        for (const point of selected) {
            ...
            // 匹配练习题数量
            const questionCount = (practice.questions || []).filter(q => Number(q.knowledge_id) === Number(point.id)).length;
            ...
            if (mastery < 40) {
                // 低掌握度：需要完整的学习路径
                tasks = [
                    { icon: "book", title: `预习先修：${prereqs.join('、') || '基础概念'}`, minutes: 20, subtitle: "补足前置知识" },
                    { icon: "video", title: `课程学习：${point.title}`, minutes: 30, subtitle: course ? `推荐：${course.title}` : "先看概念讲解" },
                    { icon: "exam", title: `基础练习：${point.title}`, minutes: 25, subtitle: `${questionCount || 3} 道基础题 · 做完后更新掌握度` },
                    { icon: "pen", title: `整理笔记：${point.title}`, minutes: 15, subtitle: "记录核心概念、常见错误和理解误区" }
                ];
            } else if (mastery < 70) {
                tasks = [
                    { icon: "exam", title: `巩固练习：${point.title}`, minutes: 20, subtitle: `${questionCount || 3} 道进阶题` },
                    { icon: "pen", title: `错题复盘：${point.title}`, minutes: 15, subtitle: errors > 0 ? `已累计 ${errors} 道错题，重点纠正` : "梳理易错点" },
                    { icon: "book", title: `拓展阅读：${point.title}`, minutes: 15, subtitle: course ? `参考：${course.title}` : "查阅资料深化理解" }
                ];
            } else {
                tasks = [
                    { icon: "exam", title: `挑战练习：${point.title}`, minutes: 15, subtitle: `${questionCount || 2} 道难题 · 检验掌握程度` },
                    { icon: "pen", title: `迁移笔记：${point.title}`, minutes: 10, subtitle: "总结可迁移的解题模式和技巧" }
                ];
            }
    ...
    async _getAllRelevantPoints(userId, subject, weakPoints) {
        ...
        if (ids.length) {
            ...
            const [prereqs] = await this.pool.query(
            ...
            const prereqMap = {};
            prereqs.forEach(r => { prereqMap[r.node_id] = r.prereq_titles ? r.prereq_titles.split('，') : []; });
    ...
    async _generateBaselineTasks(userId, goal, subject) {
        await this.pool.query(
            ...
            [
                userId, `建立诊断画像：${goal || subject}`,
                "智能体缺少足够画像数据，先完成一次快速诊断。",
                userId, `基线小测：${goal || subject}`,
                "用少量题目建立初始掌握度。",
                userId, `学习目标卡：${goal || subject}`,
                "写下目标、截止时间和当前最不确定的问题。"
            ]
        ).catch(() => {});
