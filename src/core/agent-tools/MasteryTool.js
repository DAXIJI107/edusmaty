...
class MasteryTool {
    ...
    async calculate(userId, knowledgeId) {
        ...
        // 3. 获取已有的学生知识记录
        const [studentK] = await this.pool
            .query(
        ...
        for (let i = 0; i < recent.length; i++) {
            const difficultyWeight =
                recent[i].difficulty === "hard" ? 1.5 : recent[i].difficulty === "medium" ? 1.2 : 1.0;
        ...
        const recencyHours =
            studentK.length && studentK[0].last_practice_at
        ...
        return {
            ...
            evidence: {
                ...
                redoCorrectRate: errorInfo.redo_total
                    ? Math.round((errorInfo.redo_correct / errorInfo.redo_total) * 100)
                    : null,
        ...
        await this.pool
            .query(
             ...
             ON DUPLICATE KEY UPDATE
                ...
                trend = VALUES(trend)`,
                [userId, knowledgeId, result.mastery, result.evidence.errorCount, result.trend]
            )
            .catch(() => {});

        // 4. 同步更新 knowledge_points (兼容旧表)
        await this.pool
            .query(`UPDATE knowledge_points SET mastery = ? WHERE id = ?`, [result.mastery, knowledgeId])
            .catch(() => {});
    ...
    async batchGet(userId, knowledgeIds) {
        ...
        rows.forEach(r => {
            map[r.node_id] = {
                ...
                lastPractice: r.last_practice_at
            };
        });
        return map;
    }
