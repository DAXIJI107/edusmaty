...
const TOOL_DEFINITIONS = [
    { key: 'git', name: 'Git 仓库', kind: 'tool', status: 'ready', description: '读取 diff、同步分支、生成提交计划和 PR 草稿。' },
    { key: 'mcp', name: 'MCP 工具总线', kind: 'mcp', status: 'ready', description: '统一转发外部 MCP Server，例如 GitHub、文档、浏览器、部署平台。' },
    { key: 'skill', name: 'Skill 技能库', kind: 'skill', status: 'ready', description: '把审查、测试、修正、发布检查封装为可复用技能。' },
    { key: 'agent', name: '多智能体编排', kind: 'agent', status: 'ready', description: '按需求分析、代码审查、测试生成、修复建议拆分角色协作。' },
    { key: 'ai-review', name: 'AI 代码审查', kind: 'ai', status: 'ready', description: '通过模型或本地规则识别质量、安全、可维护性和协作风险。' },
    { key: 'ai-test', name: 'AI 测试生成', kind: 'ai', status: 'ready', description: '基于文件内容生成测试思路、测试代码草稿和验收清单。' },
    { key: 'ai-fix', name: 'AI 自动修正', kind: 'ai', status: 'ready', description: '根据审查与测试结果生成最小修复建议，默认不直接覆盖代码。' },
    { key: 'ci', name: 'CI 流水线', kind: 'tool', status: 'ready', description: '运行语法检查、单元测试、页面冒烟和部署预检。' }
];
...
function buildTests({ content = '', moduleKey = 'frontend', path = '' }) {
    ...
    return {
        framework: moduleKey === 'frontend' ? 'Playwright / Jest' : moduleKey === 'backend' ? 'Jest / Supertest' : 'Jest',
...
class AIDevOpsOrchestrator {
    ...
    async reviewCode(input) {
        ...
        return {
            ...
            nextActions: [
                ...local.suggestions.slice(0, 2),
                '运行 AI 测试生成并把高风险项转为验收任务。'
            ]
        };
    }
    ...
    async generateTests(input) {
        ...
        return { ...testPlan, provider: model.provider, modelComment: model.content ? model.content.slice(0, 1200) : '' };
    }
    ...
    async runPipeline(input) {
        ...
        const gates = [
            { name: '代码审查', status: review.score >= 75 ? 'passed' : 'attention', detail: `${review.level} · ${review.score}分` },
            { name: '测试生成', status: testPlan.cases.length >= 3 ? 'passed' : 'attention', detail: `${testPlan.cases.length} 个测试场景` },
            { name: '修正建议', status: fixPlan.actions.length ? 'passed' : 'attention', detail: `${fixPlan.actions.length} 条建议` }
        ];
        return {
            provider: review.provider === 'spark' || testPlan.provider === 'spark' || fixPlan.provider === 'spark' ? 'spark' : 'local',
    ...
    async runTool({ tool, roleKey, task, project, file }) {
        const target = TOOL_DEFINITIONS.find(item => item.key === tool) || TOOL_DEFINITIONS.find(item => item.key === 'mcp');
