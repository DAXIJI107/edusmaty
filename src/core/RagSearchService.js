...
const STOP_WORDS = new Set([
    // 中文停用词
    '的', '了', '是', '在', '和', '就', '都', '而', '及', '与', '不', '也', '这', '那',
    '着', '之', '过', '但', '或', '很', '更', '最', '一', '个', '有', '人', '我', '你',
    '他', '她', '它', '们', '要', '会', '能', '可', '以', '对', '从', '到', '把', '被',
    '让', '向', '由', '将', '并', '中', '上', '下', '前', '后', '里', '外', '所', '用',
    '为', '已', '还', '又', '再', '没', '只', '如', '其', '去', '做', '该', '等', '因',
    '为', '所以', '如果', '虽然', '但是', '然后', '可以', '应该', '需要', '已经', '没有',
    '什么', '怎么', '哪', '吗', '呢', '吧', '啊', '哦', '嗯', '哈', '嘛', '呀', '啦',
    '通过', '进行', '实现', '使用', '包括', '提供', '具有', '发展', '问题', '方法',
    '技术', '系统', '数据', '信息', '知识', '学习', '研究', '工作', '主要', '方面',
    '基本', '不同', '比较', '重要', '关系', '过程', '方式', '结果', '情况', '内容',
    // 英文停用词
    ...
    'and', 'but', 'or', 'not', 'no', 'so', 'if', 'than', 'then', 'also', 'just',
    'now', 'up', 'down', 'only', 'very', 'too', 'more', 'some', 'any', 'each',
    'every', 'both', 'few', 'most', 'other', 'such', 'this', 'that', 'these', 'those',
    'it', 'its', 'he', 'she', 'they', 'them', 'we', 'you', 'us', 'all', 'there',
    'here', 'when', 'where', 'why', 'how', 'which', 'who', 'whom', 'what'
]);
...
class RagSearchService {
    ...
    async search({ query, subject = 'all', sourceName = null, userId = null, limit = 5 }) {
        ...
        const keywordConditions = queryTokens.slice(0, 6).map(() =>
            '(c.chunk_text LIKE ? OR c.knowledge_point LIKE ? OR c.course LIKE ?)'
        ).join(' OR ');
        ...
        if (userId) {
            await this.pool.query(
                'INSERT INTO rag_query_logs (user_id, query_text, subject, hit_count) VALUES (?, ?, ?, ?)',
                [userId, query, subject || 'all', ranked.length]
            ).catch(() => {});
        }
        ...
        // 构建可添加到学习列表的动作数据
        const topKnowledgePoints = [...new Set(
            evidenceChain.map(ev => ev.knowledgePoint).filter(Boolean)
        )].slice(0, 5);
...
RagSearchService.prototype.hybridSearch = async function ({
    query, subject = 'all', sourceName = null, userId = null, limit = 5
}) {
    // 先运行 BM25 搜索获取基线结果
