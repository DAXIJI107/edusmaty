...
class ProfileAgent {
    ...
    async initializeProfile() {
        try {
            const [rows] = await this.pool.query("SELECT * FROM student_profiles WHERE user_id = ?", [this.userId]);
    ...
    extractFeaturesFromMessage(message) {
        ...
        if (
            major &&
            /专业|我是|我学|学习|就读|计算机|软件|艺术|设计|医学|护理|会计|法学|建筑|新闻|传媒|电气|自动化|汉语言|心理|物流|市场营销|土木|机械|金融|经济|教育|英语|数学|物理|化学|生物/i.test(
                text
            )
        ) {
        ...
        try {
            const recognizedText =
                typeof audioData === "string"
                    ? audioData
                    : audioData && audioData.transcript
