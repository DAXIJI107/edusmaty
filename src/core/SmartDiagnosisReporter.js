...
class SmartDiagnosisReporter {
    ...
    async generateReport({ userId, catResult, profile, subject }) {
        ...
        const summary = this.generateSummary(catResult, cognitiveResult, radar, strengthAnalysis);
    ...
    generateRadarData(catResult, profile, cognitiveResult, bktMastery) {
        const dimensions = {
            labels: ["知识掌握", "认知能力", "学习韧性", "逻辑推理", "计算能力", "概念理解", "应用能力", "元认知"],
            datasets: [
                {
                    label: "当前诊断",
                    data: [
    ...
    calcLogicScore(cognitiveResult) {
        ...
        const logicScores = Object.entries(logicAttrs)
            .filter(([k]) => k === "逻辑推理")
            .map(([, v]) => v.mastery || 0);
        return logicScores.length > 0 ? Math.round(logicScores.reduce((a, b) => a + b, 0) / logicScores.length) : 50;
    }

    calcComputationScore(cognitiveResult) {
        if (!cognitiveResult) return 50;
        const allAttrs = Object.values(cognitiveResult).flatMap(d => Object.entries(d.dina?.attributeMastery || {}));
        const compScores = allAttrs
            .filter(([k]) => k === "计算能力" || k === "公式应用")
            .map(([, v]) => v.mastery || 0);
        return compScores.length > 0 ? Math.round(compScores.reduce((a, b) => a + b, 0) / compScores.length) : 50;
    }
    ...
    calcApplicationScore(cognitiveResult) {
        if (!cognitiveResult) return 50;
        const allBlooms = Object.values(cognitiveResult).flatMap(d => Object.entries(d.bloom?.levels || {}));
        const appScores = allBlooms
            .filter(([k]) => k === "应用" || k === "分析" || k === "创造")
            .map(([, v]) => v.mastery || 0);
        return appScores.length > 0 ? Math.round(appScores.reduce((a, b) => a + b, 0) / appScores.length) : 50;
    }
    ...
    generateKnowledgeHeatmap(bktMastery, filterSubject) {
        ...
        const filtered = filterSubject ? nodes.filter(n => n.subject === filterSubject) : nodes;
    ...
    generateBloomPyramid(cognitiveResult, catResult) {
            ...
            for (const [, data] of Object.entries(cognitiveResult)) {
                if (data.bloom?.levels) {
                    for (const [level, levelData] of Object.entries(data.bloom.levels)) {
                        ...
                        aggregatedCorrect[level] =
                            (aggregatedCorrect[level] || 0) +
        ...
        return {
            ...
            overallScore,
            dominantLevel:
                levels.reduce((best, l) => (l.mastery > (best?.mastery || 0) ? l : best), null)?.level || "记忆",
    ...
    generateMisconceptionCards(cognitiveResult) {
        ...
        const cards = allMisconceptions.slice(0, 10).map((mc, index) => ({
            ...
            severityColor: mc.severity === "critical" ? "#ef4444" : mc.severity === "moderate" ? "#f59e0b" : "#6366f1"
        }));
    ...
    generateStrengthAnalysis(catResult, cognitiveResult, bktMastery) {
        ...
        // 认知属性分析
        if (cognitiveResult) {
            const allAttrs = Object.values(cognitiveResult).flatMap(d =>
                Object.entries(d.dina?.attributeMastery || {})
            );
    ...
    generateLearningPathSuggestion(catResult, cognitiveResult, bktMastery, profile) {
        ...
        // 从BKT找出最需要学习的3个知识点
        const sortedNodes = Object.values(bktMastery).sort((a, b) => (a.mastery || 0) - (b.mastery || 0));
    ...
    generateProgressProjection(bktMastery, profile) {
        ...
        const weeklyImprovement =
            profile?.learningPatterns?.["学习速度"] === "fast"
                ? 0.1
                : profile?.learningPatterns?.["学习速度"] === "slow"
                  ? 0.03
                  : 0.06;
        ...
        const weeksToTarget =
            currentMastery < targetMastery ? Math.ceil((targetMastery - currentMastery) / weeklyImprovement) : 0;
    ...
    calculateOverallQuality(catResult, cognitiveResult, bktMastery) {
        ...
        const dataRichness =
            bktNodes >= 10 && cognitiveSubjects >= 2 && catQuestions >= 8
                ? "rich"
                : bktNodes >= 5
                  ? "adequate"
                  : "sparse";
        ...
        const reliability =
            (catResult?.abilitySE || 2) < 0.35 ? "high" : (catResult?.abilitySE || 2) < 0.55 ? "medium" : "low";

        return {
            overall:
                reliability === "high" && dataRichness === "rich"
                    ? "high"
                    : reliability === "low" || dataRichness === "sparse"
                      ? "low"
                      : "medium",
