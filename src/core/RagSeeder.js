...
async function ensureRagData(pool) {
    ...
    const questionTextColumn = questionColumns.has("content")
        ? "content"
        : questionColumns.has("question")
          ? "question"
          : null;
    const nodeColumn = questionColumns.has("node_id")
        ? "node_id"
        : questionColumns.has("knowledge_id")
          ? "knowledge_id"
          : null;
    const answerColumn = questionColumns.has("answer") ? "answer" : null;
    const questions =
        questionTextColumn && nodeColumn
            ? (
                  await pool.query(
                      `SELECT id,
    ...
    for (const node of nodes) {
        ...
        const qText = relatedQuestions
            .map((q, idx) => `题${idx + 1}:${q.content} 参考:${q.answer || "开放题"}`)
            .join(" ");
