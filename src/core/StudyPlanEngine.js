class StudyPlanEngine {
    ...
    withTaskIds(plan) {
        ...
        return {
            ...
            tasks: tasks.map((task, index) => ({
                ...(!task.materials || !task.practiceQuestions || !task.noteTemplate ? this.attachLearningSession(task) : task),
                id: `${plan.id}:${index}`,
                taskIndex: index,
    ...
    buildSummaryTask(time, focusNames) {
        return this.attachLearningSession({
            ...
            nodeName: '学习闭环',
            mastery: null,
            reason: focusNames.length > 0 ? `围绕 ${focusNames.join('、')} 做收束` : '把今天的学习结果转成明天可执行的动作',
            duration: 20,
            type: 'summary',
            priority: 'medium',
            icon: 'check',
            ...
            materials: [
                { title: '今日任务清单', type: 'plan', source: 'EduSmart', url: '/study-plan', instruction: '逐项检查任务产出是否真的留下。' },
                { title: '错题本', type: 'reflection', source: 'EduSmart', url: '/error-book', instruction: '把今天仍不稳定的题写入错因。' }
            ],
            practiceQuestions: [
                { title: '复盘问题 1', prompt: '今天最卡住的是哪个概念？卡住原因是什么？', expected: '能定位到一个具体概念或步骤。' },
                { title: '复盘问题 2', prompt: '明天第一道要做的题或第一个要看的材料是什么？', expected: '下一步明确到可马上执行。' }
            ],
    ...
    async generateDailyWithProfile(pool, { userId, date, title, duration }) {
        ...
        const tasks = this.buildTasksWithProfile(signals, {
            ...
            taskCount, agentSuggestions, title, duration
        });
    ...
    buildTasksWithProfile(signals, options) {
        ...
        if (title) {
            return [this.attachLearningSession({
                time: '自定', title, task: title,
                subject: 'general', subjectName: '自定义',
                nodeId: null, nodeName: title, mastery: null,
                reason: '你手动加入的学习任务',
                duration: duration || 30, type: 'custom', priority: 'medium',
                icon: 'plus', actionLabel: '开始学习', actionUrl: '/ai-tutor',
        ...
        tasks.push(this.buildLearningTask({
            time: '09:00', node, type: 'learn',
            reason: `当前掌握度${mastery}%，需建立框架`,
            duration: Math.min(attentionSpan, 30), priority: 'high'
        }));
        ...
        if (signals.weakNodes.length > 1) {
            tasks.push(this.buildLearningTask({
                ...
                type: 'practice', reason: '巩固练习',
                duration: Math.min(attentionSpan, 25), priority: 'medium'
            }));
        }
    ...
    async savePlan(pool, { userId, date, tasks, strategy }) {
        const planDate = date || this.today();
        const suggestion = tasks.length > 0
        ...
        if (plan) {
            ...
            plan = { id: result.insertId, user_id: userId, plan_date: planDate, plan_type: stg, tasks, ai_suggestion: suggestion, completion_rate: 0 };
        }
    ...
    async generateProfileAware(pool, { userId, date, profile, knowledgeMastery, cognitiveProfile, misconceptions }) {
        ...
        tasks.push(this.buildSummaryTask(
        ...
        if (plan) {
            ...
            plan = { id: result.insertId, user_id: userId, plan_date: planDate, plan_type: 'profile-aware', tasks, ai_suggestion: suggestion, completion_rate: 0 };
        }
    ...
    getStyleStrategy(cognitiveType) {
        const strategies = {
            visual: {
                ...
                materials: (topic) => [
                    { title: `${topic} 概念图解`, type: 'concept-map', source: 'EduSmart', url: `/concept-canvas?topic=${encodeURIComponent(topic)}`, instruction: '通过可视化概念图理解知识结构' },
                    { title: `${topic} 教学视频`, type: 'video', source: 'EduSmart', url: '/video-library', instruction: '观看教学视频，注意画面中的标注和动画' }
                ]
            },
            auditory: {
                label: '听觉',
                ...
                materials: (topic) => [
                    { title: `${topic} AI语音讲解`, type: 'audio', source: 'EduSmart', url: '/ai-tutor?mode=voice', instruction: '听AI助教语音讲解核心概念' },
                    { title: `${topic} 讨论练习`, type: 'discussion', source: 'EduSmart', url: '/ai-tutor', instruction: '与AI进行问答讨论，加深理解' }
                ]
            },
            kinesthetic: {
                label: '动觉',
                ...
                materials: (topic) => [
                    { title: `${topic} 交互练习`, type: 'interactive', source: 'EduSmart', url: '/practice?mode=interactive', instruction: '通过交互式练习动手验证概念' },
                    { title: `${topic} 实操任务`, type: 'lab', source: 'EduSmart', url: '/practice', instruction: '完成动手实操任务' }
                ]
            }
        };
    ...
    generateProfileSuggestion({ profile, weakNodes, misconceptionTargets, cognitiveType, learningSpeed, tasks }) {
        ...
        if (misconceptionTargets.length > 0) {
            parts.push(`检测到「${misconceptionTargets[0].category}」类误区占比${misconceptionTargets[0].percentage}%，安排了专项纠错任务`);
        }
    ...
    async generateDaily(pool, { userId, date, title, duration }) {
        ...
        if (plan) {
            ...
            plan = { id: result.insertId, user_id: userId, plan_date: planDate, plan_type: 'daily', tasks, ai_suggestion: suggestion, completion_rate: 0 };
        }
    ...
    async getPlan(pool, { userId, date, autoGenerate = true }) {
        ...
        const [rows] = await pool.query(
            `SELECT * FROM study_plans WHERE user_id = ? AND plan_date = ? LIMIT 1`,
            [userId, planDate]
        );
        if (rows.length === 0) {
    ...
    async setTaskCompletion(pool, { userId, taskId, completed }) {
        ...
        await pool.query(
            `UPDATE study_plans SET tasks = ?, completion_rate = ?, updated_at = NOW() WHERE id = ?`,
            [JSON.stringify(tasks), completionRate, parsed.planId]
        );
    ...
    async getProgress(pool, { userId }) {
        ...
        const weekMinutes = weekRows.reduce((sum, row) => {
            return sum + this.parseJson(row.tasks, [])
        ...
        return {
            ...
            progress: {
                recentDays: weekRows,
                averageCompletion: weekRows.length > 0
                    ? Math.round(weekRows.reduce((sum, row) => sum + Number(row.completion_rate || 0), 0) / weekRows.length)
                    : 0,
