class DynamicLearningPath {
    ...
    async assessUserLevel(userId, pool) {
        const [rows] = await pool.query("SELECT AVG(mastery) as avg_mastery FROM student_knowledge WHERE user_id = ?", [
            userId
        ]);
    ...
    async getUserMasteryMap(userId, pool) {
        const [rows] = await pool.query("SELECT node_id, mastery FROM student_knowledge WHERE user_id = ?", [userId]);
        const map = {};
