...
class LearningAgent {
    ...
    async getPermissions() {
        const [rows] = await this.pool.query("SELECT * FROM agent_permissions WHERE user_id = ?", [this.userId]);
        if (rows.length === 0) {
            // 默认权限
            return {
    ...
    async checkPermission(actionType) {
        ...
        switch (actionType) {
            case "BOOK_TUTOR":
                return perms.allow_auto_tutor;
            case "FORM_STUDY_GROUP":
                return perms.allow_auto_group;
            case "SCHEDULE_REVIEW":
                return perms.allow_auto_review;
            case "PURCHASE_COURSE":
                return perms.allow_auto_purchase;
            default:
                return false;
        }
    }
