class SmartPaperGenerator {
    async generate(pool, { userId, subject, difficulty, questionCount = 20, focusNodes = [] }) {
        ...
        const masteryMap = {};
        knowledge.forEach(k => { masteryMap[k.node_id] = k.mastery || 50; });
        ...
        allNodes.forEach(n => { nodeDifficultyMap[n.id] = n.difficulty || 'medium'; });
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (weakNodes.length > 0 && rand < 0.50) {
                nodePool = weakNodes.length > 0 ? weakNodes : allNodes.map(n => n.id);
            } else if (mediumNodes.length > 0 && rand < 0.85) {
                nodePool = mediumNodes.length > 0 ? mediumNodes : (weakNodes.length > 0 ? weakNodes : allNodes.map(n => n.id));
            } else {
