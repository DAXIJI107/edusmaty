...
function detectInputType({ text, imageData, fileName }) {
    ...
    if (imageData && (content.includes('报错') || content.includes('error') || name.includes('code'))) return 'code_image';
    if (imageData) return 'formula_image';
    if (content.includes('error') || content.includes('exception') || content.includes('报错') || content.includes('代码')) return 'code';
    return 'text';
}
...
function buildXfyunAuthUrl(rawUrl) {
    ...
    const signature = crypto
        .createHmac('sha256', config.ocr.apiSecret)
        ...
        authUrl,
        {
            ...
            parameter: {
                ocr: {
                    ...
                    exif_option: '0',
                    markdown_element_option: inputType === 'formula_image'
        ...
        res.json({
            ...
            data: {
                ...
                ocr: {
                    ...
                    provider: ocrText && !ocrText.includes('可在生产环境接入讯飞OCR') ? 'xfyun-ocr-large-model' : 'xfyun-ocr-fallback',
                    text: ocrText
                },
                answer,
