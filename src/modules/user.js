...
async function resolveDeactivateStatusValue() {
    try {
        ...
        const values = enumMatch[1].split(",").map(item => item.trim().replace(/^'/, "").replace(/'$/, ""));
...
router.get("/preferences", authenticateJWT, async (req, res) => {
    try {
        ...
        const [rows] = await pool.query("SELECT preferences, notifications FROM user_preferences WHERE user_id=?", [
            req.user.id
        ]);
        res.json({
            success: true,
...
router.delete("/account", authenticateJWT, async (req, res) => {
    try {
       ...
       ON DUPLICATE KEY UPDATE delete_reason=VALUES(delete_reason)`,
        ...
        await pool.query("UPDATE users SET status = ?, updated_at = NOW() WHERE id = ?", [inactiveValue, req.user.id]);
