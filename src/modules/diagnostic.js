...
const { normalizeQuestionType, normalizeAnswerForCompare, getQuestionTypeFromOptions } = require('../core/SubjectUtils');
...
router.get('/subject-test', authenticateJWT, async (req, res) => {
    try {
        ...
        if (!rows.length) {
            return res.json({ success: true, data: { subject, questions: [], tip: '该学科暂无诊断题目，请联系教师添加' } });
        }
    ...
    try {
        ...
        for (const q of questions) {
            ...
            if (type === 'single') {
                ...
                const userLetters = userAnswerStr.split(',').map(s => s.trim()).filter(Boolean);
                const userTexts = userLetters.map(l => {
                ...
                try {
                    ...
                    correctTexts = (Array.isArray(parsed) ? parsed : [String(q.correct_answer)]).map(s => String(s).trim()).sort();
                } catch (e) {
            ...
            if (isCorrect) { correctCount++; totalScore += score; }
        ...
        sections: [
            {
                title: '视觉偏好 (Visual)',
                questions: [
                    { id: 'v1', text: '我喜欢通过图表、流程图来理解概念', options: ['非常符合', '比较符合', '一般', '不太符合', '完全不符'] },
                    { id: 'v2', text: '看书时我习惯用不同颜色标记重点', options: ['非常符合', '比较符合', '一般', '不太符合', '完全不符'] },
                    { id: 'v3', text: '我喜欢看教学视频而不是纯文字教程', options: ['非常符合', '比较符合', '一般', '不太符合', '完全不符'] }
                ]
            },
            {
                title: '听觉偏好 (Auditory)',
                questions: [
                    { id: 'a1', text: '听老师讲解比看教材更容易让我理解', options: ['非常符合', '比较符合', '一般', '不太符合', '完全不符'] },
                    { id: 'a2', text: '我喜欢通过讨论学习新知识', options: ['非常符合', '比较符合', '一般', '不太符合', '完全不符'] },
                    { id: 'a3', text: '朗读或听到自己的声音能帮我记忆', options: ['非常符合', '比较符合', '一般', '不太符合', '完全不符'] }
                ]
            },
            {
                title: '读写偏好 (Read/Write)',
                questions: [
                    { id: 'r1', text: '我喜欢用文字笔记整理学习内容', options: ['非常符合', '比较符合', '一般', '不太符合', '完全不符'] },
                    { id: 'r2', text: '我更偏向通过阅读教材来学习', options: ['非常符合', '比较符合', '一般', '不太符合', '完全不符'] },
                    { id: 'r3', text: '把学到的东西写下来能帮我更好地记住', options: ['非常符合', '比较符合', '一般', '不太符合', '完全不符'] }
                ]
            },
            {
                title: '动觉偏好 (Kinesthetic)',
                questions: [
                    { id: 'k1', text: '动手操作让我学得更快', options: ['非常符合', '比较符合', '一般', '不太符合', '完全不符'] },
                    { id: 'k2', text: '我喜欢通过做实验/练习来验证知识', options: ['非常符合', '比较符合', '一般', '不太符合', '完全不符'] },
                    { id: 'k3', text: '坐着不动学习让我容易走神', options: ['非常符合', '比较符合', '一般', '不太符合', '完全不符'] }
                ]
            }
        ],
...
router.get('/knowledge-tracing', authenticateJWT, async (req, res) => {
    try {
        ...
        const summary = {
            ...
            overallMastery: Object.keys(mastery).length > 0
                ? Math.round(Object.values(mastery).reduce((s, v) => s + v.mastery, 0) / Object.keys(mastery).length * 100)
                : 0
        };
...
router.get('/cognitive-profile', authenticateJWT, async (req, res) => {
    try {
        ...
        for (const [attr, data] of Object.entries(aggregatedAttrs)) {
            ...
            data.state = data.mastery >= 75 ? 'mastered' :
                         data.mastery >= 50 ? 'developing' :
                         data.mastery >= 25 ? 'beginner' : 'weak';
        }
