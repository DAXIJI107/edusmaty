...
router.get("/resources/generate-stream", authenticateJWT, async (req, res) => {
    ...
    try {
        ...
        send("progress", {
            step: "资源类型",
            percent: 20,
            message: `准备生成${selectedTypes.length}种类型资源`,
            ...
            try {
                ...
                if (resourceResult.success && resourceResult.resources) {
                    for (const r of resourceResult.resources) {
                        const safetyCheck = await orchestrator.agents
                            .get("safety")
                            .validateAndFilter(r.content || "", profile.basicInfo?.major);
...
async function executeAgentAction(userId, actionType, params = {}) {
    ...
    switch (actionType) {
        ...
        case "SCHEDULE_REVIEW":
            result = await agent.scheduleReview(
                params.knowledgePoints?.length ? params.knowledgePoints : await pickReviewTopics(userId)
            );
            break;
