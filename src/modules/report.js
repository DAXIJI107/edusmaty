...
router.post("/", authenticateJWT, async (req, res) => {
  ...
  LIMIT 5`,
        ...
        const efficiency = {
            ...
            memory: Math.max(
                ...
                Math.min(
                    100,
                    Math.round(
                        knowledgeNodes.reduce((sum, item) => sum + Number(item.mastery || 0), 0) /
                            Math.max(knowledgeNodes.length, 1)
                    )
                )
            ),
            speed: Math.max(
                0,
                Math.min(100, Math.round((durationTrend.length / Math.max(7, durationTrend.length)) * 100))
            ),
        ...
        const efficiencyValue = Math.round(
            Object.values(efficiency).reduce((sum, item) => sum + item, 0) / Object.keys(efficiency).length
        );
        const summary = {
        ...
        const rawDistribution = knowledgeNodes.reduce(
            (acc, node) => {
