...
const ROLE_TEMPLATES = [
    { roleKey: 'frontend', roleName: '页面体验负责人', moduleName: '页面与交互', responsibility: '负责把学习项目做成清晰可用的页面，练习布局、交互和接口联调。', defaultPath: 'frontend/App.js', language: 'javascript' },
    { roleKey: 'backend', roleName: '功能逻辑负责人', moduleName: '业务逻辑', responsibility: '负责项目规则、数据处理和权限判断，练习把需求转成稳定功能。', defaultPath: 'backend/api.js', language: 'javascript' },
    { roleKey: 'testing', roleName: '质量复盘负责人', moduleName: '测试与验收', responsibility: '负责测试用例、问题记录、验收标准和复盘反馈。', defaultPath: 'tests/project.test.js', language: 'javascript' },
    { roleKey: 'deployment', roleName: '发布展示负责人', moduleName: '展示与发布', responsibility: '负责运行说明、展示流程、环境检查和成果汇报。', defaultPath: 'deploy/README.md', language: 'markdown' }
];

const REQUIREMENT_TEMPLATES = [
    { id: 'REQ-101', moduleKey: 'frontend', title: '项目首页与任务看板', status: 'doing', priority: 'P0', acceptance: '能展示项目目标、成员分工、需求卡片和最新提交。' },
    { id: 'REQ-102', moduleKey: 'backend', title: '任务与提交记录接口', status: 'review', priority: 'P0', acceptance: '保存代码后产生版本、修改位置、操作者和时间记录。' },
    { id: 'REQ-103', moduleKey: 'testing', title: '核心流程自动化验收', status: 'todo', priority: 'P1', acceptance: '覆盖创建项目、打开文件、保存同步、AI 审查四条路径。' },
    { id: 'REQ-104', moduleKey: 'deployment', title: '演示环境与发布清单', status: 'todo', priority: 'P1', acceptance: '包含环境变量、启动命令、健康检查和演示 URL。' }
];
...
async function assertProjectAccess(projectId, userId) {
    ...
    const [[member]] = await pool.query('SELECT id FROM team_project_members WHERE project_id = ? AND user_id = ? LIMIT 1', [projectId, userId]);
...
async function pickDemoUsers(currentUserId) {
    const [students] = await pool.query('SELECT id FROM users WHERE role = "student" AND status = "active" ORDER BY id LIMIT 4');
...
async function createProjectWithDefaults(userId, payload = {}) {
    ...
    const description = (payload.description || '学生在教师指导下分工完成一个可展示的学习项目，覆盖页面、功能、测试和成果发布。').trim();
    ...
    for (let i = 0; i < ROLE_TEMPLATES.length; i += 1) {
        ...
        const [fileResult] = await pool.query(
            ...
            [projectId, role.roleKey, role.defaultPath, role.language, content, memberUserId, memberUserId, Buffer.byteLength(content, 'utf-8')]
        );
        await pool.query(
            ...
            [projectId, fileResult.insertId, memberUserId, `初始化${role.moduleName}模块`, role.roleKey, role.defaultPath, content.split(/\r?\n/).length, content]
        );
    }
    await pool.query(
...
async function projectDetail(projectId) {
    ...
    return { project, members, files, commits, events, aiRuns, moduleStats, requirements: REQUIREMENT_TEMPLATES, tools: EXTERNAL_TOOLS, repoHealth, agents };
}
...
async function resolvePipelineInput(projectId, body = {}) {
    ...
    let file = null;
    if (fileId) {
        const [[found]] = await pool.query('SELECT * FROM team_project_files WHERE id = ? AND project_id = ?', [fileId, projectId]);
        if (found) {
            file = found;
...
router.get('/projects/:id/files/:fileId', authenticateJWT, async (req, res) => {
    try {
        ...
        const project = await assertProjectAccess(projectId, getUserId(req));
        if (!project) return res.status(404).json({ success: false, message: '项目不存在或无权限访问' });
        const [[file]] = await pool.query('SELECT * FROM team_project_files WHERE id = ? AND project_id = ?', [fileId, projectId]);
...
router.get('/projects/:id/download', authenticateJWT, async (req, res) => {
    try {
        ...
        const exportFiles = readme ? files : [{
            path: 'README.md',
...
router.post('/projects/:id/files/save', authenticateJWT, async (req, res) => {
    try {
        ...
        const [[existing]] = await pool.query('SELECT * FROM team_project_files WHERE project_id = ? AND path = ?', [projectId, filePath]);
        let fileId;
        let version = 1;
        if (existing) {
            fileId = existing.id;
        ...
        await pool.query('UPDATE team_projects SET updated_at = NOW(), progress = LEAST(100, progress + 3) WHERE id = ?', [projectId]);
...
router.post('/projects/:id/ai-pipeline', authenticateJWT, async (req, res) => {
    try {
        ...
        await recordAiRun({ projectId, userId, runType: `ai-${mode}`, result, targetPath: input.path, moduleKey: input.moduleKey });
        await pool.query(
            'INSERT INTO team_project_events (project_id, user_id, event_type, title, detail) VALUES (?, ?, ?, ?, ?)',
            [projectId, userId, 'ai_pipeline', 'AI DevOps 流水线完成', `${input.path} · ${mode} · ${result.provider || 'local'}`]
        );
