...
router.post("/recommend-review", authenticateJWT, async (req, res) => {
    try {
        ...
        // 获取课程所有节点
        const [nodes] = await pool.query("SELECT * FROM rhythm_nodes WHERE course_id = ? ORDER BY time_seconds", [
            courseId
        ]);
        ...
        for (const node of missedNodes) {
            ...
            const [questions] = await pool.query("SELECT id, content FROM questions WHERE node_id = ? LIMIT 2", [
                node.id
            ]);
