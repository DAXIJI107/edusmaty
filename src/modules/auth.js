...
function signUser(user) {
    return jwt.sign({ id: user.id, username: user.username, role: user.role || "student" }, JWT_SECRET, {
...
router.post("/bootstrap-admin", async (req, res) => {
    try {
        ...
        const [exists] = await pool.query("SELECT id, role FROM users WHERE username = ? OR email = ? LIMIT 1", [
            username,
            email
        ]);
        if (exists.length) {
...
router.post("/register-teacher", authenticateJWT, async (req, res) => {
    try {
        ...
        const hashedPassword = await bcrypt.hash(password, 10);
        await pool.query("INSERT INTO users (username, email, password, role) VALUES (?, ?, ?, 'teacher')", [
            username,
            email,
            hashedPassword
        ]);
