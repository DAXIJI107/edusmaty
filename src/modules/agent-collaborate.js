...
function inferSourceName(topic) {
    ...
    return (
        ["Hello-Agents", "OATutor", "OpenMAIC", "NexusRAG", "AgentScope"].find(name =>
...
router.post("/start", authenticateJWT, async (req, res) => {
    try {
        ...
        if (sourceName || /开源|公开|github|rag|agent/i.test(topic)) {
            ingestResult = await ingestPublicSources(pool, {
                sourceName: sourceName || "all",
                limit: sourceName ? 1 : 3
        ...
        for (const agentConfig of agents) {
            const content = await callSparkAgent(`You are the ${agentConfig.role}`, agentConfig.prompt);
