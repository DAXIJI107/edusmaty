        ...
        data: [
            {
                ...
                endpoint: config.xfyun.iatWsApi
            },
            {
                key: "ocr",
                name: "通用文档识别 OCR 大模型",
                type: "WebApi",
                ready: credentialReady(config.ocr),
                endpoint: config.ocr.apiUrl
            },
            {
                key: "image",
                name: "图片理解",
                type: "WebSocket",
                ready: credentialReady(),
                endpoint: config.xfyun.imageWsApi
            },
            {
                key: "ppt",
                name: "智能 PPT 生成",
                type: "WebApi",
                ready: credentialReady(),
                endpoint: config.xfyun.pptApiUrl
            },
            {
                key: "tts",
                name: "语音合成",
                type: "WebSocket",
                ready: credentialReady(config.tts),
                endpoint: config.tts.wsApi
            }
        ]
    });
});
        ...
        const messages = [
            {
                role: "system",
                content:
                    "你是一个专业的翻译助手，能够准确地将中文翻译成英文，将英文翻译成中文。请直接输出翻译结果，不要添加任何额外的解释或说明。"
            },
            {
                role: "user",
...
router.post("/ocr/document", async (req, res) => {
    try {
        if (!credentialReady(config.ocr)) {
            return res
                .status(400)
                .json({ success: false, message: "请先配置 XFYUN_OCR_APP_ID / API_KEY / API_SECRET" });
        }
...
router.post("/ppt/create", async (req, res) => {
    try {
        ...
        if (!query && !outline && !title)
            return res.status(400).json({ success: false, message: "缺少 PPT 主题或大纲" });
        ...
        const response = await axios.post(
            `${config.xfyun.pptApiUrl}${separator}${authQuery}`,
            {
                appId: APP_ID,
                query: query || title,
                outline,
