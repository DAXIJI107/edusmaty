...
function goalMatchScore(point, goal) {
    ...
    return Math.min(
        22,
        keywords.reduce((score, keyword) => score + (text.includes(keyword) ? 7 : 0), 0)
    );
}
...
function personalizedPathScore(point, { goal, intensity, profileContext }) {
    ...
    const intensityScore =
        intensity === "light" && mastery < 45 ? -12 : intensity === "intense" && questionCount > 20 ? 10 : 0;
    const profileScore = (profileContext.learningStyle || []).some(style =>
        /practice|interactive|视觉|互动|diagram|video/.test(String(style))
    )
        ? questionCount > 0
            ? 6
            : 0
        : 0;
    return Math.round(
        weakScore + wrongScore + questionScore + noteGapScore + targetScore + intensityScore + profileScore
    );
}
...
function localAssistantReply({ prompt, mode, context }) {
    ...
    return [
        ...
        `你现在可以优先处理：${weak}。`,
        prompt
            ? `针对你刚才的问题：“${String(prompt).slice(0, 80)}”，建议先拆成「概念定位、关键条件、例题验证、复盘卡片」四步。`
            : "请输入课程问题、错题截图文字、笔记草稿或复习目标。",
        "下一步：把最卡的一道题或一段课程内容贴进来，我可以继续生成讲解、追问、复习卡和学习计划。"
    ].join("\n\n");
}

async function getAssistantContext(userId) {
    const [weakPoints] = await pool
        .query("SELECT title, subject, mastery, summary FROM knowledge_points ORDER BY mastery ASC, id LIMIT 5")
        .catch(() => [[]]);
    ...
    const [courses] = await pool
        .query("SELECT title, provider, subject, progress FROM courses ORDER BY progress DESC, id LIMIT 5")
        .catch(() => [[]]);
    ...
    res.json({
        ...
        metrics: [
            toMetric(
                ...
                "#5f58ee",
                "rgba(95,88,238,.18)",
                "linear-gradient(135deg,#5f58ee,#7c4dff)"
            ),
            toMetric(
                "brain",
                "知识掌握度",
                ...
                "#18b87a",
                "rgba(24,184,122,.16)",
                "linear-gradient(135deg,#18b87a,#1ecda4)"
            ),
            toMetric(
                "bolt",
                "学习效率",
                ...
                "#ff9500",
                "rgba(255,149,0,.16)",
                "linear-gradient(135deg,#ff9500,#ffb020)"
            ),
            toMetric(
                "flame",
                "连续学习",
                ...
                "#7c4dff",
                "rgba(124,77,255,.16)",
                "linear-gradient(135deg,#7c4dff,#8f5bff)"
            )
        ],
...
function buildAgentConsole({ profile, weakPoints, tasks, notes, messages, cards, reviews, radar, teacher, services }) {
    ...
    const avgRadar = radar.length
        ? Math.round(
              radar.reduce(
                  (sum, item) => sum + normalizeMastery(item.understanding_score || item.application_score, 50),
                  0
              ) / radar.length
          )
    ...
    const masteryAfter = normalizeMastery(
        masteryBefore + Math.min(14, cards.length * 2 + doneTasks * 3 + messages.length)
    );
    ...
    return {
        ...
        conversation: {
            ...
            lastReply:
                latestAi?.content ||
                `我建议先从「${primaryWeak.title}」最容易混淆的一步开始：你能说出它和相邻概念的区别吗？`,
        ...
        teacherView: {
            ...
            recommendedActions: [
                {
                    key: "assign-remediation",
                    title: `发布 ${primaryWeak.subject} 补救练习`,
                    target: primaryWeak.subject
                },
                { key: "push-test", title: `发布 ${primaryWeak.subject} 分层测试`, target: primaryWeak.subject },
                {
                    key: "create-note-task",
                    title: `要求整理 ${primaryWeak.title} 复盘笔记`,
                    target: primaryWeak.subject
                }
            ]
        },
...
router.get("/intelligence", async (req, res, next) => {
    try {
        ...
        const [messages] = session
            ? await pool.query(
                  "SELECT sender, content, strategy, emotion_signal, created_at FROM tutor_messages WHERE session_id = ? ORDER BY id LIMIT 8",
                  [session.id]
              )
            : [[]];
        ...
        const agentConsole = buildAgentConsole({
            profile,
            weakPoints,
            tasks,
            notes: [],
...
router.post("/intelligence/run-agent-flow", async (req, res, next) => {
    try {
        ...
        const focus =
            ...
            (
                await pool.query(
                    "SELECT id, title, subject, mastery, summary FROM knowledge_points ORDER BY mastery ASC LIMIT 1"
                )
            )[0][0];
        const answers = [
        ...
        const diagnosis = {
            irt: {
                theta: Number(((answers.filter(item => item.correct).length - 2) / 4).toFixed(3)),
                confidence: 0.78
            },
        ...
        await pool.query(
            ...
            [
                ...
                JSON.stringify({
                    root: goal,
                    prerequisites: [focus?.title || topic || "当前薄弱点"],
                    next: ["导师追问", "补救练习", "智能笔记", "间隔复习"]
                })
            ]
        );
        ...
        const planItems = [
            [
                `AI 讲解：${focus?.title || topic || "薄弱点"}`,
                `${focus?.subject || "综合"} · 先听讲解再回答追问`,
                "robot",
                12
            ],
        ...
        for (let i = 0; i < planItems.length; i += 1) {
            ...
            await pool.query(
                ...
                [
                    ...
                    20 + i,
                    i === 1 ? "#18b87a" : "#2f6bff",
                    i === 1 ? "rgba(24,184,122,.12)" : "rgba(47,107,255,.12)"
                ]
            );
        }
        const content = {
        ...
        await pool.query(
            ...
            [
                userId,
                focus?.id || null,
                `${focus?.title || topic || "薄弱点"} · 智能体复盘卡`,
                JSON.stringify(content),
                JSON.stringify(["智能体任务流", "主动回忆"])
            ]
        );
        let [[session]] = await pool.query(
            'SELECT id FROM tutor_sessions WHERE user_id = ? AND status = "active" ORDER BY id DESC LIMIT 1',
            [userId]
        );
        if (!session) {
            const [created] = await pool.query(
                'INSERT INTO tutor_sessions (user_id, mode, topic) VALUES (?, "agent_flow", ?)',
                [userId, focus?.title || topic || "智能体任务流"]
            );
        ...
        await pool.query(
            'INSERT INTO tutor_messages (session_id, sender, content, strategy, emotion_signal) VALUES (?, "user", ?, "agent_flow", "focused")',
            [session.id, tutorPrompt]
        );
        await pool.query(
            'INSERT INTO tutor_messages (session_id, sender, content, strategy, emotion_signal) VALUES (?, "ai", ?, "socratic_probe", "supportive")',
            [session.id, tutorReply]
        );
        await pool.query(
        ...
        res.json({
            success: true,
            focus: focus?.title || topic || "当前薄弱点",
            diagnosis,
            generated: planItems.length,
            reply: tutorReply
        });
    } catch (error) {
        next(error);
        ...
        const services = [
            {
                ...
                value:
                    config.llm.provider === "spark"
                        ? maskSecret(config.search.apiPassword || config.spark.apiKeySecret)
...
router.post("/practice/answer", async (req, res, next) => {
    try {
        ...
        await pool.query("INSERT INTO user_answers (user_id, question_id, answer, is_correct) VALUES (?, ?, ?, ?)", [
            userId,
            questionId,
            answer,
            isCorrect ? 1 : 0
...
router.get("/practice/set", async (req, res, next) => {
    try {
        ...
        if (mode === "test") {
            ...
            const profileContext = pathGenerator.getProfileContext(
                rawProfile,
                styleWeights,
                dailyMinutes,
                attentionSpan
            ...
            if (focusIds.length) {
                ...
                return res.json({
                    ...
                    duration: Math.min(
                        35,
                        Math.max(15, Math.ceil((Number(profileContext.attentionSpan || 25) + limit) / 5) * 5)
                    ),
    ...
    try {
        ...
        const dimensions = [
            ...
            { key: "practice", label: "练习稳定", value: Math.max(20, Math.min(96, accuracy || 50)) },
            {
                key: "review",
                label: "复习沉淀",
                value: Math.max(18, Math.min(95, notes.reduce((sum, item) => sum + Number(item.total || 0), 0) * 12))
            },
            { key: "execution", label: "执行效率", value: Math.max(20, Math.min(96, efficiency || 50)) },
            {
                key: "persistence",
                label: "学习持续",
                value: Math.max(20, Math.min(96, Number(user?.continuous_days || 0) * 7))
            },
            {
                key: "transfer",
                label: "迁移应用",
                value: Math.max(18, Math.min(90, Math.round((mastery + accuracy) / 2) - 8))
            }
        ];
        ...
        const recommendations = [
            {
                title: "生成个性化路径",
                reason: weakPoints[0]
                    ? `先修复 ${weakPoints[0].title}，再进入同学科迁移练习。`
                    : "根据画像重排学习路径。",
                action: "去规划",
                target: "path"
            },
            {
                title: "整理错题笔记",
                reason: recentAnswers.some(item => !item.is_correct)
                    ? "最近有错题记录，适合沉淀为主动回忆卡。"
                    : "保持笔记复习能提升长期保持率。",
                action: "写笔记",
                target: "smartNotes"
...
router.get("/path/center", async (req, res, next) => {
    try {
        ...
        const pathNodes = agentTasks.map((task, index) => {
            ...
            const status =
                task.status === "done" ? "done" : mastery < 45 ? "priority" : mastery < 70 ? "learning" : "review";
...
router.post("/path/node/:id/start", async (req, res, next) => {
    try {
        ...
        const [[point]] = await pool.query(
            "SELECT id, title, subject, mastery FROM knowledge_points WHERE id = ? LIMIT 1",
            [nodeId]
        );
        ...
        const recommendations = [
            weakCount > 0
                ? {
                      ...
                      reason: weakPoints[0]
                          ? `「${weakPoints[0].title}」掌握度仅 ${weakPoints[0].mastery}%，建议先巩固。`
                          : "检测到知识点薄弱，建议优先强化。",
                      action: "开始练习",
                      target: "practice",
                      priority: 95
...
router.post("/course/:id/progress", async (req, res, next) => {
    try {
        ...
        const [[course]] = await pool.query("SELECT id, title, progress, subject FROM courses WHERE id = ? LIMIT 1", [
            courseId
        ]);
        ...
        await agentRuntime.recordEvent({
            ...
            payload: {
                title: course.title,
                previousProgress: Number(course.progress || 0),
    ...
    try {
        ...
        const [result] = await pool.query(
            ...
            [
                ...
                body,
                subject,
                JSON.stringify(tags.length ? tags : inferTags(`${title}\n${body}`, subject)),
                noteId,
                userId
            ]
        );
    ...
    try {
        ...
        const [result] = await pool.query(
            ...
            [
                userId,
                noteId,
                note.knowledge_id || null,
                `${note.title} · 复习卡`,
                JSON.stringify(content),
                JSON.stringify([note.subject || "综合", "笔记复习"])
            ]
        );
        await pool.query(
...
router.post("/report/generate", async (req, res, next) => {
    try {
        ...
        const [weak] = await pool.query(
            "SELECT title, subject, mastery FROM knowledge_points ORDER BY mastery ASC LIMIT 3"
        );
        const summary = {
            ...
            weak,
            advice: weak.length
                ? `优先修复 ${weak.map(w => w.title).join("、")}，再进行分科测试。`
                : "继续保持综合练习。"
        };
        await pool.query(
            `INSERT INTO activities (user_id, icon, title, time_label, badge, color, soft_color)
...
router.post("/practice/submit-set", async (req, res, next) => {
    try {
        ...
        await pool.query(
            ...
            [
                userId,
                `完成${mode === "exam" ? "在线考试" : mode === "test" ? "阶段测试" : "专项练习"}，得分 ${score}`,
                mode === "exam" ? "在线考试" : "练测闭环"
            ]
        );
...
router.post("/diagnosis/submit", async (req, res, next) => {
    try {
        ...
        const diagnosis = {
            irt: { theta, confidence: 0.7 + Math.min(0.2, answers.length / 50) },
            weakConcepts: weak.map(x => x.title),
            readiness: theta > 0 ? "可进入主线学习" : "建议先修复前置概念"
        };
        const misconceptions = weak.map((item, index) => ({
            concept: item.title,
            severity: Number((0.72 - index * 0.09).toFixed(2))
        }));
        const graph = {
            root: goal,
            prerequisites: weak.map(x => x.title),
            next: ["直观解释", "变式练习", "费曼输出", "迁移项目"]
        };
        await pool.query(
            `INSERT INTO cognitive_profiles (user_id, ability_theta, diagnosis_json, misconception_json, goal_text, goal_graph_json)
             VALUES (?, ?, CAST(? AS JSON), CAST(? AS JSON), ?, CAST(? AS JSON))`,
        ...
        await pool.query(
            ...
            [
                userId,
                preference,
                JSON.stringify({ text: 0.7, video: 0.58, dialogue: 0.92, experiment: 0.8 }),
                JSON.stringify({ source: "diagnosis_interaction", answers: answers.length })
            ]
        );
...
router.post("/tutor/message", async (req, res, next) => {
    try {
        ...
        const [[human]] = await pool.query("SELECT id FROM digital_humans WHERE user_id = ? ORDER BY id LIMIT 1", [
            userId
        ]);
        let [[session]] = await pool.query(
            'SELECT id FROM tutor_sessions WHERE user_id = ? AND status = "active" ORDER BY id DESC LIMIT 1',
        ...
        if (!session) {
            const [created] = await pool.query(
                "INSERT INTO tutor_sessions (user_id, digital_human_id, mode, topic) VALUES (?, ?, ?, ?)",
                [userId, human?.id || null, mode, topic]
            );
            session = { id: created.insertId };
        }
        await pool.query(
            'INSERT INTO tutor_messages (session_id, sender, content, strategy, emotion_signal) VALUES (?, "user", ?, "student_reasoning", "unknown")',
            [session.id, message]
        );
