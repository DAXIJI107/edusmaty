...
async function ensureTeacherTables() {
    ...
    await pool.query("ALTER TABLE teacher_path_assignments MODIFY status ENUM('in_progress','completed','locked','scheduled') DEFAULT 'in_progress'").catch(() => {});
    await pool.query("ALTER TABLE teacher_path_assignments ADD UNIQUE KEY uniq_path_student (path_id, student_id)").catch(() => {});
...
router.use(async (req, res, next) => {
    try { await ensureTeacherTables(); } catch (e) { /* ignore */ }
    next();
});
...
router.get('/overview', async (req, res) => {
    try {
        ...
        const [examCount] = await pool.query(
            "SELECT COUNT(*) as total FROM teacher_exams WHERE teacher_id = ?", [req.user.id]
        );
        const [noteCount] = await pool.query(
            "SELECT COUNT(*) as total FROM teacher_notes WHERE teacher_id = ?", [req.user.id]
        );
        const [assignCount] = await pool.query(
            "SELECT COUNT(*) as total FROM teacher_assignments WHERE teacher_id = ?", [req.user.id]
        );
...
router.get('/subjects', async (req, res) => {
    try {
        ...
        const defaultSubjects = ['数据结构与算法', '计算机网络', '操作系统', '数据库', '程序设计', '前端开发', '人工智能', '软件工程'];
...
router.post('/assign-subject', async (req, res) => {
    try {
        ...
        const [[student]] = await pool.query(
            "SELECT id, username, nickname FROM users WHERE id = ? AND role = 'student'", [student_id]
        );
...
router.get('/assignments', async (req, res) => {
    try {
        ...
        if (subject) { where += ' AND ta.subject = ?'; params.push(subject); }
        const [rows] = await pool.query(
            `SELECT ta.id, ta.subject, ta.note, ta.created_at,
             ...
             ${where}
             ORDER BY ta.created_at DESC LIMIT ${parseInt(limit)} OFFSET ${offset}`, params
        );
        const [[countResult]] = await pool.query(
            `SELECT COUNT(*) as total FROM teacher_assignments ta ${where}`, params
        );
...
router.delete('/assignments/:id', async (req, res) => {
    try {
        await pool.query("DELETE FROM teacher_assignments WHERE id = ? AND teacher_id = ?", [req.params.id, req.user.id]);
        ...
        if (questions && Array.isArray(questions) && questions.length > 0) {
            for (let i = 0; i < questions.length; i++) {
                ...
                await pool.query(
                    ...
                    [examId, q.content, q.type || 'choice', JSON.stringify(q.options || []), q.correct_answer || '', q.score || 5, i + 1]
                );
            }
        }
...
router.get('/exams/:id', async (req, res) => {
    try {
        const [[exam]] = await pool.query(
            "SELECT * FROM teacher_exams WHERE id = ? AND teacher_id = ?", [req.params.id, req.user.id]
        );
        if (!exam) return res.status(404).json({ success: false, message: '试卷未找到' });
        const [questions] = await pool.query(
            "SELECT * FROM teacher_exam_questions WHERE exam_id = ? ORDER BY sort_order", [req.params.id]
        );
        const [assignments] = await pool.query(
            `SELECT tea.*, u.username, u.nickname
             FROM teacher_exam_assignments tea
             JOIN users u ON tea.student_id = u.id
             WHERE tea.exam_id = ? ORDER BY tea.created_at DESC`, [req.params.id]
        );
...
router.post('/exams/:id/publish', async (req, res) => {
    try {
        ...
        const [[exam]] = await pool.query(
            "SELECT * FROM teacher_exams WHERE id = ? AND teacher_id = ?", [examId, req.user.id]
        );
        if (!exam) return res.status(404).json({ success: false, message: '试卷未找到' });
        if (student_ids && Array.isArray(student_ids) && student_ids.length > 0) {
            for (const sid of student_ids) {
                const [existing] = await pool.query(
                    "SELECT id FROM teacher_exam_assignments WHERE exam_id = ? AND student_id = ?", [examId, sid]
                );
                if (!existing.length) {
                    await pool.query(
                        "INSERT INTO teacher_exam_assignments (exam_id, student_id) VALUES (?, ?)", [examId, sid]
                    );
                }
            }
        }
...
router.get('/resources/search', async (req, res) => {
    try {
        ...
        if (!type || type === 'resource') {
            ...
            rows.forEach(r => { r._kind = 'resource'; results.push(r); });
        }
        if (!type || type === 'exam') {
            const [rows] = await pool.query(
            ...
            rows.forEach(r => { r._kind = 'exam'; results.push(r); });
        }
        if (!type || type === 'question') {
            ...
            rows.forEach(r => { r._kind = 'question'; results.push(r); });
        }
        if (!type || type === 'knowledge') {
            ...
            rows.forEach(r => { r._kind = 'knowledge'; results.push(r); });
        }
        if (!type || type === 'note') {
            const [rows] = await pool.query(
            ...
            rows.forEach(r => { r._kind = 'note'; results.push(r); });
        }
...
router.get('/notes', async (req, res) => {
    try {
        ...
        const params = [req.user.id];
        if (subject && subject !== 'all') { where += ' AND tn.subject = ?'; params.push(subject); }
        const [notes] = await pool.query(
            `SELECT tn.*,
                    (SELECT COUNT(*) FROM teacher_note_reads WHERE note_id = tn.id) as read_count
             FROM teacher_notes tn ${where}
             ORDER BY tn.created_at DESC LIMIT ${parseInt(limit)} OFFSET ${offset}`, params
        );
        const [[countResult]] = await pool.query(
            `SELECT COUNT(*) as total FROM teacher_notes tn ${where}`, params
        );
...
router.get('/notes/:id', async (req, res) => {
    try {
        const [[note]] = await pool.query(
            "SELECT * FROM teacher_notes WHERE id = ? AND teacher_id = ?", [req.params.id, req.user.id]
        );
        ...
        const [readers] = await pool.query(
            `SELECT tnr.read_at, u.id, u.username, u.nickname
             ...
             WHERE tnr.note_id = ? ORDER BY tnr.read_at DESC`, [req.params.id]
        );
    ...
    try {
        ...
        const [[user]] = await pool.query(
            `SELECT id, username, nickname, email, study_hours, completed_courses, 
             ...
             FROM users WHERE id = ? AND role = 'student'`, [id]
        );
        ...
        const [exams] = await pool.query(
            `SELECT ua.id, ua.is_correct, ua.answered_at as created_at, q.question as exam_name, kp.subject
             ...
             LEFT JOIN questions q ON ua.question_id = q.id
             LEFT JOIN knowledge_points kp ON q.knowledge_id = kp.id
             WHERE ua.user_id = ? ORDER BY ua.answered_at DESC LIMIT 20`, [id]
        );
        ...
        const [errors] = await pool.query(
            `SELECT kp.subject, COUNT(*) as count FROM user_answers ua
             JOIN questions q ON ua.question_id = q.id
             JOIN knowledge_points kp ON q.knowledge_id = kp.id
             WHERE ua.user_id = ? AND ua.is_correct = 0 GROUP BY kp.subject`, [id]
        );

        const [assignedSubjects] = await pool.query(
            "SELECT subject, note, created_at FROM teacher_assignments WHERE student_id = ? ORDER BY created_at DESC", [id]
        );
        ...
        const [teacherExams] = await pool.query(
            `SELECT tea.status, tea.score, tea.submit_time, te.name, te.subject
             ...
             WHERE tea.student_id = ? ORDER BY tea.created_at DESC`, [id]
        );
        ...
        const [readNotes] = await pool.query(
            `SELECT tnr.read_at, tn.title, tn.subject
             ...
             WHERE tnr.student_id = ? ORDER BY tnr.read_at DESC`, [id]
        );
    ...
    try {
        ...
        const [exams] = await pool.query(
            `SELECT ua.id, ua.is_correct as score, ua.answered_at as created_at, ua.answered_at as submit_time, q.question as exam_name, kp.subject
             ...
             LEFT JOIN questions q ON ua.question_id = q.id
             LEFT JOIN knowledge_points kp ON q.knowledge_id = kp.id
             WHERE ua.user_id = ? ORDER BY ua.answered_at DESC`, [id]
        );
    ...
    try {
        ...
        const [dailyStats] = await pool.query(
            `SELECT DATE(answered_at) as date, COUNT(*) as count, AVG(is_correct = 1) * 100 as avg_score
             ...
             GROUP BY DATE(answered_at) ORDER BY date DESC LIMIT 30`, [id]
        );
...
router.post('/students/batch-import', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: `成功导入 ${imported} 名学生，跳过 ${skipped} 名（重复或无用户名）`, imported, skipped });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});
        ...
        if (steps && Array.isArray(steps) && steps.length > 0) {
            for (let i = 0; i < steps.length; i++) {
                ...
                await pool.query(
                    ...
                    [pathId, s.title, s.type || 'text', s.content || '', s.options_json || JSON.stringify(s.options || []), s.correct_answer || '', s.duration_minutes || 10, i + 1, s.resource_id || null, s.resource_type || null]
                );
            }
        }
...
router.get('/paths', async (req, res) => {
    try {
        ...
        const [[countResult]] = await pool.query(
            "SELECT COUNT(*) as total FROM teacher_learning_paths WHERE teacher_id = ?", [req.user.id]
        );
...
router.get('/paths/:id', async (req, res) => {
    try {
        const [[path]] = await pool.query(
            "SELECT * FROM teacher_learning_paths WHERE id = ? AND teacher_id = ?", [req.params.id, req.user.id]
        );
        if (!path) return res.status(404).json({ success: false, message: '学习路径未找到' });
        const [steps] = await pool.query(
            "SELECT * FROM teacher_path_steps WHERE path_id = ? ORDER BY sort_order", [req.params.id]
        );
        const [assignments] = await pool.query(
            `SELECT tpa.*, u.username, u.nickname
             FROM teacher_path_assignments tpa
             JOIN users u ON tpa.student_id = u.id
             WHERE tpa.path_id = ? ORDER BY tpa.started_at DESC`, [req.params.id]
        );
...
router.put('/paths/:id', async (req, res) => {
    try {
        ...
        await pool.query(
            ...
            [req.params.id, current.version, current.name, current.description, current.subject, stepRows?.steps || '[]']
        );
        ...
        if (steps && Array.isArray(steps)) {
            ...
            for (let i = 0; i < steps.length; i++) {
                ...
                await pool.query(
                    "INSERT INTO teacher_path_steps (path_id, title, type, content, options_json, correct_answer, duration_minutes, sort_order, resource_id, resource_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    [req.params.id, s.title, s.type || 'text', s.content || '', s.options_json || JSON.stringify(s.options || []), s.correct_answer || '', s.duration_minutes || 10, i + 1, s.resource_id || null, s.resource_type || null]
                );
            }
        }
...
router.put('/paths/:id/steps/reorder', async (req, res) => {
    try {
        ...
        for (const step of steps) {
            await pool.query(
                "UPDATE teacher_path_steps SET sort_order = ? WHERE id = ? AND path_id = ?",
                [step.sort_order, step.id, req.params.id]
            );
        }
...
router.delete('/paths/:id', async (req, res) => {
    try {
        ...
        await pool.query("DELETE FROM teacher_path_step_progress WHERE assignment_id IN (SELECT id FROM teacher_path_assignments WHERE path_id = ?)", [req.params.id]);
        await pool.query("DELETE FROM teacher_path_assignments WHERE path_id = ?", [req.params.id]);
        await pool.query("DELETE FROM teacher_path_steps WHERE path_id = ?", [req.params.id]);
        await pool.query("DELETE FROM teacher_learning_paths WHERE id = ? AND teacher_id = ?", [req.params.id, req.user.id]);
...
router.post('/paths/:id/assign', async (req, res) => {
    try {
        ...
        const [[path]] = await pool.query(
            "SELECT * FROM teacher_learning_paths WHERE id = ? AND teacher_id = ?", [pathId, req.user.id]
        );
        if (!path) return res.status(404).json({ success: false, message: '学习路径未找到' });
        const [[stepCount]] = await pool.query(
            "SELECT COUNT(*) as total FROM teacher_path_steps WHERE path_id = ?", [pathId]
        );
        let assigned = 0;
        if (student_ids && Array.isArray(student_ids)) {
            for (const sid of student_ids) {
                ...
                if (!student) continue;
                const [existing] = await pool.query(
                    "SELECT id FROM teacher_path_assignments WHERE path_id = ? AND student_id = ?", [pathId, sid]
                );
                if (existing.length) {
                    ...
                    const [firstStep] = await pool.query(
                        "SELECT id FROM teacher_path_steps WHERE path_id = ? ORDER BY sort_order LIMIT 1", [pathId]
                    );
                    if (firstStep.length) {
                        await pool.query(
                ...
                assigned++;
                createNotification(sid, '新的学习路径', `老师已将学习路径「${path.name}」分配给你，共 ${stepCount.total} 个步骤，请开始学习。`, 'path_assigned', null, '/home');
            }
        }
        await pool.query("UPDATE teacher_learning_paths SET is_published = 1 WHERE id = ?", [pathId]);
        res.json({ success: true, message: `学习路径「${path.name}」已发布给 ${assigned} 名学生，学生将被锁定直到完成全部步骤`, assignedCount: assigned });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});
...
router.delete('/paths/:pathId/assignments/:assignmentId', async (req, res) => {
    try {
        ...
        createNotification(assignment.student_id, '学习路径已调整', `老师已将你从学习路径「${assignment.name}」中移除，学习功能已恢复。`, 'path_unlocked', req.params.pathId, '/home');
...
router.put('/paths/:id/schedule', async (req, res) => {
    try {
        ...
        const [[pathInfo]] = await pool.query(
            "SELECT name FROM teacher_learning_paths WHERE id = ?", [req.params.id]
        );
        ...
        for (const sid of student_ids) {
            ...
            if (!student) continue;
            const [[path]] = await pool.query("SELECT total_steps FROM (SELECT COUNT(*) as total_steps FROM teacher_path_steps WHERE path_id = ?) t", [req.params.id]);
            await pool.query(
            ...
            assigned++;
            createNotification(sid, '学习路径定时发布', `学习路径「${pathName}」已安排于 ${scheduled_at} 自动发布，届时会在相关学习页面提示下一步。`, 'scheduled', null, '/home');
        }
...
cron.schedule('* * * * *', async () => {
    try {
        ...
        for (const a of dueAssignments) {
            const [[stepCount]] = await pool.query("SELECT COUNT(*) as total FROM teacher_path_steps WHERE path_id = ?", [a.path_id]);
            await pool.query(
            ...
            const [firstStep] = await pool.query("SELECT id FROM teacher_path_steps WHERE path_id = ? ORDER BY sort_order LIMIT 1", [a.path_id]);
            if (firstStep.length) {
                await pool.query("INSERT INTO teacher_path_step_progress (assignment_id, step_id, status) VALUES (?, ?, 'in_progress')", [a.id, firstStep[0].id]);
            }
            const [[pathInfo]] = await pool.query("SELECT name FROM teacher_learning_paths WHERE id = ?", [a.path_id]);
            const pathName = pathInfo?.name || '学习路径';
            createNotification(a.student_id, '学习路径已发布', `学习路径「${pathName}」已自动发布，共 ${stepCount.total} 个步骤，请按页面顶部提示完成下一步。`, 'path_unlocked', null, '/home');
        }
    } catch (e) { /* cron silent fail */ }
});
...
router.get('/knowledge-graph', async (req, res) => {
    try {
        ...
        knowledgeQuery += " GROUP BY COALESCE(kp.title, q.question), kp.subject HAVING exam_count > 0 ORDER BY avg_score ASC LIMIT 30";
