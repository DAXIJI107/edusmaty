...
router.get("/", async (req, res) => {
    try {
        ...
        let sql =
            "SELECT id, content, type, options, answer, difficulty, score, node_id, subject FROM questions WHERE (is_active = 1 OR is_active IS NULL)";
        const params = [];
        ...
        if (rows.length === 0) {
            const fallbackSql =
                "SELECT id, content, type, options, answer, difficulty, score, node_id, subject FROM questions WHERE (is_active = 1 OR is_active IS NULL) ORDER BY RAND() LIMIT ?";
