...
router.post("/:id/submit", authenticateJWT, async (req, res) => {
    ...
    try {
        ...
        if (questionIds.length) {
            ...
            const [rows] = await pool.query("SELECT id, type, answer, score FROM questions WHERE exam_id = ?", [
                record.exam_id
            ]);
            questions = rows;
        }
        const [answers] = await pool.query("SELECT question_id, answer FROM user_answers WHERE user_exam_id=?", [
            user_exam_id
        ]);
        ...
        await pool.query("UPDATE exam_records SET submit_time=NOW(), status=?, score=? WHERE id=?", [
            "submitted",
            total,
            user_exam_id
        ]);
