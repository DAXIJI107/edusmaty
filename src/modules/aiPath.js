...
async function buildTodayCard(step) {
    const [lecture, questions] = await Promise.all([loadLectureDoc(step), loadQuestionSet(step.subject, step.id)]);
...
router.delete("/learning-list/:id", authenticateJWT, async (req, res) => {
    try {
        const userId = req.user.id;
        await pool.query(`DELETE FROM learning_list WHERE id = ? AND user_id = ?`, [req.params.id, userId]);
...
router.put("/learning-list/:id/done", authenticateJWT, async (req, res) => {
    try {
        const userId = req.user.id;
        await pool.query(`UPDATE learning_list SET status = 'done' WHERE id = ? AND user_id = ?`, [
            req.params.id,
            userId
        ]);
