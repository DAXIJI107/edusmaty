...
router.post("/guidance", authenticateJWT, async (req, res) => {
    try {
        ...
        // 可选：记录本次提问作为互动日志
        await pool.query("INSERT INTO interaction_logs (user_id, session_id, type, content) VALUES (?, ?, ?, ?)", [
            userId,
...
router.post("/assess", authenticateJWT, async (req, res) => {
    try {
        ...
        // 存储评分
        await pool.query("INSERT INTO metacognitive_scores (user_id, session_id, score, details) VALUES (?, ?, ?, ?)", [
            userId,
            sessionId,
            score,
            JSON.stringify({ logsSummary: logs.length })
        ]);
