(function () {
    ...
    const icons = {
        ...
        palette: '<circle cx="13.5" cy="6.5" r="1.5"/><circle cx="17.5" cy="10.5" r="1.5"/><circle cx="8.5" cy="7.5" r="1.5"/><circle cx="6.5" cy="12.5" r="1.5"/><path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.926 0 1.648-.746 1.648-1.688 0-.437-.18-.835-.437-1.125-.29-.289-.438-.652-.438-1.125a1.64 1.64 0 0 1 1.668-1.668h1.996c3.051 0 5.555-2.503 5.555-5.554C22 6.5 17.5 2 12 2z"/>',
        ...
        "file-text": '<path d="M14 3H6v18h12V7z"/><path d="M14 3v4h4"/><path d="M8 13h8"/><path d="M8 17h5"/><path d="M8 9h2"/>',
    ...
    async function request(path, options = {}) {
        ...
        const authExpired = (response.status === 401 && json.message === "未授权") || (response.status === 403 && json.message === "无效的token");
    ...
    async function markNotificationRead(id) {
        try {
            ...
            if (n && !n.is_read) { n.is_read = 1; state.data.unreadCount = Math.max(0, state.data.unreadCount - 1); }
        } catch (e) { /* silent */ }
    }
    ...
    async function submitDiagnostic(answers, freeText) {
        const json = await request("/api/diagnostic/submit", { method: "POST", body: JSON.stringify({ answers, freeText }) });
        state.data.diagnosticResult = json.data;
    ...
    async function loadObsidianKB(force = false) {
        ...
        } catch (e) { state.data.obsidianKB = null; }
    ...
    async function loadObsidianGraph(force = false) {
        ...
        } catch (e) { state.data.obsidianGraph = null; }
    ...
    async function loadRagOverview(force = false) {
        ...
        } catch (e) { state.data.ragOverview = null; }
    ...
    async function sendRagQuery() {
        ...
        try {
            ...
            state.data.ragMessages.push({ role: "bot", content: "检索失败: " + (e.message || "服务器错误"), citations: [] });
        }
    ...
    async function addRagToLearning(knowledgePoint, queryContext) {
        try {
            ...
            toast("已加入学习列表");
        } catch (e) { toast("添加失败: " + (e.message || "未知错误")); }
    }
    ...
    async function loadTeacherStudents(params = {}) {
        try {
            ...
            return json;
        } catch (e) { state.data.teacherStudents = []; return { students: [], total: 0 }; }
    }
    ...
    async function loadTeacherSubjects() {
        ...
        } catch (e) { state.data.teacherSubjects = []; return []; }
    }
    ...
    async function loadTeacherAssignments(params = {}) {
        try {
            ...
            return json;
        } catch (e) { state.data.teacherAssignments = []; return { assignments: [], total: 0 }; }
    }
    ...
    async function loadTeacherExamsList(params = {}) {
        try {
            ...
            return json;
        } catch (e) { state.data.teacherExamsList = []; return { exams: [], total: 0 }; }
    }
    ...
    async function loadTeacherExamDetail(id) {
        try {
            ...
            return json;
        } catch (e) { state.data.teacherExamDetail = null; return null; }
    }
    ...
    async function loadTeacherNotes(params = {}) {
        try {
            ...
            return json;
        } catch (e) { state.data.teacherNotesList = []; return { notes: [], total: 0 }; }
    }
    ...
    async function loadTeacherStudentDetail(id) {
        try {
            ...
            return json;
        } catch (e) { state.data.teacherStudentDetail = null; return null; }
    }
    ...
    async function loadTeacherPaths() {
        try {
            ...
            return json;
        } catch (e) { state.data.teacherPaths = []; return { paths: [], total: 0 }; }
    }
    ...
    async function loadTeacherPathDetail(id) {
        try {
            ...
            return json;
        } catch (e) { state.data.teacherPathDetail = null; return null; }
    }
    ...
    async function loadTeacherPathProgress(id) {
        try {
            ...
            return json;
        } catch (e) { state.data.teacherPathProgress = []; return { progress: [] }; }
    }
    ...
    async function loadTeacherResources(q = "", type = "") {
        try {
            const json = await request(`/api/teacher/resources/search?q=${encodeURIComponent(q)}&type=${encodeURIComponent(type)}&limit=50`);
    ...
    async function loadPathNotes(assignmentId) {
        try {
            ...
            const noteMap = {};
            state.data._pathAllNotes.forEach(n => { noteMap[n.step_id] = n.notes || ''; });
    ...
    function renderMarkdownLite(value) {
        ...
        lines.forEach(line => {
            ...
            if (/^\d+\.\s+/.test(text)) {
                closeList();
                html.push(`<p><b>${escapeHtml(text.replace(/^(\d+\.).*/, "$1"))}</b> ${escapeHtml(text.replace(/^\d+\.\s+/, ""))}</p>`);
                return;
            }
            closeList();
    ...
    function showPanel(title, body, afterOpen) {
        ...
        node.querySelectorAll("[data-panel-go]").forEach(el => el.addEventListener("click", () => {
        ...
        node._keyHandler = e => {
            if (e.key === "Escape") { node.classList.remove("show"); document.removeEventListener("keydown", node._keyHandler); }
        };
    ...
    function setView(view) {
        state.view = view;
        const route = view === "codeLab" ? "/code-lab" : view === "teamCode" ? "/team-code" : view === "agentResearch" ? "/agent-research" : view === "home" ? "/home" : `/${view}`;
    ...
    function aiFeatureDropdown() {
        return `<div class="ai-feature-menu">
            ...
            <div class="ai-feature-panel" role="menu">
                ...
                ${aiFeatures().map(([key, ic, title, desc, tag]) => `
    ...
    function closedLoopPanel(compact = false) {
        ...
        return `<section class="card loop-panel ${compact ? "compact" : ""}">
            ...
            <div class="loop-rail">${steps.map(([ic, title, desc], i) => `
...
`}
    ...
    function navGroups() {
        ...
        const groups = [
            { label: "首页", icon: "home", desc: "", items: [{ view: "home", icon: "home", label: "首页", desc: "回到总览" }] },
            { label: "学习中心", icon: "book", desc: "课程 · 练习 · 考试", items: [
            ...
            { label: "学习资源", icon: "layers", desc: "资源 · 教程 · 题库 · 视频", items: [
            ...
            { label: "知识引擎", icon: "database", desc: "Obsidian · RAG · Agent", items: [
    ...
    function metricCards(items = state.data.metrics) {
        return `<div class="metric-row">${items.map(([ic, label, value, unit, sub, up, color, glow, grad]) => `
    ...
    function planCard() {
        ...
        const tasks = state.data.tasks.length ? state.data.tasks : [
            { icon: "play", title: "完成今日推荐课程学习", meta: "数据结构 · 哈希表", time: "预计 45 分钟", done: true, color: "#635bff", soft: "rgba(99,91,255,.12)" },
            { icon: "file", title: "完成 5 道课后练习题", meta: "操作系统 · 内存管理", time: "预计 20 分钟", done: true, color: "#18b87a", soft: "rgba(24,184,122,.12)" },
            { icon: "robot", title: "与 AI 助手进行专题复盘", meta: "计算机网络 · DNS", time: "预计 15 分钟", done: false, color: "#ff9500", soft: "rgba(255,149,0,.12)" },
            { icon: "book", title: "查看本周学习报告", meta: "学习分析与建议", time: "预计 10 分钟", done: false, color: "#2f6bff", soft: "rgba(47,107,255,.12)" }
        ];
        return `
            <article class="card">
                <div class="card-head">
                ...
                <div class="plan-list">${tasks.map((task, i) => `
    ...
    function activityCard() {
        const items = state.data.activities.length ? state.data.activities : [
            { icon: "check", text: "完成了“数据结构”的学习", time: "10 分钟前", color: "#18b87a", soft: "rgba(24,184,122,.12)", badge: "学习完成" },
            { icon: "trophy", text: "连续学习第 13 天达成", time: "30 分钟前", color: "#ff9500", soft: "rgba(255,149,0,.12)", badge: "连续记录" }
        ];
        return `<article class="card"><div class="card-head"><h2 class="section-title">${icon("clock", 18)}最近动态</h2><button class="link-button" data-open-panel="activities">全部动态</button></div>
            <div class="activity">${items.map(item => `<div class="activity-item" style="--color:${item.color};--soft-color:${item.soft}">
    ...
    function quickTiles() {
        ...
        return `<div class="quick-row">${quick.map(([ic, title, desc, color, soft, view]) => `
    ...
    function achievementCard() {
        const titles = state.data.achievements.length ? state.data.achievements.map(item => item.title) : ["勤奋学习者", "知识点掌握", "学习里程碑", "连续学习"];
            ...
            <div class="grid-2" style="gap:12px;margin-top:16px;grid-template-columns:1fr 1fr">
                <div class="card" style="box-shadow:none;padding:14px;background:rgba(47,107,255,.05)">
                    ...
                    <div class="list" style="margin-top:10px">
                        ${materials.slice(0, 3).map(item => `<div class="list-row" style="align-items:flex-start;gap:10px">
                        ...
                        </div>`).join("") || `<p style="color:var(--muted)">暂无材料，点击任务按钮进入学习。</p>`}
                    </div>
                </div>
                <div class="card" style="box-shadow:none;padding:14px;background:rgba(24,184,122,.05)">
                    <b>${icon("pen",15)} 练习题</b>
                    <div class="list" style="margin-top:10px">
                        ${practiceQuestions.slice(0, 3).map((q, index) => `<div class="list-row" style="display:block">
                        ...
                        </div>`).join("") || `<p style="color:var(--muted)">完成学习材料后自动推荐练习题。</p>`}
                    </div>
                </div>
            </div>
            <div class="grid-3" style="gap:12px;margin-top:16px">
                ${steps.slice(0, 3).map((step, index) => `<div class="card" style="box-shadow:none;padding:14px;background:rgba(99,91,255,.05)"><b>${index + 1}. ${escapeHtml(step)}</b></div>`).join("")}
            </div>
            <div class="grid-2" style="gap:12px;margin-top:14px">
                <div class="card" style="box-shadow:none;padding:14px">
                    <b>${icon("edit",15)} 笔记模板</b>
                    <div class="tag-row" style="margin-top:10px">${noteSections.slice(0, 4).map(s => `<span class="pill">${escapeHtml(s.heading)}</span>`).join("") || `<span class="pill">核心概念</span><span class="pill">错因</span><span class="pill">下一步</span>`}</div>
    ...
    function studyPlanWeekView() {
        ...
        return `<section class="card">
            ...
            <div class="grid-3" style="grid-template-columns:repeat(7,minmax(90px,1fr));gap:10px">
                ${days.map(day => `<div class="card ${day.is_today ? "interactive" : ""}" style="box-shadow:none;text-align:center;padding:14px;background:${day.is_today ? "rgba(99,91,255,.08)" : "rgba(248,250,252,.9)"}">
    ...
    function studyPlanMonthView() {
        ...
        return `<section class="card">
            ...
            <div class="list">
                ${goals.map(goal => `<div class="list-row" style="display:block">
    ...
    function studentPathGuidedCard() {
        ...
        const stepMap = {};
        if (stepProgress) stepProgress.forEach(sp => { stepMap[sp.step_id] = sp; });
        ...
        return `<section class="guided-path-card">
            ...
            <div class="gpc-body">
                <div class="gpc-step-indicators">${allSteps.map((s, i) => {
                ...
                ${step ? `<div class="gpc-current-step">
    ...
    function studentPathView() {
        ...
        return `<main class="page path-lock-page" style="${Object.entries(lockBg).map(([k,v]) => `${k.replace(/([A-Z])/g,'-$1').toLowerCase()}:${v}`).join(';')}">
            ...
            <section class="path-lock-grid">
                <aside class="path-lock-sidebar">
                    <div class="card path-lock-steps-card">
                        ...
                        <div class="path-sidebar-steps">${allSteps.map((s, i) => {
                            const sp = stepMap[s.id];
                            const isCurrent = (i + 1) === p.currentStep;
                            const isDone = sp && sp.status === 'completed';
                            let cls = 'pss-item';
                    ...
                    ${step ? `
                        ...
                        <div class="plc-notes-section">
                            ...
                            <div class="plc-notes-list">
                                ${state.data._pathAllNotes.map(n => `
        ...
        return `<main class="page">
            <section class="path-dashboard">
                ...
                <div class="pd-section">
                    ...
                    ${dashboard.active.length === 0 ? `<div class="pd-empty">暂无进行中的学习路径</div>` :
                ...
                <div class="pd-section">
                    ...
                    ${dashboard.completed.length === 0 ? `<div class="pd-empty">暂无已完成的路径</div>` :
                    `<div class="pd-card-grid">${dashboard.completed.map(a => {
                ...
                <div class="pd-section">
                    ...
                    ${dashboard.upcoming.length === 0 ? `<div class="pd-empty">暂无即将开始的路径</div>` :
                    dashboard.upcoming.map(u => `<div class="pd-card pd-card-upcoming">
                        <div class="pd-card-header">
    ...
    function renderPathCalendar(dashboard) {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const markers = events.map(e => `<span class="pd-cal-dot ${e.type}" title="${escapeHtml(e.name)}"></span>`).join('');
    ...
    function studentPathCompactPanel(context = "home") {
        ...
        const activeCards = active.slice(0, 2).map(a => {
        ...
        const recentDone = completed.slice(0, 2).map(a => `<div class="path-mini-row">
            <span>${icon("check",14)}${escapeHtml(a.name)}</span><small>${escapeHtml(a.subject || "综合")} · 已完成</small>
        </div>`).join("");
        const upcomingRows = upcoming.slice(0, 2).map(a => `<div class="path-mini-row">
    ...
    function teacherPathInlineGuide(context = state.view) {
        ...
        const matched = context === targetView || (targetView === "practice" && ["practice", "test", "onlineExam"].includes(context));
        const typeLabels = { text: "阅读", video: "视频", quiz: "测验", code: "编程", exercise: "练习" };
        const answerBox = matched && step.type === "quiz" ? `<div class="tpg-answer-row">${parsePathOptions(step.options_json).map((opt, index) => `<label><input type="radio" name="path-quiz" value="${escapeHtml(opt)}"> ${escapeHtml(opt)}</label>`).join("")}</div>`
            : matched && step.type === "code" ? `<textarea class="input tpg-answer-box" data-path-code-answer placeholder="在当前页面完成代码后，把关键代码或说明粘贴到这里提交。"></textarea>`
            : matched && step.type === "exercise" ? `<textarea class="input tpg-answer-box" data-path-exercise-answer placeholder="把本页练习结果或答案写在这里提交。"></textarea>`
            : "";
    ...
    function majorTracks() {
        return [
            { key: "backend", icon: "db", title: "服务逻辑方向", fit: 92, stack: "Java / Node.js / MySQL / Redis", goal: "学会把项目需求拆成清晰的数据处理和业务规则", subjects: ["数据结构与算法", "数据库系统", "计算机网络", "操作系统"] },
            { key: "ai", icon: "brain", title: "AI算法", fit: 86, stack: "Python / 机器学习 / 向量检索 / RAG", goal: "构建机器学习与智能应用能力", subjects: ["概率统计", "线性代数", "机器学习", "算法设计"] },
            { key: "frontend", icon: "layers", title: "前端体验", fit: 81, stack: "HTML / CSS / JavaScript / 可视化", goal: "做出高质量交互体验与学习产品界面", subjects: ["Web工程", "数据可视化", "交互设计", "软件工程"] },
            { key: "security", icon: "lock", title: "网络安全", fit: 74, stack: "网络协议 / Linux / 安全攻防 / 密码学", goal: "建立安全工程和攻防分析基础", subjects: ["计算机网络", "操作系统", "密码学", "安全工程"] }
        ];
    }
            ...
            return `<section class="profile-intake">
                <article class="card profile-diagnostic-summary">
                    ...
                    <div class="profile-diagnostic-body">
                        ...
                        <div class="edge-cloud">${strengths.slice(0,4).map(s => `<span>${escapeHtml(s)} <b>优势方向</b></span>`).join("")}</div>
    ...
    function diagnosticQuestionnaireView() {
        ...
        if (!questionnaire) return `<section class="diagnostic-loading"><div class="loading-spinner"></div><p>加载诊断问卷中...</p></section>`;
        ...
        const questionHtml = (currentStep.questions || []).map(q => {
            const currentAnswer = answers[q.id];
            let inputHtml = "";
            switch (q.type) {
                case "single_choice":
                    inputHtml = `<div class="diagnostic-options">${(q.options || []).map(opt => `
                    ...
                    break;
                case "multi_choice":
                    inputHtml = `<div class="diagnostic-options multi">${(q.options || []).map(opt => {
    ...
    function diagnosticCatTab() {
        ...
        return `<section class="diagnostic-cat-tab">
            ...
            <article class="card cat-question-card">
                ...
                ${question.options ? `<div class="options-list">
                    ${question.options.map((opt, i) => `<label class="option-item" data-option="${i}">
    ...
    function diagnosticVarkTab() {
        ...
        return `<section class="diagnostic-vark-tab">
            ...
            <article class="card vark-card">
                ...
                ${(questionnaire.sections?.[activeSection]?.questions || []).map((q, qIdx) => `<div class="vark-question">
            ...
            <div class="report-grid">
                ...
                <article class="card misconception-card">
                    ...
                    <div class="misconception-list">
                        ${(misconceptions.cards || [{category: "数组操作", count: 3, suggestion: "建议多做相关练习"}, {category: "递归", count: 2, suggestion: "从简单例子开始理解"}]).slice(0, 5).map(mc => `<div class="misconception-item ${mc.severity || 'moderate'}">
                            <div class="mc-header">
    ...
    function diagnosticVarkView() {
        ...
        return `<main class="page diagnostic-page vark-page">
            ...
            ${questionnaire.sections.map((section, idx) => `<section class="vark-section" ${idx !== state.data.diagnosticStep ? 'hidden' : ''}>
                <article class="card vark-card">
                    <div class="card-head">
                        <h2>${icon("list",18)} ${section.title}</h2>
                    </div>
                    ${section.questions.map((q, qIdx) => `<div class="vark-question">
                        <p>${q.text}</p>
                        <div class="vark-options">
                            ${q.options.map((opt, optIdx) => `<button class="btn ghost vark-option" data-question="${idx}-${qIdx}" data-value="${optIdx + 1}">
                                ${opt}
                            </button>`).join("")}
                        </div>
                    </div>`).join("")}
            ...
            <section class="report-grid">
                ...
                <article class="card heatmap-card">
                    ...
                    <div class="heatmap-grid">
                        ${heatmap.nodes.slice(0, 20).map(node => `<div class="heatmap-cell" style="background-color:${node.color}" title="${escapeHtml(node.title)}: ${node.mastery}%">
                ...
                <article class="card misconception-card">
                    ...
                    <div class="misconception-list">
                        ${misconceptions.cards.slice(0, 5).map(mc => `<div class="misconception-item ${mc.severity}">
                            <div class="mc-header">
                                <span>${escapeHtml(mc.category)}</span>
                ...
                <article class="card path-card">
                    ...
                    <div class="path-suggestions">
                        ${pathSuggestion.suggestions.map((s, i) => `<div class="path-item">
                            <span class="path-index">${i + 1}</span>
                            <div class="path-content">
                                <h4>${escapeHtml(s.target)}</h4>
                                <p>${escapeHtml(s.action)}</p>
            ...
            <section class="cognitive-grid">
                <article class="card attrs-card">
                    ...
                    <div class="attrs-grid">
                        ${Object.entries(attrs).map(([name, data]) => `<div class="attr-item">
                ...
                <article class="card blooms-card">
                    ...
                    <div class="blooms-chart">
                        ${Object.entries(blooms).map(([level, data]) => `<div class="bloom-bar-item">
            ...
            <section class="knowledge-list">
                <article class="card">
                    ...
                    <div class="knowledge-table">
                        ${nodes.slice(0, 20).map(node => `<div class="knowledge-row">
            ...
            <section class="misconception-list">
                <article class="card">
                    ...
                    <div class="misconception-cards">
                        ${misconceptions.map(mc => `<div class="misconception-card ${mc.severity}">
                            <div class="mc-header">
                            ...
                            <div class="mc-suggestion">${mc.suggestion}</div>
                            ${mc.sampleQuestions && mc.sampleQuestions.length > 0 ? `<div class="mc-samples">
                                ${mc.sampleQuestions.slice(0, 2).map(q => `<div class="mc-sample">
    ...
    function radarChartComponent(radar) {
        ...
        const dataPointsStr = data.map((value, i) => {
            const angle = angleStep * i - Math.PI / 2;
    ...
    function diagnosticSubjectTestView() {
        ...
        const questionsHtml = test.questions.map((q, idx) => {
            ...
            switch (q.type) {
                case "single":
                    inputHtml = `<div class="subject-options">${(q.options || []).map((opt, oi) => {
                    ...
                    break;
                case "judge":
                    inputHtml = `<div class="subject-options">${["正确", "错误"].map(opt => {
                    ...
                    break;
                case "multiple":
                    inputHtml = `<div class="subject-options multi">${(q.options || []).map((opt, oi) => {
                        const optLabel = String.fromCharCode(65 + oi);
    ...
    function diagnosticSubjectResultView(result) {
        ...
        const accuracyColor = accuracy >= 85 ? "var(--green)" : accuracy >= 70 ? "var(--blue)" : accuracy >= 50 ? "var(--amber)" : "var(--red)";

        const weakTagsHtml = weakNodes.map(n => {
            const ml = masteryLevel(n.accuracy);
            return `<span class="tag warn">${escapeHtml(n.nodeName)} <small>${n.accuracy}%</small></span>`;
        }).join("");

        const strongTagsHtml = strongNodes.map(n => {
            const ml = masteryLevel(n.accuracy);
            return `<span class="tag good">${escapeHtml(n.nodeName)} <small>${n.accuracy}%</small></span>`;
        }).join("");

        const nodeBarsHtml = nodeResults.map(n => {
            const color = n.accuracy >= 85 ? "var(--green)" : n.accuracy >= 70 ? "var(--blue)" : n.accuracy >= 50 ? "var(--amber)" : "var(--red)";
        ...
        return `<main class="page diagnostic-page subject-result-page">
            ...
            ${radarChartHtml ? `<section class="diagnostic-radar-section">
                <article class="card radar-card">
            ...
            ${analysis.rootCause ? `<section class="diagnostic-insight-section">
                <article class="card insight-card">
            ...
            ${recsHtml ? `<section class="diagnostic-recs-section">
    ...
    function diagnosticRadarChart(radarData) {
        ...
        if (count < 3) return `<p class="muted" style="text-align:center;padding:40px">至少需要3个知识点才能显示雷达图</p>`;
        ...
        for (let l = 1; l <= levels; l++) {
            ...
            for (let i = 0; i < count; i++) {
                points.push(`${cx + r * Math.cos(angleSlice * i - Math.PI / 2)},${cy + r * Math.sin(angleSlice * i - Math.PI / 2)}`);
            }
            svgParts.push(`<polygon points="${points.join(" ")}" fill="none" stroke="rgba(148,163,184,.2)" stroke-width="1"/>`);
        }

        for (let i = 0; i < count; i++) {
            const x = cx + radius * Math.cos(angleSlice * i - Math.PI / 2);
            const y = cy + radius * Math.sin(angleSlice * i - Math.PI / 2);
            svgParts.push(`<line x1="${cx}" y1="${cy}" x2="${x}" y2="${y}" stroke="rgba(148,163,184,.15)" stroke-width="1"/>`);
        }
        ...
        svgParts.push(`<polygon points="${dataPoints.join(" ")}" fill="rgba(22,179,148,.15)" stroke="var(--teal)" stroke-width="2"/>`);
        ...
        radarData.forEach((d, i) => {
            ...
            svgParts.push(`<text x="${lx}" y="${ly}" text-anchor="${anchor}" font-size="12" fill="var(--text)" dominant-baseline="middle">${escapeHtml(d.name)} <tspan fill="var(--text-muted)">${d.value}%</tspan></text>`);
        });
    ...
    function reportResourcePackageView(result) {
        ...
        return `<article class="card report-result-card"><div class="card-head"><h2 class="section-title">${icon("layers",18)}个性化学习资源包</h2><span class="pill good">${resources.length} 类资源</span></div>
            ...
            <div class="resource-package-grid">${resources.map((resource, index) => `<div class="resource-pack-item">
    ...
    function resourceDetailHtml(resource) {
        ...
        if (resource.type === "ppt") {
            ...
            return `${header}<div class="ppt-outline">${slides.map((slide, i) => `<article>
        ...
        if (resource.type === "quiz") {
            ...
            return `${header}<div class="quiz-preview">${questions.map((q, i) => `<article>
        ...
        if (resource.type === "practice") {
            ...
            return `${header}<div class="practice-case-list">${cases.map(item => `<article>
    ...
    function questionOptionsHtml(question) {
        ...
        if (typeof options === "string") {
            try { options = JSON.parse(options); } catch { options = options.split(/[|,，]/).map(item => item.trim()).filter(Boolean); }
        }
    ...
    function buildAlgoSteps(kind, rawValues) {
        ...
        if (kind === "queue") {
            ...
            steps[0] = makeAlgoStep(queue, [], [], "队列遵循先进先出：左侧是队头 Front，右侧是队尾 Rear。", { front: 0, rear: queue.length - 1, op: "ready" });
            if (queue.length) {
                steps.push(makeAlgoStep(queue, [0], [], `查看队头元素 ${queue[0]}，它会最先出队。`, { front: 0, rear: queue.length - 1, op: "peek" }));
                const removed = queue.shift();
                steps.push(makeAlgoStep(queue, [], [], `${removed} 出队，Front 指针移动到下一个元素。`, { front: queue.length ? 0 : -1, rear: queue.length - 1, op: "dequeue", removed }));
            }
            const nextValue = Math.max(...initial, 0) + 1;
            steps.push(makeAlgoStep([...queue, nextValue], [queue.length], [], `${nextValue} 从队尾入队，Rear 指针指向新元素。`, { front: 0, rear: queue.length, op: "enqueue", added: nextValue }));
            steps.push(makeAlgoStep([...queue, nextValue], [], [...Array(queue.length + 1).keys()], "队列演示完成：入队从 Rear 进入，出队从 Front 离开。", { front: 0, rear: queue.length, op: "done" }));
            return steps;
        }
        if (kind === "linked") {
            const list = initial.slice(0, 6);
            steps[0] = makeAlgoStep(list, [], [], "链表由节点和 next 指针组成，访问时需要沿箭头一个个走。", { current: 0, prev: null, next: 1, op: "ready" });
            for (let i = 0; i < list.length; i += 1) {
                steps.push(makeAlgoStep(list, [i], Array.from({ length: i }, (_, n) => n), `访问节点 ${i}，读取值 ${list[i]}。`, { prev: i > 0 ? i - 1 : null, current: i, next: i + 1 < list.length ? i + 1 : null, op: "traverse" }));
            }
            const reversed = [];
            for (let i = 0; i < list.length; i += 1) {
                reversed.unshift(list[i]);
                steps.push(makeAlgoStep([...reversed, ...list.slice(i + 1)], [0], Array.from({ length: reversed.length }, (_, n) => n), `反转指针：节点 ${i} 的 next 指向前一个节点。`, { prev: reversed.length > 1 ? 1 : null, current: 0, next: reversed.length, op: "reverse" }));
            }
            steps.push(makeAlgoStep([...list].reverse(), [], list.map((_, i) => i), "链表反转完成：所有 next 指针方向已经改变。", { current: null, prev: 0, next: null, op: "done" }));
            return steps;
        }
        if (kind === "selection") {
            const arr = [...initial];
            for (let i = 0; i < arr.length - 1; i += 1) {
                let min = i;
                steps.push(makeAlgoStep(arr, [i], Array.from({ length: i }, (_, n) => n), `第 ${i + 1} 轮：先假设位置 ${i} 是最小值。`, { i, min }));
                for (let j = i + 1; j < arr.length; j += 1) {
                    steps.push(makeAlgoStep(arr, [min, j], Array.from({ length: i }, (_, n) => n), `比较当前最小值 ${arr[min]} 和候选值 ${arr[j]}。`, { i, j, min }));
                    if (arr[j] < arr[min]) {
                        min = j;
                        steps.push(makeAlgoStep(arr, [min], Array.from({ length: i }, (_, n) => n), `发现更小的 ${arr[min]}，更新最小值位置。`, { i, j, min }));
                    }
                }
                if (min !== i) {
                    [arr[i], arr[min]] = [arr[min], arr[i]];
                    steps.push(makeAlgoStep(arr, [i, min], Array.from({ length: i + 1 }, (_, n) => n), `交换位置 ${i} 和 ${min}，把本轮最小值放到前面。`, { i, min }));
                } else {
                    steps.push(makeAlgoStep(arr, [i], Array.from({ length: i + 1 }, (_, n) => n), `位置 ${i} 已经是本轮最小值，不需要交换。`, { i, min }));
                }
            }
            steps.push(makeAlgoStep(arr, [], arr.map((_, i) => i), "选择排序完成：每一轮都把最小值放到未排序区开头。"));
            return steps;
        }
        if (kind === "binary") {
            ...
            steps.push(makeAlgoStep(arr, [], [], `二分查找需要有序数组，这里查找目标值 ${target}。`, { left, right, target }));
            while (left <= right) {
                const mid = Math.floor((left + right) / 2);
                steps.push(makeAlgoStep(arr, [mid], [], `检查中点 ${mid}，值为 ${arr[mid]}。`, { left, right, mid, target }));
                if (arr[mid] === target) {
                    steps.push(makeAlgoStep(arr, [mid], [mid], `找到目标值 ${target}，位置是 ${mid}。`, { left, right, mid, target }));
                    return steps;
                }
                if (arr[mid] < target) {
                    left = mid + 1;
                    steps.push(makeAlgoStep(arr, [], [], `${arr[mid]} 小于 ${target}，丢弃左半区。`, { left, right, target }));
                } else {
                    right = mid - 1;
                    steps.push(makeAlgoStep(arr, [], [], `${arr[mid]} 大于 ${target}，丢弃右半区。`, { left, right, target }));
                }
            }
        ...
        for (let end = arr.length - 1; end > 0; end -= 1) {
            for (let i = 0; i < end; i += 1) {
                steps.push(makeAlgoStep(arr, [i, i + 1], Array.from({ length: arr.length - 1 - end }, (_, n) => end + 1 + n), `比较 ${arr[i]} 和 ${arr[i + 1]}。`, { i, j: i + 1 }));
                if (arr[i] > arr[i + 1]) {
                    [arr[i], arr[i + 1]] = [arr[i + 1], arr[i]];
                    steps.push(makeAlgoStep(arr, [i, i + 1], Array.from({ length: arr.length - end }, (_, n) => end + n), `左边更大，交换这两个元素。`, { i, j: i + 1 }));
                } else {
                    steps.push(makeAlgoStep(arr, [i, i + 1], Array.from({ length: arr.length - 1 - end }, (_, n) => end + 1 + n), `顺序正确，不交换。`, { i, j: i + 1 }));
                }
            }
            steps.push(makeAlgoStep(arr, [end], Array.from({ length: arr.length - end }, (_, n) => end + n), `本轮结束，最大值已经冒泡到位置 ${end}。`, { end }));
        }
        steps.push(makeAlgoStep(arr, [], arr.map((_, i) => i), "冒泡排序完成：相邻元素不断比较和交换。"));
        return steps;
    }

    function ensureAlgoVizSteps() {
        const viz = state.data.algoViz || { kind: "bubble", values: "8,3,5,1,9,2", steps: [], index: 0, playing: false };
    ...
    function renderAlgoScene(viz, step, max) {
        ...
        if (viz.kind === "linked") {
            return `<div class="algo-linked-scene">
                ${step.array.map((value, index) => {
                    ...
                    const pointer = step.pointers?.current === index ? "current" : step.pointers?.prev === index ? "prev" : step.pointers?.next === index ? "next" : "";
        ...
        if (viz.kind === "binary") {
            return `<div class="algo-array-scene search">
                ${step.array.map((value, index) => {
                    ...
                    const outside = step.pointers?.left !== undefined && (index < step.pointers.left || index > step.pointers.right) && !found;
        ...
        return `<div class="algo-array-scene ${isSwap ? "is-swapping" : ""}">
            ${step.array.map((value, index) => {
                const active = step.active.includes(index);
                const sorted = step.sorted.includes(index);
                const swapClass = isSwap && active
                    ? index === activePair[0] ? "swap-left" : index === activePair[1] ? "swap-right" : ""
                    : "";
    ...
    function renderAlgorithmVisualizer() {
        ...
        const pointerText = step.pointers?.target !== undefined
        ...
        return `<section class="algo-viz-card">
            ...
            <div class="algo-step-timeline">
                ${nearbySteps.map(item => `<button class="${item.index === (viz.index || 0) ? "active" : ""}" data-algo-jump="${item.index}">
    ...
    function reportAiLearningView(result) {
        ...
        return `<article class="card report-result-card"><div class="card-head"><h2 class="section-title">${icon("brain",18)}AI学习建议</h2><span class="pill">个性化</span></div>
            ...
            <div class="report-split">
                <div><h3>薄弱点诊断</h3><div class="list">${(test.focusNodes || []).slice(0,4).map(n => `<div class="list-row"><span>${escapeHtml(n.name)}<small>掌握度 ${n.mastery}%</small></span><span class="pill warn">优先</span></div>`).join("")}</div></div>
    ...
    function reportDemoLoopView(result) {
        ...
        return `<article class="card report-result-card demo-loop-card"><div class="card-head"><h2 class="section-title">${icon("bolt",18)}完整学习闭环</h2><span class="pill good">可跟踪</span></div>
            ...
            <div class="report-split">
                ...
                <div><h3>推荐路径</h3><div class="list">${(path.nodes || []).slice(0,4).map(n => `<div class="list-row"><span>${escapeHtml(n.name)}<small>${escapeHtml(n.subject || "")} · ${n.mastery}%</small></span><span class="pill">${escapeHtml(n.status || "pending")}</span></div>`).join("")}</div></div>
            </div>
            <div class="review-strip">${review.slice(0,4).map(r => `<span>${escapeHtml(r.nodeName)} · ${r.intervalDays}天后复习</span>`).join("")}</div>
        </article>`;
    }
    ...
    function modeMeta(mode) {
        const map = {
            practice: { title: "专项练习", desc: "按薄弱计算机知识点推题，答完立即反馈并回写掌握度。", label: "练习", button: "提交练习", count: "6题" },
            test: { title: "阶段测试", desc: "覆盖多个计算机知识模块，适合检查阶段性掌握情况。", label: "测试", button: "提交测试", count: "10题" },
            onlineExam: { title: "在线考试", desc: "模拟正式考试环境，限时完成、统一评分、生成闭环建议。", label: "考试", button: "交卷评分", count: "20题" }
        };
    ...
    function assessmentFocusView(mode, meta, set, questions, answers) {
        ...
        const remainPercent = durationSeconds ? Math.max(0, Math.min(100, Math.round((remaining / durationSeconds) * 100))) : 0;
    ...
    function assessmentResultView(mode, meta, set, answers, result) {
        ...
        return `<main class="page assessment-result-page">
            ...
            <section class="result-grid">
                ...
                <article class="card"><h2 class="section-title">${icon("chart",18)}能力雷达</h2><div class="ability-bars">${["记忆","理解","应用","分析","迁移"].map((name, i) => { const v = Math.max(22, Math.min(96, (result.score || 0) - i * 6 + (i === 0 ? 10 : 0))); return `<div><span>${name}</span><b>${v}%</b><div class="bar"><span style="width:${v}%"></span></div></div>`; }).join("")}</div></article>
    ...
    function aiReviewText(result, mode) {
        if ((result.score || 0) >= 80) return `${mode === "onlineExam" ? "综合考试" : "分科测试"}结果显示你已经具备较稳定的计算机知识框架。下一步建议做跨知识点迁移题，并把仍然犹豫的题整理成主动回忆卡。`;
        if ((result.score || 0) >= 60) return "你有一定基础，但错题暴露出概念边界和应用条件还不够稳。建议先整理错题笔记，再做一轮同学科补救练习。";
    ...
    function subjectTestSelector() {
        const subjects = state.data.subjects.length ? state.data.subjects : [{ subject: state.data.selectedSubject, questionCount: 0, avgMastery: 0 }];
        return `<section class="subject-strip">${subjects.map(item => `
    ...
    function questionSetItem(mode, q, index, answers, result) {
        ...
        return `<div class="paper-question" id="q_${q.id}">
            ...
            <div class="choice-grid">${(q.options || []).map((option, i) => {
                const selected = answers[q.id] === option;
                const checked = detail ? (detail.correctAnswer === option ? "ok" : selected ? "bad" : "") : selected ? "selected" : "";
        ...
        return `<main class="page notes-page"><section class="hero-row"><div class="hero"><h1>笔记复习</h1><p>课程、练习、考试和个人记录沉淀为可编辑笔记，并同步到 Obsidian Markdown，形成可双链、可复习的长期知识库。</p><div class="hero-actions"><button class="btn primary glow" data-new-note>${icon("pen",17)}新建笔记</button><button class="btn ghost" data-view="course">${icon("book",17)}从课程学习</button><button class="btn ghost" data-view="onlineExam">${icon("exam",17)}从考试复盘</button><button class="btn ghost" data-view="knowledgeGraph">${icon("radar",17)}Obsidian双链图谱</button><button class="btn ghost" data-view="conceptCanvas">${icon("layout",17)}概念画布</button></div></div>${metricCards()}</section>
            ...
            <section class="notes-workbench">
                <aside class="card note-sidebar">
                    ...
                    </button>`).join("") || `<div class="empty-state compact"><p>还没有笔记，先从课程学习或手动添加一条。</p></div>`}</div>
                </aside>
                ...
                <aside class="side-card">
                    ...
                    <article class="card"><div class="card-head"><h2 class="section-title">${icon("file",18)}卡片</h2><span class="pill">${cards.length}</span></div><div class="note-card-list">${cards.slice(0, 5).map(card => `<div class="smart-card"><span class="pill">${escapeHtml(card.card_type || "card")}</span><h3>${escapeHtml(card.title)}</h3><p>${escapeHtml(card.content?.activeRecall || card.content?.explanation || "")}</p><div class="bar"><span style="width:${card.mastery_signal || 50}%"></span></div></div>`).join("") || "<p>选择一条笔记生成复习卡。</p>"}</div></article>
        ...
        return `<main class="page account-page-v2">
            <section class="account-hero-v2">
                ...
                <div class="hero-info">
                    ...
                    <div class="tag-row">${interests.slice(0, 5).map(tag => `<span class="pill">${escapeHtml(tag)}</span>`).join('') || '<span class="pill">未设置兴趣</span>'}</div>
                </div>
            ...
            <nav class="account-tabs">
                ${['dashboard','courses','exams','notes','settings'].map(tab => {
                    const labels = { dashboard: '📊 学习概览', courses: '📚 课程信息', exams: '📝 考试记录', notes: '📓 学习笔记', settings: '⚙️ 账户设置' };
        ...
        return `<div class="dashboard-grid">
            ...
            <div class="dash-main">
                <article class="card dash-card-full">
                    ...
                    <div class="subject-bars">${subjects.slice(0, 6).map(s => `<div class="subject-bar-row"><span class="subject-label">${escapeHtml(s.subject)}</span><div class="bar-wrap"><div class="bar"><span style="width:${s.mastery || 0}%"></span></div></div><b>${s.mastery || 0}%</b></div>`).join('') || '<p>暂无学科数据</p>'}</div>
                </article>
                <article class="card dash-card-full">
                    <div class="card-head"><h2 class="section-title">${icon('bell', 18)}最近学习动态</h2><span class="pill">${activities.length || 0} 条</span></div>
                    <div class="activity compact">${activities.slice(0, 6).map(item => `<div class="activity-item" style="--color:${item.color};--soft-color:${item.soft_color}"><span class="round-icon">${icon(item.icon, 17)}</span><div><div class="activity-text">${escapeHtml(item.title)}</div><div class="activity-time">${escapeHtml(item.time_label)}</div></div><span class="pill">${escapeHtml(item.badge || '动态')}</span></div>`).join('') || '<p>暂无动态</p>'}</div>
                </article>
            </div>
            <div class="dash-side">
                <article class="card">
                ...
                <article class="card">
                    ...
                    <div class="weak-list">${weak.slice(0, 4).map(p => `<div class="weak-item"><b>${escapeHtml(p.title)}</b><span>${escapeHtml(p.subject)} · ${p.mastery || 0}%</span></div>`).join('') || '<p>暂无薄弱点</p>'}</div>
                </article>
                ${achievements.length ? `<article class="card">
    ...
    function accountCoursesTab(courses) {
        return `<div class="tab-panel">
            ...
            <div class="course-grid">${courses.length ? courses.map(c => `<article class="course-card">
            ...
            </article>`).join('') : '<p class="empty-hint">暂无课程，去首页看看吧 👆</p>'}</div>
        </div>`;
    }
    ...
    function accountExamsTab(examStats, recentAnswers, stats) {
        return `<div class="tab-panel">
            ...
            ${examStats.length ? `<article class="card"><div class="card-head"><h2 class="section-title">${icon('chart', 18)}7 天正确率趋势</h2></div>
    ...
    function accountNotesTab(notes, noteStats) {
        return `<div class="tab-panel">
            ...
            <div class="notes-grid">${notes.length ? notes.map(n => {
                ...
                return `<article class="note-card">
                    ...
                    <div class="note-tags">${tags.slice(0, 3).map(t => `<span class="pill tiny">${escapeHtml(t)}</span>`).join('')}</div>
                    <small>更新于 ${formatDate(n.updated_at) || '最近'}</small>
                </article>`;
            }).join('') : '<p class="empty-hint">还没有笔记，去智能笔记页面开始写吧 ✍️</p>'}</div>
    ...
    function accountSettingsTab(user, state) {
        ...
        return `<div class="tab-panel settings-panel account-settings-redesign">
            <aside class="settings-profile-panel">
                ...
                <div class="settings-tag-cloud">${interests.slice(0, 8).map(tag => `<b>${escapeHtml(tag)}</b>`).join('') || '<b>未设置兴趣标签</b>'}</div>
            </aside>
    ...
    function teacherWorkbenchView() {
        ...
        const subjects = state.data.teacherSubjects.length ? state.data.teacherSubjects : ["数据结构与算法","计算机网络","操作系统","数据库","程序设计","前端开发","人工智能","软件工程"];
        ...
        if (tab === "overview") {
            ...
            <section class="teacher-layout">
                ...
                <article class="card"><div class="card-head"><h2 class="section-title">${icon("clock",18)}最近练测动态</h2></div><div class="list">${(dashboard.recentExams || []).slice(0, 8).map(item => `<div class="list-row"><span>${escapeHtml(item.nickname || item.username)}<small>${escapeHtml(item.subject)} · ${escapeHtml(item.exam_name)}</small></span><span class="pill ${item.is_correct ? 'good' : 'warn'}">${item.is_correct ? '正确' : '错误'}</span></div>`).join("") || "<p>暂无练测记录</p>"}</div></article>
            </section>`;
            ...
            <section class="teacher-student-grid">
                ${students.length === 0 ? '<div class="empty-state"><p>暂无学生数据</p></div>' : students.map(s => `
                    <div class="teacher-student-card">
                        ...
                        <div class="tsc-subjects">${s.assigned_subjects ? s.assigned_subjects.split(", ").map(sub => `<span class="pill">${escapeHtml(sub)}</span>`).join(" ") : '<span class="pill ghost">未分配学科</span>'}</div>
            ...
            <section class="teacher-assign-list">
                ${assignments.length === 0 ? '<div class="empty-state"><p>暂无分配记录</p></div>' : assignments.map(a => `
            ...
            if (examDetail) {
                ...
                <section class="teacher-exam-list">
                    ${examsList.length === 0 ? '<div class="empty-state"><p>暂无试卷，点击"创建试卷"开始</p></div>' : examsList.map(e => `
            ...
            <section class="teacher-notes-grid">
                ${notesList.length === 0 ? '<div class="empty-state"><p>暂无笔记，点击"发布笔记"开始</p></div>' : notesList.map(n => `
            ...
            if (studentDetail) {
                ...
                tabContent = `<section class="student-progress-section">
                    ...
                    <div class="progress-detail-grid">
                        <div class="card"><div class="card-head"><h3>${icon("brain",16)} 知识点掌握</h3></div><div class="knowledge-bar-list">${knowledge.slice(0, 10).map(k => `<div class="kb-row"><span>${escapeHtml(k.name)}</span><div class="bar"><span style="width:${k.mastery}%"></span></div><b>${k.mastery}%</b></div>`).join("") || "<p>暂无知识点数据</p>"}</div></div>
                        <div class="card"><div class="card-head"><h3>${icon("file",16)} 考试记录</h3></div><div class="list">${exams.slice(0, 8).map(e => `<div class="list-row"><span>${escapeHtml(e.exam_name || "考试")}<small>${escapeHtml(e.subject)}</small></span><span class="pill ${e.is_correct ? 'good' : 'warn'}">${e.is_correct ? '正确' : '错误'}</span></div>`).join("") || "<p>暂无考试记录</p>"}</div></div>
                        ...
                        <div class="card"><div class="card-head"><h3>${icon("book",16)} 阅读笔记</h3></div><div class="list">${readNotes.slice(0, 8).map(r => `<div class="list-row"><span>${escapeHtml(r.title)}<small>${escapeHtml(r.subject)}</small></span><small>${new Date(r.read_at).toLocaleDateString()}</small></div>`).join("") || "<p>未阅读任何笔记</p>"}</div></div>
                    </div>
                </section>`;
            } else {
                tabContent = `<section class="teacher-toolbar">
                ...
                <section class="teacher-student-grid">${students.length === 0 ? '<div class="empty-state"><p>暂无学生数据</p></div>' : students.map(s => `
                    <div class="teacher-student-card">
                        <div class="tsc-head">
                            <span class="tsc-avatar">${escapeHtml((s.nickname || s.username || "?").charAt(0).toUpperCase())}</span>
                            <div><b>${escapeHtml(s.nickname || s.username)}</b><small>@${escapeHtml(s.username)}</small></div>
            ...
            if (pathDetail) {
                ...
                const activeAssignments = assignStudents.filter(a => a.status === "in_progress" || a.status === "scheduled");
                const completedAssignments = assignStudents.filter(a => a.status === "completed");
                const avgProgress = assignStudents.length
                    ? Math.round(assignStudents.reduce((sum, a) => sum + ((Number(a.completed_steps) || 0) / Math.max(Number(a.total_steps) || steps.length || 1, 1) * 100), 0) / assignStudents.length)
                    : 0;
                tabContent = `<section class="teacher-path-detail">
                    ...
                    <div class="teacher-path-steps">
                        ...
                        <div class="path-steps-timeline">${steps.map((s, i) => `
                    ...
                    <div class="list">${assignStudents.length === 0 ? '<p>尚未分配给任何学生</p>' : assignStudents.map(a => `
                ...
                <section class="teacher-path-grid">
                    ${pathsList.length === 0 ? '<div class="empty-state"><p>暂未创建学习路径。点击"创建学习路径"设计一套完整的学习流程，分配给指定学生后将锁定学生只能按路径学习。</p></div>' : pathsList.map(p => `
                ...
                tabContent = `<section class="scan-upload-area">
                    ...
                    <div class="card scan-history-card">
                        ...
                        <div class="scan-history-list">
                            ${scanHistory.length === 0 ? '<p class="text-muted">暂无扫描记录</p>' : scanHistory.map(h => `
    ...
    function parseInterestTags(value) {
        ...
        try {
            ...
            return String(value || "计算机科学,数据结构,人工智能").split(",").map(item => item.trim()).filter(Boolean);
        }
    }

    function dailyQuestionCard() {
        const q = state.data.dailyQuestion || { id: "", question: "时间复杂度 O(n log n) 通常对应哪种算法？", correctAnswer: "归并排序", options: ["冒泡排序", "二分查找", "归并排序", "线性查找"], sourceName: "本地题库", knowledgeTitle: "算法复杂度" };
    ...
    function pathView() {
        ...
        return `<main class="page path-page">
            ...
            <section class="major-direction-band">
                ...
                <div class="major-track-grid">${majorTracks().map(track => `<button class="major-track ${state.data.majorTrack === track.key ? "active" : ""}" data-major-track="${track.key}">
    ...
    function assetView() {
        const tab = ["knowledge", "notes", "mistakes"].includes(state.data.assetTab) ? state.data.assetTab : "knowledge";
        const weak = state.data.weakPoints[0] || {};
        const ragResult = state.data.ragAskResult || {};
        const ragCitations = ragResult.citations || [];
        const tabBody = tab === "notes"
        ...
        return `<main class="page"><section class="hero-row"><div class="hero"><h1>学习资产中心</h1><p>把课程资料、个人笔记和错题复盘放在一起，方便持续复习和证据追溯。知识库问答已合并到 AI 助手。</p></div>${metricCards()}</section><div class="tabs"><button class="${tab === "knowledge" ? "active" : ""}" data-asset-tab="knowledge">${icon("db",16)}知识库</button><button class="${tab === "notes" ? "active" : ""}" data-asset-tab="notes">${icon("pen",16)}笔记</button><button class="${tab === "mistakes" ? "active" : ""}" data-asset-tab="mistakes">${icon("book",16)}错题本</button></div>
            ...
            ${tabBody}
            <aside class="side-card"><article class="card"><h2 class="section-title">${icon("file",18)}课程来源</h2><div class="list">${state.data.courses.slice(0,5).map(c => `<div class="list-row"><span>${escapeHtml(c.title)}</span><span class="pill">${escapeHtml(c.provider || "公开")}</span></div>`).join("")}</div><button class="btn tiny ghost" data-ai-assistant-mode="rag">${icon("robot",14)}去知识库问答</button></article></aside></section></main>`;
    }

    function courseView() {
        const courses = state.data.courses.length ? state.data.courses : [{ id: 0, title: "数据结构 · 哈希表", provider: "公开课程", subject: "数据结构与算法", progress: 75 }];
        return `<main class="page"><section class="hero-row"><div class="hero"><h1>课程学习</h1><p>围绕当前目标和薄弱知识点推荐课程，学生可以按进度学习，教师可以看到课程完成情况。</p></div>${metricCards()}</section><section class="grid-3">${courses.map((course,i)=>`<article class="quick-tile course-card interactive" style="min-height:250px;--soft-color:${["#eef3ff","#effaf5","#fff6e8","#f6f0ff","#eaf8ff","#fff0f5"][i % 6]};--grad:linear-gradient(135deg,#2f6bff,#7c4dff)">
    ...
    function intelligenceView() {
        ...
        const flowInput = escapeHtml(state.data._agentFlowInput || `我总是分不清 ${judgment.focus || "当前薄弱点"} 的关键条件`);
        ...
        return `<main class="page intelligence-page">
            ...
            <section class="agent-console-grid">
                ...
                <article class="card agent-section-card agent-chat-card">
                    ...
                    <div class="chat-mini">${messages.slice(-5).map(m => `<div class="chat-bubble ${m.sender === "ai" ? "ai" : "user"}"><b>${m.sender === "ai" ? "导师" : "我"}</b><span>${escapeHtml(m.content)}</span></div>`).join("") || `<div class="chat-bubble ai"><b>${escapeHtml(conversation.tutor || "小星")}</b><span>${escapeHtml(conversation.lastReply || "把最卡的一步发给我，我会先追问，再生成练习和复盘卡。")}</span></div>`}</div>
                ...
                <article class="card agent-section-card">
                    ...
                    <div class="collaborate-steps">
                        ${state.data._collaborateSteps.map((step, i) => `
                ...
                <article class="card agent-section-card">
                    ...
                    <div class="teacher-action-strip">${recommendedActions.map(action => canTeach
            ...
            <section class="card xfyun-console-card">
                ...
                <div class="xfyun-cap-grid">${xfyunCapabilities.map(item => `<article class="xfyun-cap-card ${item.ready ? "ready" : "pending"}">
                ...
                </article>`).join("") || `<div class="empty-state">点击刷新状态读取讯飞能力配置。</div>`}</div>
...
print("需要复习", weak)`
            ...
            <section class="ide-topbar">
                ...
                <div class="topbar-right">
                    ...
                    <select class="topbar-select" data-code-language>
                        ${codeLanguages().map(([key, short, label]) => `<option value="${key}" ${language === key ? "selected" : ""}>${label}</option>`).join("")}
                    </select>
                </div>
            </section>
    ...
    function repoExplorerView(repo, files, activeFile) {
        ...
        <div class="explorer-files">
            ${files.length ? files.map(f => `<div class="explorer-file ${activeFile && activeFile.id === f.id ? "active" : ""}" data-code-open-file="${f.id}">
            ...
            </div>`).join("") : '<div class="explorer-empty">暂无文件，点击上方上传</div>'}
        </div>`;
    }
    ...
    function templateExplorerView(templates) {
        ...
        <div class="explorer-files">
            ${Object.entries(templates).map(([key, item]) => `<div class="explorer-file ${state.data.codeTemplate === key ? "active" : ""}" data-code-template="${key}">
    ...
    function fileIcon(filename) {
        ...
        const map = { js: "terminal", html: "layers", css: "palette", py: "code", java: "code", cpp: "code", c: "code", txt: "file", md: "file", json: "db", sql: "db" };
    ...
    function normalizeRepoPath(name, prefix = "") {
        const safe = String(name || "uploaded.txt").replace(/\\/g, "/").split("/").filter(Boolean).join("/");
    ...
    function inferLanguageFromPath(path) {
        ...
        const map = { js: "javascript", ts: "javascript", jsx: "javascript", tsx: "javascript", py: "python", html: "html", css: "css", md: "markdown", txt: "markdown", sql: "sql", json: "javascript" };
        return map[ext] || "markdown";
    }

    function repoBranchName(project) {
        return (project?.repository_name || project?.name || "team-repo").toLowerCase().replace(/[^\w\u4e00-\u9fa5-]+/g, "-").replace(/^-+|-+$/g, "") || "team-repo";
    }
    ...
    function repoReadmeFile(files) {
        return (files || []).find(file => /^readme\.md$/i.test(file.path))
    ...
    function repoFolderRows(files, currentPath = "") {
        ...
        (files || []).forEach(file => {
            ...
            if (parts.length > 1) {
                ...
                folder.count += 1;
                if (new Date(file.updated_at || 0) > new Date(folder.updated_at || 0)) folder.updated_at = file.updated_at;
                return;
            }
    ...
    function repoTreeHtml(files, currentPath = "", activeFile) {
        ...
        const renderNode = (node, depth = 0) => Object.keys(node).sort((a, b) => {
            ...
            const expanded = !currentPath || path === currentPath || currentPath.startsWith(path + "/") || depth < 1;
    ...
    function repoBreadcrumbHtml(project, currentPath) {
        ...
        const crumbs = [`<button data-team-repo-folder="">${escapeHtml(project.repository_name || "team-repo")}</button>`];
        parts.forEach((part, index) => {
...
5. 点击 Code 查看拉取和上传说明。`
    ...
    function extractProjectIntake(docs, existing = {}) {
        ...
        const stackMatchers = [
            ["Vue", /vue/i], ["React", /react/i], ["HTML/CSS", /html|css|页面|样式/i],
            ["Node.js", /node|express|后端服务/i], ["Java", /spring|java/i], ["Python", /python|flask|django/i],
            ["MySQL", /mysql|sql|数据库/i], ["Redis", /redis/i], ["Docker", /docker|容器/i],
            ["GitHub Actions", /github actions|ci|流水线/i], ["Playwright", /playwright|自动化测试/i]
        ];
        const picked = (words, limit) => sentences.filter(line => words.some(word => line.includes(word))).slice(0, limit);
        ...
        return {
            ...
            summary: sentences[0]?.slice(0, 160) || existing.summary || "已上传项目说明书，系统会继续提取项目目标、功能范围和岗位任务。",
            features: features.length ? features.map(item => item.slice(0, 70)) : existing.features || ["项目说明书已上传，等待补充明确功能需求。"],
            techStack: techStack.length ? techStack : existing.techStack || ["JavaScript", "Node.js", "MySQL"],
            notes: notes.length ? notes.map(item => item.slice(0, 80)) : existing.notes || ["注意把说明书中的未确认点转成待办卡片。"],
            roleTodos
        };
    }
    ...
    function teamRoleWorkspace(roleKey) {
        const workspaces = {
            frontend: {
                title: "前端开发工作台",
                handbook: "负责把项目需求转化为可使用、可演示、可联调的页面体验，需要关注信息架构、组件拆分、状态流转、接口对接和移动端适配。",
                outputs: ["页面原型与交互流程", "组件目录与样式规范", "接口联调记录", "用户体验验收截图"],
                tools: ["VS Code / WebStorm", "Chrome DevTools", "Figma 标注", "接口 Mock", "Playwright 页面冒烟"],
                todos: [
                    { title: "搭建项目首页与研发看板", priority: "P0", detail: "展示项目说明、成员分工、需求卡片和最新提交。" },
                    { title: "实现需求卡片状态视图", priority: "P0", detail: "按 todo / doing / review / done 呈现，并支持模块筛选。" },
                    { title: "完成接口联调与空状态", priority: "P1", detail: "对接后端任务接口，处理加载、错误、无数据状态。" }
                ]
            },
            backend: {
                title: "后端开发工作台",
                handbook: "负责把业务规则、数据模型、权限边界和接口契约落成稳定服务，需要关注参数校验、异常处理、数据一致性和审计记录。",
                outputs: ["接口契约文档", "数据表设计", "权限与审计规则", "接口自测结果"],
                tools: ["Postman / Apifox", "MySQL Workbench", "Node.js 调试器", "接口日志", "Swagger / OpenAPI"],
                todos: [
                    { title: "设计任务与提交记录接口", priority: "P0", detail: "保存文件时记录操作者、模块、位置、版本和时间。" },
                    { title: "补充项目成员权限校验", priority: "P0", detail: "限制成员只能访问所属项目，并记录越权风险。" },
                    { title: "实现需求状态流转 API", priority: "P1", detail: "支持需求认领、评审、完成和回退。" }
                ]
            },
            testing: {
                title: "测试验收工作台",
                handbook: "负责把需求变成可执行验收标准，覆盖主流程、异常路径、权限边界和回归风险，让团队知道什么才算真正完成。",
                outputs: ["测试用例清单", "缺陷记录", "回归报告", "验收结论"],
                tools: ["Playwright", "Jest / Vitest", "缺陷看板", "测试数据构造器", "截图与日志采集"],
                todos: [
                    { title: "编写团队项目核心流程用例", priority: "P0", detail: "覆盖创建项目、打开文件、保存同步和 AI 审查。" },
                    { title: "补充权限与异常测试", priority: "P0", detail: "验证未登录、无权限、空文件路径、空内容等场景。" },
                    { title: "形成验收报告模板", priority: "P1", detail: "输出通过项、阻塞项、风险项和复测建议。" }
                ]
            },
            deployment: {
                title: "实施部署工作台",
                handbook: "负责把团队成果变成可访问、可演示、可回滚的交付物，需要关注环境变量、启动命令、健康检查、部署记录和演示脚本。",
                outputs: ["部署说明书", "环境变量清单", "健康检查结果", "演示与回滚方案"],
                tools: ["GitHub Actions", "Docker / PM2", "云服务器控制台", "日志监控", "部署截图"],
                todos: [
                    { title: "整理本地与线上启动说明", priority: "P0", detail: "写清 npm install、npm start、数据库和 JWT 配置。" },
                    { title: "配置部署前检查清单", priority: "P0", detail: "包含语法检查、接口健康、页面访问和登录验证。" },
                    { title: "准备项目演示脚本", priority: "P1", detail: "按角色展示需求、代码、日志、审查和部署结果。" }
                ]
            }
        };
    ...
    function teamCodeView() {
        ...
        const sharedProblems = [
            { id: 1, author: "成员A", avatar: "A", title: "前端跨域问题", desc: "API请求时遇到CORS错误，需要后端配置允许跨域", tags: ["前端", "已解决"], time: "2小时前", replies: 3 },
            { id: 2, author: "成员B", avatar: "B", title: "数据库连接池耗尽", desc: "高并发时数据库连接不够用，需要优化连接池配置", tags: ["后端", "进行中"], time: "昨天", replies: 5 },
            { id: 3, author: "成员C", avatar: "C", title: "测试环境与生产不一致", desc: "本地测试通过但部署后报错，环境变量差异导致", tags: ["测试", "已解决"], time: "3天前", replies: 2 }
        ];
        const sharedPrompts = [
            { id: 1, title: "代码审查提示词", desc: "请审查以下代码，重点关注：安全性、性能、可维护性", author: "成员B", usage: 12 },
            { id: 2, title: "生成单元测试", desc: "为以下函数生成Jest单元测试，覆盖正常/异常/边界情况", author: "成员C", usage: 8 },
            { id: 3, title: "API文档生成", desc: "根据以下代码生成OpenAPI 3.0规范的接口文档", author: "成员A", usage: 5 }
        ];
        const sharedApis = [
            ...
            { id: 4, method: "GET", path: "/api/files/:path", desc: "获取代码文件内容", module: "backend", status: "todo" }
        ];
        const sharedSkills = agents && agents.length ? agents : [
            { key: "code-review", name: "AI 代码审查", desc: "自动审查代码质量、安全漏洞和性能问题", category: "质量", status: "ready" },
            { key: "test-gen", name: "测试用例生成", desc: "根据代码逻辑自动生成单元测试和集成测试", category: "测试", status: "ready" },
            { key: "doc-gen", name: "文档自动生成", desc: "从代码注释和接口定义生成 API 文档", category: "文档", status: "draft" },
            { key: "deploy-check", name: "部署前检查", desc: "检查环境配置、依赖版本和安全策略", category: "运维", status: "ready" }
        ];
        ...
        return `<main class="page team-v3-page">
            ...
            <section class="team-v3-hero">
                <div class="team-v3-hero-left">
                    ...
                    <div class="team-v3-hero-tags">${(intake.techStack || []).slice(0, 6).map(item => `<span>${escapeHtml(item)}</span>`).join("")}</div>
            ...
            <nav class="team-v3-tabs">
                ...
                ${roleKeys.map(key => `<button class="${screen === "role" && activeRole === key ? "active" : ""}" data-team-role="${key}">
                ...
                <aside class="team-v3-sidebar">
                    <div class="team-v3-sidebar-section">
                        ...
                        <div class="team-v3-project-list">
                            ${(projects.length ? projects : [project]).map(item => `<div class="team-v3-project-row ${Number(item.id) === Number(project?.id) ? "active" : ""}">
                    ...
                    <div class="team-v3-sidebar-section">
                        ...
                        ${members.length ? members.map(m => `<div class="team-v3-member-row">
                        ...
                        </div>`).join("") : `<div class="team-v3-empty-sm">暂无成员</div>`}
                    </div>
                    ...
                    <div class="team-v3-repo-sidebar-files">
                        ...
                        <div class="team-v3-file-table-sidebar">
                            ${repoCurrentPath ? `<button class="team-v3-file-row folder" data-team-repo-path="${escapeAttr(repoCurrentPath.split("/").slice(0, -1).join("/") || "")}" data-team-repo-folder="${escapeAttr(repoCurrentPath.split("/").slice(0, -1).join("/") || "")}">
                                <span>${icon("folder", 15)}..</span>
                            </button>` : ""}
                            ${repoRows.map(row => row.type === "folder" ? `<button class="team-v3-file-row folder" data-team-repo-path="${escapeAttr(row.path)}" data-team-repo-folder="${escapeAttr(row.path)}">
                                <span>${icon("folder", 15)}${escapeHtml(row.name)}</span>
                            </button>` : `<div class="team-v3-file-row file ${activeFile && Number(activeFile.id) === Number(row.id) ? "active" : ""}">
                ...
                <section class="team-v3-main">
                    ...
                    <div class="team-v3-code-wall">
                        ...
                        <div class="team-v3-code-wall-grid">
                            ...
                            <div class="team-v3-review-board">
                                ${reviewChecklist.map(item => `<div class="team-v3-review-check">
                    ...
                    <div class="team-v3-modules">
                        ...
                        <div class="team-v3-module-grid">
                            ${moduleStats.map(role => {
                                const percent = Math.min(100, 24 + Number(role.fileCount || 0) * 18 + Number(role.commitCount || 0) * 10);
                    ...
                    <div class="team-v3-requirements">
                        ...
                        <div class="team-v3-requirement-grid">
                            ${requirements.length ? requirements.map(item => `<article class="team-v3-req-card ${escapeHtml(item.status)}">
                            ...
                            </article>`).join("") : `<div class="team-v3-empty-sm">
                                ${icon("target", 32)}
                    ...
                    <div class="team-v3-docs">
                        ...
                        <div class="team-v3-docs-grid">
                            <article class="team-v3-doc-card">
                                ...
                                <div class="team-v3-doc-tags">${(intake.techStack || []).slice(0, 8).map(item => `<span>${escapeHtml(item)}</span>`).join("") || "<span>待补充技术栈</span>"}</div>
                            </article>
                            <article class="team-v3-doc-card">
                                <h3>${icon("users", 14)}人员分工</h3>
                                ${moduleStats.length ? moduleStats.map(role => {
                                    const member = members.find(item => item.role_key === role.roleKey);
                            ...
                            <article class="team-v3-doc-card">
                                ...
                                ${(intake.notes || []).length ? intake.notes.slice(0, 5).map(item => `<p>${escapeHtml(item)}</p>`).join("") : "<p>暂无注意事项</p>"}
                            </article>
                        </div>
                    </div>
                    ...
                    <div class="team-v3-role">
                        ...
                        <div class="team-v3-role-tasks">
                            ...
                            <div class="team-v3-todo-grid">
                                ${roleWorkspace.todos.map(item => `<article class="team-v3-todo-card">
                                    <span class="team-v3-todo-badge ${(item.priority || "").toLowerCase()}">${escapeHtml(item.priority)}</span>
                                    <b>${escapeHtml(item.title)}</b>
                                    <p>${escapeHtml(item.detail)}</p>
                                </article>`).join("")}
                                ${roleRequirements.map(item => `<article class="team-v3-todo-card requirement">
                                    <span class="team-v3-todo-badge ${(item.priority || "").toLowerCase()}">${escapeHtml(item.priority)}</span>
                                    <b>${escapeHtml(item.title)}</b>
                                    <p>${escapeHtml(item.acceptance)}</p>
                    ...
                    <div class="team-v3-tools">
                        ...
                        <div class="team-v3-skills-grid">
                            ${sharedSkills.map(skill => `<button class="team-v3-skill-card" data-team-tool="${escapeHtml(skill.key)}">
                            ...
                            ${tools.length ? tools.map(tool => `<button class="team-v3-skill-card" data-team-tool="${escapeHtml(tool.key)}">
                                <div class="team-v3-skill-top">
                        ...
                        <div class="team-v3-collab-main">
                            <div class="team-v3-collab-block">
                                ...
                                <div class="team-v3-problem-list">
                                    ${sharedProblems.map(p => `<article class="team-v3-problem-card">
                            ...
                            <div class="team-v3-collab-block">
                                ...
                                <div class="team-v3-prompt-grid">
                                    ${sharedPrompts.map(p => `<article class="team-v3-prompt-card">
                            ...
                            <div class="team-v3-collab-block">
                                ...
                                <div class="team-v3-api-table">
                                    ...
                                    ${sharedApis.map(api => `<div class="team-v3-api-row">
                    ...
                    <div class="team-v3-editor-panel" style="margin: 0 0 20px 0;">
                        ...
                        ${review ? `<div class="team-v3-review-result">
                ...
                <aside class="team-v3-activity">
                    <div class="team-v3-activity-section">
                        ...
                        <div class="team-v3-stage">
                            ${["需求分析", "设计开发", "测试验收", "部署上线"].map((label, i) => {
                    ...
                    <div class="team-v3-activity-section">
                        ...
                        <div class="team-v3-member-events">
                            ${members.slice(0, 5).length ? members.slice(0, 5).map(m => `<div class="team-v3-member-event">
                            ...
                            </div>`).join("") : `<div class="team-v3-empty-sm">暂无成员动态</div>`}
                        </div>
                    </div>
                    <div class="team-v3-sidebar-divider"></div>
                    <div class="team-v3-activity-section">
                        ...
                        <div class="team-v3-timeline">
                            ${commits.slice(0, 8).length ? commits.slice(0, 8).map(commit => `<div class="team-v3-timeline-item">
                            ...
                            </div>`).join("") : `<div class="team-v3-empty-sm">暂无提交记录</div>`}
                        </div>
                    </div>
                    <div class="team-v3-sidebar-divider"></div>
                    <div class="team-v3-activity-section">
                        ...
                        <div class="team-v3-event-feed">
                            ${events.slice(0, 6).length ? events.slice(0, 6).map(event => `<div class="team-v3-event-item">
                                <b>${escapeHtml(event.title)}</b>
                                <small>${escapeHtml(event.username || "系统")} · ${escapeHtml(event.detail || "")}</small>
                            </div>`).join("") : `<div class="team-v3-empty-sm">暂无动态</div>`}
                        </div>
                    </div>
                </aside>
            </div>
    ...
    function resourcesView() {
        ...
        const catList = categories.length ? categories : [
            { category: "编程入门", count: 4 }, { category: "算法与数据结构", count: 5 },
            { category: "页面交互学习", count: 4 }, { category: "服务逻辑学习", count: 3 },
            { category: "数据库", count: 3 }, { category: "人工智能", count: 4 },
            { category: "操作系统与网络", count: 4 }, { category: "视频教程", count: 5 }
        ];
        const diffColors = { beginner: "var(--green)", intermediate: "var(--blue)", advanced: "var(--orange)", all: "var(--muted)" };
        const typeIcons = { tutorial: "book", video: "play", practice: "code", reference: "file", tool: "settings", community: "folder" };
        ...
        return `<main class="page">
            ...
            <section class="resource-grid">
                ${resources.length === 0 ? `<div class="resource-empty"><span>${icon("search",48)}</span><h3>暂无资源数据</h3><p>请点击下方按钮初始化资源数据</p><button class="btn primary" data-seed-resources>${icon("refresh",16)} 初始化资源</button></div>` : resources.map(r => `
    ...
    function tutorialsView() {
        const quickLinks = [
            ...
            { name: "MDN Web Docs", url: "https://developer.mozilla.org/zh-CN", icon: "book", desc: "适合查阅页面交互知识点" },
            ...
            return `<main class="page problem-detail-page">
                <section class="problem-detail-layout">
                    <div class="problem-detail-left">
                        ...
                        <div class="problem-tags">${(detail.tags || "").split(",").filter(Boolean).map(t => `<span class="pill">${escapeHtml(t.trim())}</span>`).join("")}</div>
    ...
    function videosView() {
        const videos = [
            { name: "B站 编程区", url: "https://www.bilibili.com/v/popular/programming", icon: "play", desc: "B站编程开发热门视频", tag: "综合" },
            { name: "中国大学MOOC", url: "https://www.icourse163.org", icon: "play", desc: "名校计算机专业课程", tag: "课程" },
            { name: "慕课网", url: "https://www.imooc.com", icon: "play", desc: "程序员实战视频课", tag: "实战" },
            { name: "学堂在线", url: "https://www.xuetangx.com", icon: "play", desc: "清华出品计算机课程", tag: "课程" },
            { name: "YouTube CS", url: "https://www.youtube.com", icon: "play", desc: "CS50/FreeCodeCamp 编程频道", tag: "综合" },
            { name: "极客学院", url: "https://www.jikexueyuan.com", icon: "play", desc: "IT职业培训视频课程", tag: "职业" }
        ];
        ...
        const lastAnswer = [...state.data.aiAssistantMessages].reverse().find(message => message.role === "ai")?.content || "";
        const aiConfig = state.data.aiConfig || {};
	        ...
	        const modes = [
	            { key: "rag", icon: "db", label: "RAG知识库", desc: "本地模型按证据回答" },
	            ...baseModes
	        ].map(item => [item.key, item.icon, item.label, item.desc]);
        const activeMode = modes.find(([key]) => key === state.data.aiAssistantMode) || modes[0];
        const agentCards = [
            ["感知", "search", "支持截图搜题与题干识别", "上传题图后自动进入错题教练"],
            ["推理", "brain", "结合薄弱点、课程和笔记回答", weakPoints[0] ? `当前优先：${weakPoints[0].title}` : "暂无薄弱点数据"],
            ["行动", "pen", "回答可沉淀为笔记和复习卡", lastAnswer ? "最近回答可继续整理" : "等待第一次对话"],
            ["编排", "route", "把问答转成计划、练习和复盘", courses[0] ? `最近课程：${courses[0].title}` : "可生成学习路径"]
        ];
	        const assistantPromptMap = {
	            rag: ["软件测试中的边界值分析是什么？", "软件架构中的可测试性和可维护性应该怎么理解？", "RAG 回答为什么需要引用证据？"],
	        ...
	        const quickPrompts = { ...assistantPromptMap, ...(aiConfig.assistantPrompts || {}) }[state.data.aiAssistantMode] || [];
	        const messages = state.data.aiAssistantMessages.length ? state.data.aiAssistantMessages : [
            { role: "ai", content: "你好，我已经接入学习数据。你可以问课程问题、贴错题、整理笔记，或者让我生成复习计划。" }
        ];
        ...
        const taskFlowCards = [
            ["今日判断", "brain", todayJudgment.focus || weakPoints[0]?.title || "当前薄弱点", todayJudgment.reason || "结合答题、课程、笔记与考试数据生成下一步学习动作。"],
            ["任务流", "bolt", todayJudgment.nextTask || "生成闭环任务", executionRecords[0]?.detail || "一键把问答转为练习、笔记、复习和报告信号。"],
            ["多智能体", "users", agents.length ? `${agents.length} 个 Agent 协同` : "规划 / 教学 / 出题 / 评估", "学习助手、资源、画像、监督等 Agent 共同完成学习闭环。"]
        ];
        return `<main class="page ai-assistant-page">
            ...
            <section class="assistant-product-grid">
                <article class="card assistant-console">
	                ...
	                <aside class="side-card">
	                    ${state.data.aiAssistantMode === "rag" ? `<article class="card assistant-rag-card">
                        ...
                        <div class="agent-timeline compact">${executionRecords.slice(0, 4).map(record => `<div class="agent-timeline-item ${escapeHtml(record.status)}"><span></span><div><b>${escapeHtml(record.title)}</b><p>${escapeHtml(record.detail)}</p><small>${escapeHtml(record.agent)} · ${escapeHtml(record.type)}</small></div></div>`).join("") || `<div class="empty-state compact">点击“运行任务流”后，会在这里显示练习、笔记和复习安排。</div>`}</div>
                    </article>
                    <article class="card assistant-trace-card">
                        ...
                        <div class="agent-trace-list">${runtimeTraces.length ? runtimeTraces.map(step => `<div class="agent-trace-step ${escapeHtml(step.stepType || step.step_type || "")}">
                        ...
                        </div>`).join("") : `<div class="empty-state compact">输入学习目标后，这里会展示智能体读取画像、分析路径、调用工具和写回任务的过程。</div>`}</div>
                    </article>
                    ${runtimeResult.courseDesign?.design ? `<article class="card assistant-course-design">
                        ...
                        <div class="course-design-mini">${runtimeResult.courseDesign.design.units.slice(0, 4).map(unit => `<div><b>${escapeHtml(unit.title)}</b><p>${escapeHtml(unit.objective)}</p><small>${escapeHtml(unit.reason)}</small></div>`).join("")}</div>
                    </article>` : ""}
                    ${runtimeResult.rag?.citations?.length ? `<article class="card assistant-rag-card">
                        ...
                        <div class="list">${runtimeResult.rag.citations.slice(0, 4).map(ev => `<a class="list-row" href="${escapeAttr(ev.url || ev.source?.url || "#")}" target="_blank" rel="noreferrer"><span><b>${escapeHtml(ev.title)}</b><small>${escapeHtml(ev.source?.name || "")} · ${escapeHtml(ev.snippet || "")}</small></span><span class="pill">${escapeHtml(String(ev.rank))}</span></a>`).join("")}</div>
                    </article>` : ""}
    ...
    function agentResearchView() {
        ...
        const features = [
            ["智能体学习路线", "route", "基于 Hello-Agents 章节生成 4 周学习计划，每个节点绑定课程、练习、笔记模板和项目任务。", "path"],
            ["Obsidian 式智能笔记", "pen", "为笔记增加双向链接、标签、主动回忆卡、复习状态和知识图谱视角。", "smartNotes"],
            ["AgentScope 实训舱", "code", "在 AI 编程舱里提供 ReAct Agent、工具调用、RAG 检索、多 Agent 协作的可运行模板。", "codeLab"],
            ["个人知识库 Agent", "db", "读取平台笔记和学习数据，回答“我哪里不会、下一步学什么、如何复习”。", "aiAssistant"],
            ["多智能体项目工坊", "users", "用团队项目模块模拟产品经理、架构师、开发、测试、审查四类 Agent 协作。", "teamCode"],
            ["Agent 效果评估", "radar", "用正确率、任务完成率、工具调用成功率、笔记复用率评估智能体是否真的帮到学习。", "report"]
        ];
        const sprint = [
        ...
        return `<main class="page agent-research-page">
            ...
            <section class="agent-source-grid">
                ${integrations.map(item => `<article class="card agent-source-card">
                    ...
                    <div class="agent-module-chips">${(item.actions || item.landing || []).map(action => {
            ...
            <section class="card agent-roadmap-card">
                ...
                <div class="agent-roadmap-rail">${roadmap.map(item => `<button class="agent-roadmap-step" data-view="${escapeAttr(item.view)}">
            ...
            <section class="agent-feature-grid">
                ${features.map(([title, ic, desc, view]) => `<article class="card agent-feature-card">
...
- 教师可以看到学生学习 Agent 的进度、项目成果和能力变化。`)}</div>
    ...
    function conceptCanvasView() {
        ...
        <section class="canvas-list">
            ${canvases.length === 0 ? '<div class="empty-state card"><p>还没有画布。点击"新建画布"开始整理你的知识。</p></div>' : canvases.map(c => `
    ...
    function obsidianView() {
        ...
        const tags = Array.isArray(tagsArr) ? tagsArr.slice(0, 15) : Object.entries(tagsArr).map(([k,v]) => ({ tag: k, name: k, total: v })).slice(0, 15);

        // 统计 - stats uses "notes" not "total"
        const stats = kb ? (kb.stats || {}) : {};

        function noteKindIcon(kind) {
            if (kind === "题目" || kind === "题库") return `<div class="vault-note-icon exam">${icon("exam", 18)}</div>`;
            if (kind === "知识" || kind === "知识点") return `<div class="vault-note-icon graph">${icon("radar", 18)}</div>`;
    ...
    function bindEvents() {
        ...
        document.querySelectorAll("[data-cat-submit]").forEach(btn => {
            btn.addEventListener("click", () => {
                ...
                if (diag.questionsAnswered >= 8 || diag.abilitySE < 0.4) {
                    ...
                    const mockQuestions = [
                        { id: diag.questionsAnswered + 1, content: "以下哪个不是 JavaScript 的循环语句？", options: ["for", "while", "foreach", "do...while"], difficulty: "easy", correctIndex: 2 },
                        { id: diag.questionsAnswered + 2, content: "typeof null 的返回值是什么？", options: ["null", "undefined", "object", "boolean"], difficulty: "medium", correctIndex: 2 }
                    ];
        ...
        document.querySelectorAll("[data-diagnostic-fill-sample]").forEach(btn => {
            btn.addEventListener("click", () => {
                state.data.profileInput = "我是大二计算机科学与技术专业学生，目标是从事后端开发。数据结构会写基础题，树和图还不太熟；数据库会基础SQL增删改查；计算机网络只记得七层模型的大概。每天能投入约 120 分钟，想在 2 个月内做一个完整的项目练手。";
                render();
            });
        });
        ...
        document.querySelectorAll("[data-ai-feature]").forEach(el => el.addEventListener("click", async event => {
            event.stopPropagation();
            await runAiFeature(el.dataset.aiFeature, el);
        }));
        document.querySelectorAll("[data-agent-source-plan]").forEach(el => el.addEventListener("click", async () => {
            const sourceName = el.dataset.agentSourcePlan || "开源资料";
            const source = (state.data.aiConfig?.openSourceIntegrations || []).find(item => (item.name || item.title) === sourceName) || {};
        ...
        document.querySelectorAll("[data-scroll-question]").forEach(el => el.addEventListener("click", () => {
            document.getElementById("daily-question")?.scrollIntoView({ behavior: "smooth", block: "center" });
        }));
        document.querySelectorAll("[data-open-panel]").forEach(el => el.addEventListener("click", () => openInfoPanel(el.dataset.openPanel)));
        document.querySelectorAll("[data-study-plan-tab]").forEach(el => el.addEventListener("click", async () => {
            state.data.studyPlanTab = el.dataset.studyPlanTab || "today";
            await render();
        }));
        document.querySelectorAll("[data-plan-action]").forEach(el => el.addEventListener("click", () => {
        ...
        document.querySelectorAll("[data-study-plan-add]").forEach(form => form.addEventListener("submit", async event => {
        ...
        document.querySelectorAll("[data-continue-learning]").forEach(el => el.addEventListener("click", async () => {
            const topic = state.data.weakPoints[0]?.title || "Node.js";
            try {
                const result = await request("/api/app/closed-loop/run", { method: "POST", body: JSON.stringify({ topic }) });
                await loadData(true);
        ...
        /* 教师工作台 - Tab切换 */
        document.querySelectorAll("[data-teacher-tab]").forEach(el => el.addEventListener("click", async () => {
        ...
        /* 教师工作台 - 搜索学生 */
        document.querySelectorAll("[data-teacher-search]").forEach(el => el.addEventListener("input", e => {
            state.data._teacherSearch = e.target.value;
            clearTimeout(el._timer);
            el._timer = setTimeout(() => { state.data.teacherTab = "students"; render(); }, 400);
        }));
        document.querySelectorAll("[data-teacher-progress-search]").forEach(el => el.addEventListener("input", e => {
        ...
        /* 教师工作台 - 分配学科 */
        document.querySelectorAll("[data-teacher-assign]").forEach(el => el.addEventListener("click", async () => {
            ...
            if (!subjects || subjects.length === 0) {
                try {
                    const json = await request("/api/teacher/subjects");
                    state.data.teacherSubjects = json.subjects || [];
                } catch(e) { toast("获取学科列表失败"); return; }
            }
            const subjList = state.data.teacherSubjects;
            const subjOptions = subjList.map((s, i) => `<label style="display:flex;align-items:center;gap:8px;padding:6px 0;cursor:pointer"><input type="radio" name="assign_subject" value="${escapeHtml(s)}" ${i === 0 ? "checked" : ""}> ${escapeHtml(s)}</label>`).join("");
            ...
            if (panel) {
                ...
                document.getElementById("confirm-assign")?.addEventListener("click", async () => {
                    ...
                    if (!selected) { toast("请选择学科"); return; }
                    try {
                        const res = await request("/api/teacher/assign-subject", {
                            method: "POST", body: JSON.stringify({ student_id: parseInt(studentId), subject: selected.value, note })
                        });
                        toast(res.message || "分配成功");
                        closePanel();
                        await render();
                    } catch(e) { toast(e.message || "分配失败"); }
                });
            }
        }));
        /* 教师工作台 - 查看学生学情 */
        document.querySelectorAll("[data-teacher-view-student]").forEach(el => el.addEventListener("click", async () => {
            ...
            try {
                ...
                render();
            } catch(e) { toast("获取学生详情失败"); }
        }));
        /* 教师工作台 - 返回学情列表 */
        document.querySelectorAll("[data-teacher-back-progress]").forEach(el => el.addEventListener("click", () => {
            state.data.teacherStudentDetail = null;
            render();
        }));
        /* 教师工作台 - 取消分配 */
        document.querySelectorAll("[data-teacher-del-assign]").forEach(el => el.addEventListener("click", async () => {
        ...
        /* 教师工作台 - 移除学生（软删除） */
        document.querySelectorAll("[data-teacher-remove-student]").forEach(el => el.addEventListener("click", async () => {
        ...
        /* 教师工作台 - 分配学科筛选 */
        document.querySelectorAll("[data-teacher-assign-filter]").forEach(el => el.addEventListener("change", async () => {
        ...
        /* 教师工作台 - 笔记学科筛选 */
        document.querySelectorAll("[data-teacher-note-filter]").forEach(el => el.addEventListener("change", async () => {
        ...
        /* 教师工作台 - 创建试卷 */
        document.querySelectorAll("[data-teacher-action='create-exam']").forEach(el => el.addEventListener("click", async () => {
            const subjects = state.data.teacherSubjects;
            const subjOptions = (subjects.length ? subjects : ["数据结构与算法","计算机网络","操作系统","数据库","程序设计","前端开发","人工智能","软件工程"]).map((s, i) => `<option value="${escapeHtml(s)}" ${i === 0 ? "selected" : ""}>${escapeHtml(s)}</option>`).join("");
            ...
            document.getElementById("add_question")?.addEventListener("click", () => {
                ...
                qDiv.style.cssText = "border:1px solid #e8edf8;border-radius:10px;padding:12px;display:grid;gap:6px";
                ...
                qDiv.querySelector(".remove-question")?.addEventListener("click", () => { qDiv.remove(); questionCount--; document.querySelector("#q_count").textContent = questionCount; });
                qDiv.querySelector(`[data-q-type="${questionCount}"]`)?.addEventListener("change", (e) => {
                    ...
                    optsDiv.innerHTML = e.target.value === "judge" ? `<div style="display:flex;gap:12px"><label><input type="radio" name="judge_opts_${questionCount}" value="对" checked> 对</label><label><input type="radio" name="judge_opts_${questionCount}" value="错"> 错</label></div>` : `<input class="input" placeholder="选项A" style="margin-bottom:4px"><input class="input" placeholder="选项B">`;
                });
            });
            const panel = document.querySelector(".panel-content");
            if (panel) {
                ...
                document.getElementById("confirm-create-exam")?.addEventListener("click", async () => {
                    ...
                    document.querySelectorAll("#question_list > div").forEach(qDiv => {
                        ...
                        if (type === "judge") {
                            ...
                            qDiv.querySelectorAll(`[data-q-options="${num}"] input`).forEach(inp => { if (inp.value) options.push(inp.value); });
                        }
                        if (content) questions.push({ content, type, options, correct_answer: answer, score });
                    });
                    if (questions.length === 0) { toast("请至少添加一道题目"); return; }
                    try {
        ...
        /* 教师工作台 - 查看试卷详情 */
        document.querySelectorAll("[data-teacher-view-exam]").forEach(el => el.addEventListener("click", async () => {
            ...
            try {
                ...
                render();
            } catch(e) { toast("获取试卷详情失败"); }
        }));
        /* 教师工作台 - 返回试卷列表 */
        document.querySelectorAll("[data-teacher-exam-back]").forEach(el => el.addEventListener("click", () => {
            state.data.teacherExamDetail = null;
            render();
        }));
        /* 教师工作台 - 发布试卷 */
        document.querySelectorAll("[data-teacher-publish-exam]").forEach(el => el.addEventListener("click", async () => {
            const id = el.dataset.teacherPublishExam;
            el.classList.add("is-loading");
            try {
                const students = state.data.teacherStudents.length ? state.data.teacherStudents : await (await request("/api/teacher/students?limit=100")).students || [];
                const studentOpts = students.map(s => `<label style="display:flex;align-items:center;gap:8px;padding:4px 0;cursor:pointer"><input type="checkbox" name="pub_student" value="${s.id}" checked> ${escapeHtml(s.nickname || s.username)}</label>`).join("");
                showPanel("发布试卷", `<div class="modal-form"><p>选择要发布的学生：</p><div style="max-height:260px;overflow-y:auto;margin:12px 0;display:grid;gap:2px">${studentOpts || "<p>暂无学生</p>"}</div></div>`);
                const panel = document.querySelector(".panel-content");
                if (panel) {
                    const btn = document.createElement("div");
                    ...
                    document.getElementById("confirm-publish")?.addEventListener("click", async () => {
                        const selected = [...document.querySelectorAll("input[name='pub_student']:checked")].map(cb => parseInt(cb.value));
                        if (!selected.length) { toast("请选择至少一名学生"); return; }
                        try {
                            const res = await request(`/api/teacher/exams/${id}/publish`, {
                                method: "POST", body: JSON.stringify({ student_ids: selected })
                            });
            ...
            } catch(e) { toast("获取学生列表失败"); } finally { el.classList.remove("is-loading"); }
        }));
        /* 教师工作台 - 删除试卷 */
        document.querySelectorAll("[data-teacher-del-exam]").forEach(el => el.addEventListener("click", async () => {
        ...
        /* 教师工作台 - 发布笔记 */
        document.querySelectorAll("[data-teacher-action='create-note']").forEach(el => el.addEventListener("click", async () => {
            const subjects = state.data.teacherSubjects;
            const subjOptions = (subjects.length ? subjects : ["数据结构与算法","计算机网络","操作系统","数据库","程序设计","前端开发","人工智能","软件工程"]).map(s => `<option value="${escapeHtml(s)}">${escapeHtml(s)}</option>`).join("");
            showPanel("发布笔记", `<div class="modal-form" style="display:grid;gap:12px">
        ...
        /* 教师工作台 - 查看笔记详情 */
        document.querySelectorAll("[data-teacher-view-note]").forEach(el => el.addEventListener("click", async () => {
            const id = el.dataset.teacherViewNote;
            try {
                const json = await request(`/api/teacher/notes/${id}`);
                const note = json.note || {};
                const readers = json.readers || [];
                showPanel(note.title || "笔记详情", `<div style="display:grid;gap:12px">
            ...
            } catch(e) { toast("获取笔记详情失败"); }
        }));
        /* 教师工作台 - 删除笔记 */
        document.querySelectorAll("[data-teacher-del-note]").forEach(el => el.addEventListener("click", async () => {
        ...
        /* 教师工作台 - 刷新/通用操作 */
        document.querySelectorAll("[data-teacher-action='refresh-students']").forEach(el => el.addEventListener("click", async () => {
            state.data.teacherTab = "students";
            await render();
        }));
        document.querySelectorAll("[data-teacher-action='batch-import']").forEach(el => el.addEventListener("click", () => {
            showPanel(`
...
zhaoliu,赵六"></textarea>
            ...
            `, () => {
                document.querySelector("#btn-batch-import")?.addEventListener("click", async () => {
                    ...
                    try {
                        ...
                        btn.disabled = true;
                        btn.textContent = "导入中...";
                        const r = await request("/api/teacher/students/batch-import", { method: "POST", body: JSON.stringify({ students }) });
        ...
        document.querySelectorAll("[data-teacher-action='refresh-assignments']").forEach(el => el.addEventListener("click", async () => {
            state.data.teacherTab = "assignments";
            await render();
        }));
        document.querySelectorAll("[data-teacher-action='refresh-exams']").forEach(el => el.addEventListener("click", async () => {
            state.data.teacherTab = "exams";
            await render();
        }));
        document.querySelectorAll("[data-teacher-action='refresh-paths']").forEach(el => el.addEventListener("click", async () => {
            state.data.teacherTab = "paths";
            await render();
        }));
        document.querySelectorAll("[data-teacher-action='create-path']").forEach(el => el.addEventListener("click", async () => {
            state.data._editingPathId = null;
            openPathForm();
        }));
        document.querySelectorAll("[data-teacher-edit-path]").forEach(el => el.addEventListener("click", async () => {
        ...
        async function openPathForm() {
            ...
            let editData = null;
            if (editId) { await loadTeacherPathDetail(editId); editData = state.data.teacherPathDetail; }
            const subjects = state.data.teacherSubjects || [];
            ...
            `, async () => {
                ...
                function injectResourcePickers() {
                    document.querySelectorAll(".path-step-edit").forEach(stepDiv => {
                        ...
                        rid.type = "hidden"; rid.className = "path-step-resource-id";
                        const rt = oldRt || document.createElement("input");
                        rt.type = "hidden"; rt.className = "path-step-resource-type";
                        row.appendChild(rid); row.appendChild(rt);
                ...
                async function pickResource(stepDiv) {
                    ...
                    const renderResourceRows = rows => rows.map(r => {
                        const kindLabels = { resource: '资源', exam: '试卷', question: '题目', knowledge: '知识点', note: '笔记' };
                    ...
                    picker.querySelector("[data-close-resource-picker]")?.addEventListener("click", () => picker.classList.remove("show"));
                    ...
                    document.querySelector("#resource-search-input")?.addEventListener("input", refreshResourceList);
                    document.querySelector("#resource-type-filter")?.addEventListener("change", refreshResourceList);
                    bindResourcePickItems(stepDiv);
                }
                function bindResourcePickItems(stepDiv) {
                    document.querySelectorAll(".pick-resource-item").forEach(item => {
                        item.addEventListener("click", () => {
                            ...
                            if (contentInput) contentInput.value = desc + (contentInput.value ? '\n' + contentInput.value : '');
                ...
                document.querySelector("#btn-add-step")?.addEventListener("click", () => {
                    ...
                    `);
                    document.querySelectorAll(".path-step-type").forEach(s => s.addEventListener("change", () => toggleStepExtra(s)));
                    document.querySelectorAll(".remove-step-btn").forEach(b => b.addEventListener("click", () => b.parentElement.remove()));
                    injectResourcePickers();
                });
                /* 拖拽排序步骤 */
                ...
                document.querySelector("#btn-save-path")?.addEventListener("click", async () => {
                    ...
                    document.querySelectorAll(".path-step-edit").forEach(div => {
                        ...
                        let opts = null, answer = null;
                        if (type === "quiz" || type === "code") {
                            try { opts = JSON.parse(div.querySelector(".path-step-options").value || "[]"); } catch(e) { opts = []; }
                        ...
                        if (title) {
                            ...
                            steps.push({ title, type, content, options: opts, correct_answer: answer, duration_minutes: dur, resource_id: resourceId ? parseInt(resourceId) : null, resource_type: resourceType });
                        }
                    });
                    ...
                    try {
                        ...
                        const r = await request(endpoint, { method, body: JSON.stringify({ name, description: desc, subject, steps }) });
        ...
        function toggleStepExtra(s) {
            ...
            if (s.value === "quiz" || s.value === "code") { extra.style.display = ""; } else { extra.style.display = "none"; }
        }
    }
    document.querySelectorAll("[data-teacher-view-path]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-teacher-path-back]").forEach(el => el.addEventListener("click", async () => {
            state.data.teacherPathDetail = null;
            render();
        }));
        document.querySelectorAll("[data-teacher-path-assign]").forEach(el => el.addEventListener("click", async () => {
            ...
            `, () => {
                document.querySelectorAll("input[name='assign-mode']").forEach(r => r.addEventListener("change", () => {
                    document.querySelector("#assign-schedule-time").style.display = r.value === "scheduled" ? "" : "none";
                    document.querySelector("#btn-confirm-assign").innerHTML = r.value === "scheduled" ? `${icon("clock",16)} 定时发布` : `${icon("lock",16)} 锁定并分配`;
                }));
                /* 学生搜索过滤 */
                ...
                document.querySelector("#btn-confirm-assign")?.addEventListener("click", async () => {
                    const ids = Array.from(document.querySelectorAll(".path-assign-check:checked")).map(c => parseInt(c.value));
                    if (ids.length === 0) return toast("请至少选择一名学生");
                    const mode = document.querySelector("input[name='assign-mode']:checked")?.value || "immediate";
                    ...
                    try {
                        const r = await request(endpoint, { method: body.scheduled_at ? "PUT" : "POST", body: JSON.stringify(body) });
        ...
        document.querySelectorAll("[data-teacher-del-path]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-teacher-remove-path-student]").forEach(el => el.addEventListener("click", async () => {
            ...
            try {
                const r = await request(`/api/teacher/paths/${pathId}/assignments/${assignmentId}`, { method: "DELETE" });
        ...
        document.querySelectorAll("[data-path-complete]").forEach(el => el.addEventListener("click", async (e) => {
            e.preventDefault();
            e.stopPropagation();
            ...
            try {
                el.disabled = true;
                el.textContent = "提交中...";
                const r = await request("/api/student-paths/step-complete", { method: "POST", body: JSON.stringify({ assignment_id: parseInt(assignmentId), step_id: parseInt(stepId), answer: "" }) });
                toast(r.message || "已完成");
        ...
        document.querySelectorAll("[data-path-submit-answer]").forEach(el => el.addEventListener("click", async (e) => {
            e.preventDefault();
            e.stopPropagation();
            const assignmentId = el.dataset.assignmentId;
            const stepId = el.dataset.stepId;
            ...
            try {
                ...
                el.textContent = "提交中...";
                const r = await request("/api/student-paths/step-complete", { method: "POST", body: JSON.stringify({ assignment_id: parseInt(assignmentId), step_id: parseInt(stepId), answer }) });
                const msg = r.is_correct === 1 ? `✅ 回答正确！${r.message}` : r.is_correct === 0 ? `❌ 回答错误。${r.message}` : r.message;
                toast(msg || "已提交");
                const updatedPath = await loadStudentActivePath();
                state.data._studentLocked = !!(updatedPath && updatedPath.active);
                render();
        ...
        document.querySelectorAll("[data-path-save-note]").forEach(el => el.addEventListener("click", async () => {
            const assignmentId = el.dataset.assignmentId;
            const stepId = el.dataset.stepId;
            ...
            try {
                el.disabled = true;
                el.textContent = "保存中...";
                await request("/api/student-paths/notes", { method: "POST", body: JSON.stringify({ assignment_id: parseInt(assignmentId), step_id: parseInt(stepId), notes }) });
                toast("笔记已保存");
            } catch(e) { toast(e.message); }
            finally { el.disabled = false; el.innerHTML = icon("check",14) + " 保存笔记"; }
        }));
        /* 教师知识图谱 - 刷新 */
        document.querySelectorAll("[data-teacher-kg-refresh]").forEach(el => el.addEventListener("click", async () => {
            await loadTeacherKnowledgeGraph();
            render();
        }));
        /* 教师知识图谱 - 学科筛选 */
        document.querySelectorAll("[data-teacher-kg-filter]").forEach(el => el.addEventListener("change", async (e) => {
        ...
        if (teacherKgCanvas) {
            ...
            if (nodes.length > 0) {
                const cx = W / 2, cy = H / 2, radius = Math.min(W, H) / 2.5;
                const positions = {};
                nodes.forEach((node, i) => {
                    const angle = (2 * Math.PI * i) / nodes.length - Math.PI / 2;
                    positions[node.id] = { x: cx + radius * Math.cos(angle), y: cy + radius * Math.sin(angle), ...node };
                });
        ...
        /* 教师知识图谱 - 补救练习 */
        document.querySelectorAll("[data-teacher-kg-remedy]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-scan-choose]").forEach(el => el.addEventListener("click", () => {
        ...
        /* 试卷扫描 - 开始识别 */
        document.querySelectorAll("[data-scan-start]").forEach(el => el.addEventListener("click", async () => {
            ...
            try {
                ...
                (json.data.questions || []).forEach((_, i) => { state.data._scanSelected[i] = true; });
        ...
        /* 试卷扫描 - 返回上传页 */
        document.querySelectorAll("[data-scan-back]").forEach(el => el.addEventListener("click", () => {
            state.data._scanState = "upload";
        ...
        /* 试卷扫描 - 全选/取消全选 */
        document.querySelectorAll("[data-scan-select-all]").forEach(el => el.addEventListener("click", () => {
            const qs = state.data._scanQuestions || [];
            qs.forEach((_, i) => { state.data._scanSelected[i] = true; });
            render();
        }));
        document.querySelectorAll("[data-scan-deselect-all]").forEach(el => el.addEventListener("click", () => {
            const qs = state.data._scanQuestions || [];
            qs.forEach((_, i) => { state.data._scanSelected[i] = false; });
            render();
        }));
        /* 试卷扫描 - 复选框选择 */
        document.querySelectorAll("[data-scan-check]").forEach(el => el.addEventListener("change", (e) => {
            const idx = parseInt(el.dataset.scanCheck);
            state.data._scanSelected[idx] = e.target.checked;
            render();
        }));
        /* 试卷扫描 - 点击行切换选择 */
        document.querySelectorAll("[data-scan-qindex]").forEach(el => el.addEventListener("click", (e) => {
            if (e.target.tagName === "INPUT" || e.target.tagName === "SELECT" || e.target.tagName === "TEXTAREA" || e.target.getAttribute("contenteditable") === "true") return;
        ...
        /* 试卷扫描 - 保存到题库 */
        document.querySelectorAll("[data-scan-save]").forEach(el => el.addEventListener("click", async () => {
            const toSave = [];
        ...
        /* 试卷扫描 - 刷新历史 */
        document.querySelectorAll("[data-scan-refresh-history]").forEach(el => el.addEventListener("click", async () => {
            try {
        ...
        /* 试卷扫描 - 查看历史详情 */
        document.querySelectorAll("[data-scan-view-history]").forEach(el => el.addEventListener("click", async () => {
            ...
            try {
                ...
                state.data._scanSelected = {};
                (json.data.parsed_questions || []).forEach((_, i) => { state.data._scanSelected[i] = true; });
                state.data._scanState = "results";
                render();
        ...
        document.querySelectorAll("[data-run-closed-loop]").forEach(el => el.addEventListener("click", async () => {
            el.classList.add("is-loading");
            try {
                const result = await request("/api/app/closed-loop/run", { method: "POST", body: JSON.stringify({ topic: el.dataset.runClosedLoop || "Node.js" }) });
                state.loaded = false;
        ...
        document.querySelectorAll("[data-path-subject]").forEach(el => el.addEventListener("change", async () => {
            state.data.pathSubject = el.value;
            state.data.pathCenter = null;
            await loadPathCenter(true);
            render();
        }));
        document.querySelectorAll("[data-path-intensity]").forEach(el => el.addEventListener("change", () => {
            state.data.pathIntensity = el.value;
        }));
        document.querySelectorAll("[data-path-goal]").forEach(el => el.addEventListener("change", () => {
            state.data.pathGoal = el.value.trim() || "系统掌握计算机核心能力";
            state.data.pathCenter = null;
        }));
        document.querySelectorAll("[data-path-generate]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-major-track]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-path-start]").forEach(el => el.addEventListener("click", async () => {
            el.classList.add("is-loading");
            try {
                const result = await request(`/api/app/path/node/${el.dataset.pathStart}/start`, { method: "POST", body: "{}" });
                state.loaded = false;
                state.data.pathCenter = null;
                await loadData(true);
                await loadPathCenter(true);
        ...
        document.querySelectorAll("[data-fill-profile-sample]").forEach(el => el.addEventListener("click", () => {
            state.data.profileInput = "我是大二计算机学生，想走后端开发。数据结构会写简单题，但树和图比较弱；数据库会基础 SQL，网络只记得七层模型。每天能学 90 分钟，想两个月做出一个项目。";
            render();
        }));
        document.querySelectorAll("[data-profile-input]").forEach(el => el.addEventListener("input", () => {
            state.data.profileInput = el.value;
        }));
        document.querySelectorAll("[data-profile-analyze]").forEach(el => el.addEventListener("click", async () => {
            const text = document.querySelector("[data-profile-input]")?.value?.trim() || state.data.profileInput || "";
        ...
        document.querySelectorAll("[data-diagnostic-switch-mode]").forEach(el => el.addEventListener("click", async () => {
            const mode = el.dataset.diagnosticSwitchMode;
            if (mode === "questionnaire") {
                state.data.diagnosticMode = "questionnaire";
                state.data.diagnosticStep = 0;
                state.data.diagnosticAnswers = {};
                try { await loadDiagnosticQuestionnaire(); } catch (e) { toast("加载问卷失败: " + (e.message || "网络错误")); return; }
                render();
            } else if (mode === "text") {
                state.data.diagnosticMode = "text";
                state.data.diagnosticStep = 0;
                ...
                state.data.diagnosticSubjectAnswers = {};
                try { await loadDiagnosticSubjects(); } catch (e) { toast("加载学科列表失败: " + (e.message || "网络错误")); return; }
                render();
            } else {
        ...
        document.querySelectorAll("[data-diagnostic-start]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-diagnostic-fill-sample]").forEach(el => el.addEventListener("click", () => {
            state.data.profileInput = "我是大二计算机学生，想走后端开发。数据结构会写简单题，但树和图比较弱；数据库会基础 SQL，网络只记得七层模型。每天能学 90 分钟，想两个月做出一个项目。";
            render();
        }));
        document.querySelectorAll("[data-diagnostic-answer]").forEach(el => {
            ...
            if (type === "checkbox") {
                ...
                el.addEventListener("change", () => {
                    state.data.diagnosticAnswers[qid] = document.querySelector(`input[name="${qid}"]:checked`)?.value || "";
                    render();
                });
            } else if (type === "range") {
        ...
        document.querySelectorAll("[data-diagnostic-step-next]").forEach(el => el.addEventListener("click", () => {
            const questionnaire = state.data.diagnosticQuestionnaire;
        ...
        document.querySelectorAll("[data-diagnostic-step-prev]").forEach(el => el.addEventListener("click", () => {
            if (state.data.diagnosticStep > 0) {
                state.data.diagnosticStep--;
                render();
            }
        }));
        document.querySelectorAll("[data-diagnostic-submit]").forEach(el => el.addEventListener("click", async () => {
            state.data.diagnosticLoading = true;
            render();
            try {
        ...
        document.querySelectorAll("[data-diagnostic-restart]").forEach(el => el.addEventListener("click", () => {
            state.data.diagnosticResult = null;
            state.data.diagnosticMode = "";
            state.data.diagnosticStep = 0;
            state.data.diagnosticAnswers = {};
        ...
        document.querySelectorAll("[data-diagnostic-subject]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-subject-answer]").forEach(el => {
            ...
            if (type === "checkbox") {
                ...
                el.addEventListener("change", () => {
                    state.data.diagnosticSubjectAnswers[qid] = document.querySelector(`input[name="sq_${qid}"]:checked`)?.value || "";
                    render();
                });
            } else {
                el.addEventListener("input", () => {
        ...
        document.querySelectorAll("[data-subject-submit]").forEach(el => el.addEventListener("click", async () => {
            const test = state.data.diagnosticSubjectTest;
            if (!test) return;
        ...
        document.querySelectorAll("[data-code-template]").forEach(el => el.addEventListener("click", () => {
        ...
        document.querySelectorAll("[data-code-language]").forEach(el => el.addEventListener("change", () => {
        ...
        document.querySelectorAll("[data-code-upload]").forEach(el => el.addEventListener("change", async event => {
            const file = event.target.files?.[0];
            if (!file) return;
            const text = String(await file.text());
            const name = file.name.toLowerCase();
            const lang = name.endsWith(".html") ? "html" : name.endsWith(".py") ? "python" : name.endsWith(".java") ? "java" : name.endsWith(".cpp") || name.endsWith(".c") ? "cpp" : "javascript";
            state.data.codeSource = text;
            state.data.codeLanguage = lang;
            state.data.codeTemplate = lang === "html" ? "html" : lang === "python" ? "python" : lang === "java" ? "java" : "algorithm";
            state.data.codePreview = lang === "html" ? text : "";
            state.data.codeOutput = `已载入文件：${file.name}`;
                state.data.codeInsight = { score: 80, summary: "练习文件已载入，可以运行、预览，或请 AI 帮你梳理知识点。", tags: ["文件导入", lang, "待分析"], next: "先运行一次，再根据输出补测试用例。" };
        ...
        document.querySelectorAll("[data-code-upload-trigger]").forEach(el => el.addEventListener("click", () => {
            const input = document.querySelector("[data-code-upload]");
            if (input) input.click();
        }));
        document.querySelectorAll("[data-reset-code]").forEach(el => el.addEventListener("click", () => {
        ...
        document.querySelectorAll("[data-run-code]").forEach(el => el.addEventListener("click", async () => {
            ...
            if (state.data.codeLanguage === "html") {
                ...
                state.data.codeInsight = { score: 88, summary: "前端页面已生成预览。检查移动端布局和可访问性。", tags: ["HTML预览", source.includes("<style") ? "含样式" : "可补样式", source.includes("<script") ? "含交互" : "可补交互"], next: "添加表单或卡片列表" };
            ...
            try {
                ...
                if (result.error) {
                    ...
                    state.data.codeInsight = { score: 46, summary: "代码未通过运行，检查语法和逻辑。", tags: ["运行错误", "需要调试"], next: "查看报错信息，逐行排查" };
                } else {
                    ...
                    state.data.codeInsight = {
                        ...
                        summary: hasFunction ? "结构清晰，逻辑已封装成函数。检查边界条件。" : "建议将逻辑封装为函数，方便复用。",
                ...
                state.data.codeInsight = { score: 40, summary: "代码运行服务暂时不可用。", tags: ["服务异常"], next: "稍后重试或检查网络" };
            }
            state.data.codeRunning = false;
            render();
        }));
        document.querySelectorAll("[data-code-challenge]").forEach(el => el.addEventListener("click", () => {
        ...
        document.querySelectorAll("[data-code-explorer]").forEach(el => el.addEventListener("click", async () => {
            if (el.dataset.codeExplorer === "templates") {
                ...
                state.data.codeRepoActive = state.data.codeRepoFiles.length ? state.data.codeRepos.find(r => r.id === firstRepo.id) || firstRepo : firstRepo;
                render();
            }
        }));
        document.querySelectorAll("[data-code-new-repo]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-code-back-templates]").forEach(el => el.addEventListener("click", () => {
            state.data.codeRepoActive = null;
            state.data.codeRepoFiles = [];
            state.data.codeRepoFileActive = null;
            render();
        }));
        document.querySelectorAll("[data-code-open-file]").forEach(el => el.addEventListener("click", async () => {
            ...
            try {
                ...
                state.data.codeInsight = { score: 80, summary: "项目文件已加载。", tags: ["项目文件", file.language, "已保存"], next: "修改后保存到项目，方便老师查看学习过程" };
                render();
            } catch (e) {
                toast("打开文件失败: " + e.message);
            }
        }));
        document.querySelectorAll("[data-code-delete-file]").forEach(el => el.addEventListener("click", async event => {
            event.stopPropagation();
        ...
        document.querySelectorAll("[data-team-demo]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-team-create]").forEach(el => el.addEventListener("click", () => {
            showPanel(`${icon("plus",18)}添加团队项目`, `<div class="team-v2-create-form">
        ...
        document.querySelectorAll("[data-team-delete-project]").forEach(el => el.addEventListener("click", async event => {
            event.stopPropagation();
            const projectId = parseInt(el.dataset.teamDeleteProject, 10);
            const current = (state.data.teamCodeSummary?.projects || []).find(item => Number(item.id) === projectId);
            if (!projectId) return;
            if (!confirm(`确认删除项目「${current?.name || "团队项目"}」？项目文件、提交记录和协作动态都会删除。`)) return;
            try {
        ...
        document.querySelectorAll("[data-team-refresh]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-team-add-demo-code]").forEach(el => el.addEventListener("click", async () => {
            const projectId = state.data.teamCodeActiveProjectId;
            if (!projectId) return toast("请先创建或选择团队项目");
            const sample = teamDemoSnippet(state.data.teamCodeActiveRole || state.data.teamCodeModule || "frontend");
            state.data.teamCodeLoading = true;
            render();
            try {
        ...
        document.querySelectorAll("[data-team-project]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-team-role-select]").forEach(el => el.addEventListener("change", () => {
            switchTeamRole(el.value);
            render();
        }));
        document.querySelectorAll("[data-team-role]").forEach(el => el.addEventListener("click", () => {
            switchTeamRole(el.dataset.teamRole || "frontend");
            render();
        }));
        document.querySelectorAll("[data-team-ide]").forEach(el => el.addEventListener("click", () => {
            ...
            if (commandHelp[ide]) {
                return showPanel(`${icon("terminal",18)}${ide === "explorer" ? "File Explorer" : ide === "gitbash" ? "Git Bash" : "Terminal"} 打开方式`, `<div class="team-v2-code-help">
            ...
            toast(`正在尝试通过浏览器协议打开 ${ide.toUpperCase()}。\n如果没有反应，请先安装对应 IDE 并配置协议关联。`);
            try { window.open(url, "_blank"); } catch (_) { /* 协议调用失败则由用户手动打开 */ }
        }));
        document.querySelectorAll("[data-team-code-upload]").forEach(el => el.addEventListener("change", async e => {
            const projectId = state.data.teamCodeActiveProjectId;
            if (!projectId) return toast("请先创建或选择团队项目");
        ...
        document.querySelectorAll("[data-team-doc-upload]").forEach(el => el.addEventListener("change", async e => {
            const projectId = state.data.teamCodeActiveProjectId;
            if (!projectId) return toast("请先创建或选择团队项目");
            const selected = Array.from(e.target.files || []);
            if (!selected.length) return;
            ...
            try {
                ...
                state.data.teamCodeIntake = extractProjectIntake([...(state.data.teamCodeIntake.docs || []), ...docs], {
        ...
        document.querySelectorAll("[data-team-code-panel]").forEach(el => el.addEventListener("click", () => {
            const project = state.data.teamCodeProject?.project;
            if (!project) return toast("请先创建或选择团队项目");
            const commands = repoCloneCommands(project);
            showPanel(`${icon("git",18)}代码拉取与上传说明`, `<div class="team-v2-code-help">
        ...
        document.querySelectorAll("[data-team-clone-copy]").forEach(el => el.addEventListener("click", async () => {
            const project = state.data.teamCodeProject?.project;
            if (!project) return toast("请先创建或选择团队项目");
        ...
        document.querySelectorAll("[data-team-download]").forEach(el => el.addEventListener("click", async () => {
            const projectId = state.data.teamCodeActiveProjectId;
            const project = state.data.teamCodeProject?.project;
            if (!projectId) return toast("请先创建或选择团队项目");
            try {
                el.classList.add("is-loading");
                const res = await fetch(`/api/team-code/projects/${projectId}/download`, { credentials: "include" });
        ...
        document.querySelectorAll("[data-team-file]").forEach(el => el.addEventListener("click", async () => {
            const projectId = state.data.teamCodeActiveProjectId;
            ...
            try {
                ...
                state.data.teamCodeRepoPath = String(file.path || "").split("/").slice(0, -1).join("/");
                render();
            } catch (e) {
                toast("文件打开失败: " + (e.message || "网络错误"));
            }
        }));
        document.querySelectorAll("[data-team-repo-folder]").forEach(el => el.addEventListener("click", () => {
            state.data.teamCodeRepoPath = el.dataset.teamRepoFolder || "";
            render();
        }));
        document.querySelectorAll("[data-team-save]").forEach(el => el.addEventListener("click", async () => {
            const projectId = state.data.teamCodeActiveProjectId;
            if (!projectId) return toast("请先创建或选择团队项目");
            const source = document.querySelector("[data-team-source]")?.value || "";
            const filePath = document.querySelector("[data-team-path]")?.value?.trim()
                || state.data.teamCodePath
                || state.data.teamCodeActiveFile?.path
                || "";
            if (!filePath) return toast("请填写文件路径");
            const moduleKey = document.querySelector("[data-team-module]")?.value
                || state.data.teamCodeModule
                || state.data.teamCodeActiveFile?.module_key
                || inferModuleFromPath(filePath);
            const language = document.querySelector("[data-team-language]")?.value
            ...
            const payload = {
                ...
                positionLabel: document.querySelector("[data-team-position]")?.value || state.data.teamCodePosition || filePath,
        ...
        document.querySelectorAll("[data-team-ai-review]").forEach(el => el.addEventListener("click", async () => {
            const projectId = state.data.teamCodeActiveProjectId;
            if (!projectId) return toast("请先创建或选择团队项目");
            const source = document.querySelector("[data-team-source]")?.value || "";
            const filePath = document.querySelector("[data-team-path]")?.value?.trim() || state.data.teamCodePath || "当前编辑区";
            state.data.teamCodeLoading = true;
            render();
            try {
                await reviewTeamCode(projectId, {
                    ...
                    path: filePath,
                    content: source,
                    moduleKey: document.querySelector("[data-team-module]")?.value || state.data.teamCodeModule || "frontend"
                });
                await loadTeamCodeProject(projectId, true);
                toast("AI 代码审查已完成");
            } catch (e) {
        ...
        document.querySelectorAll("[data-team-ai-pipeline]").forEach(el => el.addEventListener("click", async () => {
            const projectId = state.data.teamCodeActiveProjectId;
            if (!projectId) return toast("请先创建或选择团队项目");
            const source = document.querySelector("[data-team-source]")?.value || state.data.teamCodeSource || "";
            const filePath = document.querySelector("[data-team-path]")?.value?.trim() || state.data.teamCodePath || state.data.teamCodeActiveFile?.path || "当前编辑区";
            ...
            try {
                await runTeamCodePipeline(projectId, {
                    ...
                    path: filePath,
                    content: source,
                    moduleKey: document.querySelector("[data-team-module]")?.value || state.data.teamCodeModule || state.data.teamCodeActiveRole || "frontend"
                });
                await loadTeamCodeProject(projectId, true);
        ...
        document.querySelectorAll("[data-team-screen]").forEach(el => el.addEventListener("click", () => {
            state.data.teamCodeScreen = el.dataset.teamScreen || "overview";
            render();
        }));
        document.querySelectorAll("[data-team-tool-task]").forEach(el => el.addEventListener("input", () => {
            state.data.teamCodeToolTask = el.value;
        }));
        document.querySelectorAll("[data-team-tool]").forEach(el => el.addEventListener("click", async () => {
            ...
            render();
            try {
                const result = await runTeamCodeTool(projectId, { tool: action, roleKey: state.data.teamCodeActiveRole, task });
        ...
        document.querySelectorAll("[data-compiler-run]").forEach(el => el.addEventListener("click", async () => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    state.data.teamCodeCompilerOutput = lines.length ? lines.join("\n") : (result.stdout || result.output || "代码执行完成，无输出");
                } else {
        ...
        document.querySelectorAll("[data-compiler-clear]").forEach(el => el.addEventListener("click", () => {
            state.data.teamCodeCompilerSource = "";
            state.data.teamCodeCompilerOutput = "运行结果将在这里显示...";
            state.data.teamCodeCompilerLang = "javascript";
            render();
        }));
        document.querySelectorAll("[data-compiler-source]").forEach(el => el.addEventListener("input", () => {
            state.data.teamCodeCompilerSource = el.value;
        }));
        document.querySelectorAll("[data-compiler-language]").forEach(el => el.addEventListener("change", () => {
            state.data.teamCodeCompilerLang = el.value;
        }));
        document.querySelectorAll("[data-ai-provider]").forEach(el => el.addEventListener("change", () => {
            state.data.aiProvider = el.value;
        }));
        document.querySelectorAll("[data-save-ai-provider]").forEach(el => el.addEventListener("click", () => {
        ...
        document.querySelectorAll("[data-run-db]").forEach(el => el.addEventListener("click", () => {
            const sql = document.querySelector("[data-db-console]")?.value?.trim() || "";
            state.data.dbConsole = sql;
            if (!/^select\b/i.test(sql)) {
                state.data.dbOutput = "为了安全，前端演示只允许 SELECT。INSERT/UPDATE/DELETE 请通过后端白名单 API 和权限审计执行。";
            } else {
                state.data.dbOutput = `模拟查询成功\nrows: 3\n\nsubject        mastery\n数据结构与算法    76\n数据库系统        84\n计算机网络        68`;
            }
            render();
        }));
        document.querySelectorAll("[data-browser-task]").forEach(el => el.addEventListener("input", () => {
            state.data.browserTask = el.value;
        }));
        document.querySelectorAll("[data-workflow-action]").forEach(el => el.addEventListener("click", () => {
        ...
        document.querySelectorAll("[data-ai-assistant-mode]").forEach(el => el.addEventListener("click", () => {
        ...
        document.querySelectorAll("[data-refresh-ai-assistant]").forEach(el => el.addEventListener("click", async () => {
            el.classList.add("is-loading");
            try {
                await loadAiAssistant(true);
                render();
        ...
        document.querySelectorAll("[data-ai-quick-prompt]").forEach(el => el.addEventListener("click", () => {
            const input = document.querySelector("[data-ai-assistant-input]");
            if (!input) return;
            input.value = el.dataset.aiQuickPrompt || "";
            input.focus();
        }));
	        document.querySelectorAll("[data-send-ai-assistant], [data-send-ai-assistant-note]").forEach(el => el.addEventListener("click", async () => {
	            const input = document.querySelector("[data-ai-assistant-input]");
	            ...
	            try {
	                ...
	                const runtimeIntent = state.data.aiAssistantMode === "plan" || state.data.aiAssistantMode === "agent"
	                    ? "design_course"
        ...
        document.querySelectorAll("[data-save-last-ai-note]").forEach(el => el.addEventListener("click", async () => {
            const lastAnswer = [...state.data.aiAssistantMessages].reverse().find(message => message.role === "ai")?.content;
        ...
        document.querySelectorAll("[data-ai-screenshot-trigger]").forEach(el => el.addEventListener("click", () => {
            document.querySelector("[data-ai-screenshot-input]")?.click();
        }));
        document.querySelectorAll("[data-ai-screenshot-input]").forEach(el => el.addEventListener("change", async () => {
            ...
            try {
                ...
                const answer = [
                    ...
                    data.graphLocation ? `知识点定位：${data.graphLocation.name || data.graphLocation.title || "已定位"}` : "",
        ...
        document.querySelectorAll("[data-ai-assistant-demo]").forEach(el => el.addEventListener("click", () => {
            const type = el.dataset.aiAssistantDemo;
            const message = type === "ocr"
                ? "拍照识题需要接入讯飞 OCR/试题识别服务。接入后这里会支持上传题图、识别题干、自动进入错题教练。"
                : "语音提问需要接入讯飞语音听写 IAT。接入后这里会支持按住说话、课堂录音转写和口语陪练。";
            toast(message);
        }));
        document.querySelectorAll("[data-asset-tab]").forEach(el => el.addEventListener("click", () => {
            state.data.assetTab = el.dataset.assetTab;
            state.data._assetTabTouched = true;
            render();
        }));
        document.querySelectorAll("[data-rag-example]").forEach(el => el.addEventListener("click", () => {
            state.data.ragAskQuery = el.dataset.ragExample || "";
            render();
        }));
        document.querySelectorAll("[data-rag-ask]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-note-filter-subject]").forEach(el => el.addEventListener("change", async () => {
        ...
        document.querySelectorAll("[data-note-filter-source]").forEach(el => el.addEventListener("change", async () => {
        ...
        document.querySelectorAll("[data-note-open]").forEach(el => el.addEventListener("click", () => {
            state.data.selectedNoteId = Number(el.dataset.noteOpen);
            render();
        }));
        document.querySelectorAll("[data-new-note]").forEach(el => el.addEventListener("click", () => {
            state.data.selectedNoteId = null;
            render();
            setTimeout(() => document.querySelector("[data-note-title]")?.focus(), 40);
        }));
        document.querySelectorAll("[data-save-note-center]").forEach(el => el.addEventListener("click", async () => {
            ...
            const tags = (document.querySelector("[data-note-tags]")?.value || "").split(/[,，]/).map(item => item.trim()).filter(Boolean);
            if (!title || !body) return toast("请先填写标题和内容");
            el.classList.add("is-loading");
            try {
                const result = noteId
                    ? await request(`/api/app/notes/${noteId}`, { method: "PUT", body: JSON.stringify({ title, body, subject, tags }) })
                    : await request("/api/app/notes/save", { method: "POST", body: JSON.stringify({ title, body, subject, tags, sourceType: "manual" }) });
                state.data.notesCenter = null;
        ...
        document.querySelectorAll("[data-delete-note]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-note-card]").forEach(el => el.addEventListener("click", async () => {
            el.classList.add("is-loading");
            try {
        ...
        document.querySelectorAll("[data-review-note]").forEach(el => el.addEventListener("click", async () => {
            try {
                const result = await request(`/api/app/notes/${el.dataset.reviewNote}/review`, { method: "POST", body: JSON.stringify({ quality: el.dataset.reviewQuality || "good" }) });
                state.data.notesCenter = null;
                await loadNotesCenter(true);
                render();
        ...
        document.querySelectorAll("[data-generate-report]").forEach(el => el.addEventListener("click", async () => {
            el.classList.add("is-loading");
            try {
                const result = await request("/api/app/report/generate", { method: "POST", body: "{}" });
                await loadData(true);
                showPanel(`${icon("chart",18)}AI学习报告`, `<div class="report-summary"><b>正确率 ${result.summary.accuracy}%</b><p>${escapeHtml(result.summary.advice)}</p><div class="list">${(result.summary.weak || []).map(w => `<div class="list-row"><span>${escapeHtml(w.title)}<small>${escapeHtml(w.subject)} · 掌握度 ${w.mastery}%</small></span><span class="pill warn">优先</span></div>`).join("")}</div></div>`);
                render();
            } catch (error) {
                toast(error.message);
            } finally {
        ...
        document.querySelectorAll("[data-report-action]").forEach(el => el.addEventListener("click", async () => {
            await runReportAction(el.dataset.reportAction, el);
        }));
        document.querySelectorAll("[data-report-resource-detail]").forEach(el => el.addEventListener("click", () => {
            const index = Number(el.dataset.reportResourceDetail);
            const resource = state.data.reportResourcePackage?.resources?.[index];
            showPanel(`${icon(resourceIcon(resource?.type),18)}${escapeHtml(resource?.title || "资源详情")}`, resourceDetailHtml(resource));
        }));
        document.querySelectorAll("[data-save-note]").forEach(el => el.addEventListener("click", async () => {
            const weak = state.data.weakPoints[0] || {};
            const body = document.querySelector("[data-asset-note]")?.value || weak.summary || "";
            try {
                const result = await request("/api/app/notes/save", { method: "POST", body: JSON.stringify({ title: `${weak.title || "计算机知识"} 学习笔记`, body }) });
                state.data.notesCenter = null;
                await loadData(true);
        ...
        document.querySelectorAll("[data-course-id]").forEach(el => el.addEventListener("click", async () => {
            ...
            el.classList.add("is-loading");
            try {
                const result = await request(`/api/app/course/${el.dataset.courseId}/progress`, { method: "POST", body: JSON.stringify({ delta: 8 }) });
                state.data.notesCenter = null;
                await loadData(true);
                render();
        ...
        document.querySelectorAll("[data-load-question-set]").forEach(el => el.addEventListener("click", async () => {
            el.classList.add("is-loading");
            try {
        ...
        document.querySelectorAll("[data-start-assessment]").forEach(el => el.addEventListener("click", () => {
        ...
        document.querySelectorAll("[data-reset-assessment]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-ai-wrong-notes]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-select-subject]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-set-answer]").forEach(el => el.addEventListener("click", () => {
            ...
            document.querySelectorAll(`[data-set-mode="${mode}"][data-set-question="${questionId}"]`).forEach(btn => btn.classList.remove("selected"));
            el.classList.add("selected");
            const answeredCount = Object.keys(state.data.answerDrafts[mode]).length;
            document.querySelector("[data-exam-answered]")?.replaceChildren(document.createTextNode(String(answeredCount)));
            document.querySelectorAll(`.answer-sheet-grid a[href="#q_${questionId}"]`).forEach(item => item.classList.add("done"));
        }));
        document.querySelectorAll("[data-submit-question-set]").forEach(el => el.addEventListener("click", async () => {
            ...
            try {
                const apiMode = mode === "onlineExam" ? "exam" : mode;
                const result = await request("/api/app/practice/submit-set", { method: "POST", body: JSON.stringify({ mode: apiMode, answers }) });
                state.data.notesCenter = null;
                const subject = mode === "test" ? state.data.selectedSubject : "all";
                const key = `${mode}:${subject}`;
        ...
        document.querySelectorAll("[data-teacher-action]").forEach(el => el.addEventListener("click", async event => {
            const localActions = new Set([
                "create-path", "refresh-paths", "create-exam", "refresh-exams",
                "create-note", "refresh-students", "batch-import", "refresh-assignments"
            ]);
            ...
            try {
                ...
                if (state.view === "teacherWorkbench") {
                    ...
                    if (["students", "progress"].includes(teacherTab)) { await loadTeacherStudents({ search: state.data._teacherSearch || "" }); }
                    if (teacherTab === "assignments") { await loadTeacherSubjects(); await loadTeacherAssignments(); }
                    if (teacherTab === "exams") { await loadTeacherExamsList(); }
                    if (teacherTab === "notes") { await loadTeacherSubjects(); await loadTeacherNotes(); }
            if (teacherTab === "paths") { await loadTeacherSubjects(); await loadTeacherPaths(); }
                }
                render();
                showPanel(`${icon("check",18)}教师任务已发布`, `<div class="closed-loop-result"><p>${escapeHtml(result.message || "教师任务已发布到学生端")}</p><div class="loop-result-grid"><div><b>学生端推荐</b><p>已新增一条高优先级学习推荐。</p></div><div><b>学习动态</b><p>学生首页通知与最近动态可见。</p></div><div><b>闭环入口</b><p>学生可进入练习、测试或智能笔记完成任务。</p></div><div><b>数据回流</b><p>完成后更新教师工作台学情。</p></div></div><div class="hero-actions"><button class="btn primary" data-panel-go="home">${icon("home",17)}看学生首页效果</button><button class="btn ghost" data-panel-go="teacherWorkbench">${icon("chart",17)}回到工作台</button></div></div>`);
            } catch (error) {
                toast(error.message);
            } finally {
                el.classList.remove("is-loading");
        ...
        document.querySelectorAll("[data-task-id]").forEach(el => el.addEventListener("click", async event => {
            event.stopPropagation();
        ...
        document.querySelectorAll("[data-generate-plan]").forEach(el => el.addEventListener("click", async () => {
            el.classList.add("is-loading");
            try {
        ...
        document.querySelectorAll("[data-answer]").forEach(el => el.addEventListener("click", async () => {
            const questionId = el.dataset.questionId;
            if (!questionId) return;
            try {
                const result = await request("/api/app/practice/answer", { method: "POST", body: JSON.stringify({ questionId, answer: el.dataset.answer }) });
                ...
                if (box) box.innerHTML = result.isCorrect ? `<span class="pill good">回答正确，掌握度提升到 ${result.nextMastery}%</span>` : `<span class="pill warn">已加入错题闭环，正确答案：${escapeHtml(result.correctAnswer)}</span>`;
                await loadData(true);
            } catch (error) {
                toast(error.message);
            }
        }));
        document.querySelectorAll("[data-ai-diagnose]").forEach(el => el.addEventListener("click", async () => {
            try {
        ...
        document.querySelectorAll("[data-run-agent-flow]").forEach(el => el.addEventListener("click", async () => {
            const input = document.querySelector("[data-tutor-input]");
            const assistantInput = document.querySelector("[data-ai-assistant-input]");
            const message = assistantInput?.value?.trim() || input?.value?.trim() || state.data.intelligence?.agentConsole?.todayJudgment?.reason || "根据我的学习数据生成下一步智能学习任务";
            const topic = state.data.intelligence?.agentConsole?.todayJudgment?.focus || state.data.weakPoints[0]?.title || "当前薄弱点";
            el.classList.add("is-loading");
            try {
                const result = await request("/api/agent-runtime/run", {
                    method: "POST",
                    body: JSON.stringify({
                        message,
                        intent: "next_action",
                        context: { goal: message || topic, subject: state.data.pathSubject || "all", intensity: state.data.pathIntensity || "normal" }
                    })
                });
                state.data.agentRuntimeResult = result;
                state.data.agentRuntimeTraces = result.traces || [];
        ...
        document.querySelectorAll("[data-agent-speak]").forEach(el => el.addEventListener("click", () => {
        ...
        document.querySelectorAll("[data-agent-voice]").forEach(el => el.addEventListener("click", () => {
        ...
        document.querySelectorAll("[data-agent-ocr]").forEach(el => el.addEventListener("change", async () => {
            const file = el.files?.[0];
            if (!file) return;
            const input = document.querySelector("[data-tutor-input]");
            ...
            try {
                const topic = state.data.intelligence?.agentConsole?.todayJudgment?.focus || state.data.weakPoints[0]?.title || "拍照识题";
        ...
        document.querySelectorAll("[data-xfyun-refresh]").forEach(el => el.addEventListener("click", async () => {
            try {
        ...
        document.querySelectorAll("[data-xfyun-ppt-topic]").forEach(el => el.addEventListener("input", () => {
            state.data.xfyunPptTopic = el.value;
        }));
        document.querySelectorAll("[data-xfyun-action]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-xfyun-ocr-file]").forEach(el => el.addEventListener("change", async () => {
            const file = el.files?.[0];
            if (!file) return;
            try {
        ...
        document.querySelectorAll("[data-xfyun-ppt-create]").forEach(el => el.addEventListener("click", async () => {
            const topic = document.querySelector("[data-xfyun-ppt-topic]")?.value?.trim() || state.data.xfyunPptTopic;
        ...
        document.querySelectorAll("[data-tutor-send]").forEach(el => el.addEventListener("click", async () => {
            const input = document.querySelector("[data-tutor-input]");
            const message = input?.value?.trim();
            if (!message) return toast("先输入一个问题");
            try {
                const result = await request("/api/app/tutor/message", { method: "POST", body: JSON.stringify({ message, topic: "智能体页面" }) });
                if (input) input.value = "";
                await loadIntelligence(true);
                render();
        ...
        document.querySelectorAll("[data-ai-card]").forEach(el => el.addEventListener("click", async () => {
            try {
                const weak = state.data.weakPoints[0];
                const customText = document.querySelector("[data-note-source]")?.value?.trim();
                await request("/api/app/notes/generate-card", {
                    method: "POST",
                    body: JSON.stringify({ text: customText || weak?.summary || "把当前学习内容转为概念、类比、例子、主动回忆四段式卡片。" })
                });
                await loadIntelligence(true);
                render();
                toast("智能笔记卡已生成");
        ...
        document.querySelectorAll("[data-feynman-review]").forEach(el => el.addEventListener("click", async () => {
            const text = document.querySelector("[data-feynman-text]")?.value?.trim() || "我理解这个概念是用旧知识解释新现象，因为它能把问题拆成可验证的步骤。";
            try {
                const result = await request("/api/app/feynman/review", { method: "POST", body: JSON.stringify({ explanation: text }) });
                await loadIntelligence(true);
                render();
        ...
        if (form) {
            form.addEventListener("submit", async event => {
                ...
                try {
                    ...
                    state.user = { username: json.user?.nickname || json.user?.username || payload.username, role: json.user?.role || "student" };
                    localStorage.setItem("edusmart_user", JSON.stringify(state.user));
                    state.loaded = false;
        ...
        document.querySelectorAll("[data-settings-form='profile']").forEach(el => el.addEventListener("submit", async event => {
            event.preventDefault();
        ...
        document.querySelectorAll("[data-settings-form='password']").forEach(el => el.addEventListener("submit", async event => {
            event.preventDefault();
            const formData = new FormData(el);
        ...
        document.querySelectorAll("[data-course-progress]").forEach(el => el.addEventListener("click", async () => {
            el.classList.add("is-loading");
            try {
        ...
        /* 学习资源中心 - 事件绑定 */
        document.querySelectorAll("[data-resource-search]").forEach(el => el.addEventListener("input", e => {
            state.data.resourceFilter.search = e.target.value;
            clearTimeout(el._timer);
            el._timer = setTimeout(() => render(), 400);
        }));
        document.querySelectorAll("[data-resource-filter]").forEach(el => el.addEventListener("change", () => {
            const key = el.dataset.resourceFilter;
            state.data.resourceFilter[key] = el.value;
            render();
        }));
        document.querySelectorAll("[data-track-click]").forEach(el => el.addEventListener("click", () => {
            const id = el.dataset.resourceId;
            if (id) trackResourceClick(id);
        }));
        document.querySelectorAll("[data-seed-resources]").forEach(el => el.addEventListener("click", async () => {
            el.classList.add("is-loading");
            try {
        ...
        /* 编程题库 - 事件绑定 */
        document.querySelectorAll("[data-problem-id]").forEach(el => el.addEventListener("click", async () => {
            ...
            if (algoVizHandle) { clearInterval(algoVizHandle); algoVizHandle = null; }
            render();
        ...
        document.querySelectorAll("[data-problem-back]").forEach(el => el.addEventListener("click", () => {
            state.data.problemDetail = null;
            state.data.problemCode = "";
            state.data.problemResult = "";
        ...
        document.querySelectorAll("[data-problem-search]").forEach(el => el.addEventListener("input", e => {
            state.data.problemFilter.search = e.target.value;
            clearTimeout(el._timer);
            el._timer = setTimeout(() => render(), 400);
        }));
        document.querySelectorAll("[data-problem-diff]").forEach(el => el.addEventListener("change", () => {
            state.data.problemFilter.difficulty = el.value;
            render();
        }));
        document.querySelectorAll("[data-problem-code]").forEach(el => el.addEventListener("input", e => {
            state.data.problemCode = e.target.value;
        }));
        document.querySelectorAll("[data-problem-lang]").forEach(el => el.addEventListener("change", () => {
        ...
        document.querySelectorAll("[data-problem-run]").forEach(el => el.addEventListener("click", async () => {
        ...
        document.querySelectorAll("[data-algo-kind]").forEach(el => el.addEventListener("change", e => {
        ...
        document.querySelectorAll("[data-algo-values]").forEach(el => el.addEventListener("input", e => {
            state.data.algoViz.values = e.target.value;
        }));
        document.querySelectorAll("[data-algo-generate]").forEach(el => el.addEventListener("click", () => {
            state.data.algoViz.steps = buildAlgoSteps(state.data.algoViz.kind, state.data.algoViz.values);
            state.data.algoViz.index = 0;
            state.data.algoViz.playing = false;
            if (algoVizHandle) { clearInterval(algoVizHandle); algoVizHandle = null; }
        ...
        document.querySelectorAll("[data-algo-prev]").forEach(el => el.addEventListener("click", () => {
            state.data.algoViz.index = Math.max(0, (state.data.algoViz.index || 0) - 1);
            state.data.algoViz.playing = false;
            if (algoVizHandle) { clearInterval(algoVizHandle); algoVizHandle = null; }
            render();
        }));
        document.querySelectorAll("[data-algo-next]").forEach(el => el.addEventListener("click", () => {
            const viz = ensureAlgoVizSteps();
        ...
        document.querySelectorAll("[data-algo-jump]").forEach(el => el.addEventListener("click", () => {
            const viz = ensureAlgoVizSteps();
        ...
        document.querySelectorAll("[data-algo-reset]").forEach(el => el.addEventListener("click", () => {
            state.data.algoViz.steps = buildAlgoSteps(state.data.algoViz.kind, state.data.algoViz.values);
            state.data.algoViz.index = 0;
            state.data.algoViz.playing = false;
            if (algoVizHandle) { clearInterval(algoVizHandle); algoVizHandle = null; }
        ...
        document.querySelectorAll("[data-algo-play]").forEach(el => el.addEventListener("click", () => {
            const viz = ensureAlgoVizSteps();
        ...
        /* 在线教程 - 事件绑定 */
        document.querySelectorAll("[data-tutorial-url]").forEach(el => el.addEventListener("click", () => {
            state.data.tutorialsUrl = el.dataset.tutorialUrl;
            render();
        }));
        /* 视频教程 - 事件绑定 */
        document.querySelectorAll("[data-video-url]").forEach(el => el.addEventListener("click", () => {
            state.data.tutorialsUrl = el.dataset.videoUrl;
            render();
        }));
        /* 多Agent协作 - 启动 */
        document.querySelectorAll("[data-agent-collaborate-start]").forEach(el => el.addEventListener("click", async () => {
            ...
            } catch(e) { toast("协作失败: " + e.message); }
            finally { el.disabled = false; el.innerHTML = icon("bolt", 14) + " 启动协作"; }
        }));
        /* ReAct 思考链折叠 */
        document.querySelectorAll("[data-react-toggle]").forEach(el => el.addEventListener("click", () => {
    ...
    function openInfoPanel(type) {
        if (type === "search") {
            ...
            return showPanel(`${icon("search",18)}智能搜索`, `<div class="search-panel"><input data-panel-search placeholder="搜索课程、知识点、题目..." value="${escapeHtml(weak[0]?.title || "数据结构")}"><div class="list">${weak.map(p => `<button class="list-row action-row" data-panel-go="asset"><span>${escapeHtml(p.title)}<small>${escapeHtml(p.subject)} · 掌握度 ${p.mastery}%</small></span><span class="pill">知识点</span></button>`).join("")}</div></div>`);
        }
        if (type === "notifications" || type === "activities") {
            loadNotifications(true).then(() => {
                ...
                const typeLabels = { path_assigned: '路径分配', path_completed: '路径完成', path_unlocked: '路径解锁', scheduled: '定时发布', system: '系统通知' };
                const typeColors = { path_assigned: '#5f58ee', path_completed: '#18b87a', path_unlocked: '#ff9500', scheduled: '#7c4dff', system: '#666' };
                const typeIcons = { path_assigned: 'route', path_completed: 'trophy', path_unlocked: 'lock', scheduled: 'clock', system: 'bell' };
                const hasUnread = state.data.notifications.some(n => !n.is_read);
                showPanel(`${icon("bell",18)}通知中心`, `<div class="notif-header">${hasUnread ? `<button class="link-button" data-mark-all-read>全部已读</button>` : ''}</div>
                    <div class="activity">${items.map(n => {
                        const isNotif = 'type' in n;
                        if (!isNotif) return `<div class="activity-item" style="--color:${n.color};--soft-color:${n.soft}"><span class="round-icon">${icon(n.icon,17)}</span><div><div class="activity-text">${escapeHtml(n.text)}</div><div class="activity-time">${escapeHtml(n.time)}</div></div><span class="pill">${escapeHtml(n.badge || '动态')}</span></div>`;
                        const tc = typeColors[n.type] || '#666';
                        const ti = typeIcons[n.type] || 'bell';
                        const timeStr = new Date(n.created_at).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' });
                ...
                setTimeout(() => {
                    ...
                    document.querySelectorAll("[data-notif-id]").forEach(el => {
                        el.addEventListener("click", async () => {
                            ...
                            if (!isNaN(id)) { await markNotificationRead(id); el.classList.remove("unread"); }
                            const link = el.dataset.notifLink;
                            if (link) {
                                const panel = document.querySelector(".modal-layer");
                                if (panel) panel.classList.remove("show");
                                if (link.startsWith("/teacher/")) { setView("teacherWorkbench"); }
                                else if (link === "/student-paths") { setView("path"); }
                            }
                        });
                    });
                }, 50);
        ...
        if (type === "achievements") {
            return showPanel(`${icon("trophy",18)}学习成就`, `<div class="achievement-modal">${state.data.achievements.map(item => `<div class="smart-card"><span class="pill">${escapeHtml(item.badge || "成就")}</span><h3>${escapeHtml(item.title)}</h3><p>经验值 ${item.xp || 100} · 来自计算机学习闭环</p></div>`).join("") || "<p>完成练习和测试后会解锁成就。</p>"}</div>`);
        }
        if (type === "profile") {
            return showPanel(`${icon("brain",18)}画像数据来源`, `<div class="diagnosis-map"><div><b>答题记录</b><p>练习、测试、考试提交后实时回写掌握度。</p></div><div><b>学习行为</b><p>课程进度、笔记保存、复习任务完成都会进入画像。</p></div><div><b>教师干预</b><p>教师发布的补救练习和分科测试会进入推荐闭环。</p></div></div>`);
        }

        /* 知识图谱 - 刷新 */
        document.querySelectorAll("[data-kg-refresh]").forEach(el => el.addEventListener("click", async () => {
            try {
                const json = await request("/api/knowledge-graph");
                state.data.knowledgeGraph = json.data || { nodes: [], edges: [] };
                render();
            } catch(e) { toast("加载图谱失败"); }
        }));
        /* 知识图谱 - 添加关联 */
        ...
        if (kgAddLinkBtn) {
            kgAddLinkBtn.addEventListener("click", async () => {
                ...
                if (!sourceNoteId) { toast("请选择源笔记"); return; }
                if (!targetTitle) { toast("请输入关联目标标题"); return; }
                try {
        ...
        if (kgCanvas) {
            kgCanvas.addEventListener("click", (e) => {
                ...
                const cx = kgCanvas.width / 2, cy = kgCanvas.height / 2;
    ...
    function showClosedLoopResult(result, title) {
        showPanel(`${icon("bolt",18)}${title}`, `<div class="closed-loop-result">
    ...
    async function runAiFeature(feature, el) {
        ...
        try {
            if (feature === "closed-loop") {
                ...
                await request("/api/app/notes/generate-card", {
                    method: "POST",
                    body: JSON.stringify({ text: weak?.summary || "把当前薄弱知识点生成概念、例子、类比、主动回忆卡片。" })
                });
                await request("/api/app/tutor/message", {
                    method: "POST",
                    body: JSON.stringify({ message: `请围绕${weak?.title || "今日薄弱点"}追问我一个关键问题`, topic: "AI一键学习闭环" })
                });
            ...
            if (feature === "diagnosis") {
                const result = await request("/api/app/diagnosis/submit", {
                    method: "POST",
                    body: JSON.stringify({ goal: weak?.title || "学机器学习", preference: "dialogue+visual", answers: [{ correct: 1 }, { correct: 0 }, { correct: 1 }, { correct: 1 }] })
                });
                await refreshAfterAiAction("intelligence");
            ...
            if (feature === "path" || feature === "review") {
                ...
                return toast(feature === "review" ? "已按遗忘曲线生成复习任务" : `AI已重排 ${result.generated} 个路径任务`);
            }
            if (feature === "practice") {
                await refreshAfterAiAction("exam");
                setTimeout(() => document.getElementById("daily-question")?.scrollIntoView({ behavior: "smooth", block: "center" }), 80);
            ...
            if (feature === "feynman") {
                const result = await request("/api/app/feynman/review", {
                    method: "POST",
                    body: JSON.stringify({ explanation: `${weak?.title || "这个概念"}可以通过定义、例子和反例来解释，因为边界条件能暴露真正的理解缺口。` })
                });
                await refreshAfterAiAction("intelligence");
    ...
    async function runReportAction(action, el) {
        const topic = document.querySelector("[data-report-topic]")?.value?.trim()
        ...
        try {
            ...
            if (action === "ai-learning") {
                const [dashboard, bkt, adaptiveTest, learningPath, reviewSchedule, reviewMessage] = await Promise.all([
                    ...
                    request("/api/ai-learning/bkt/estimate", { method: "POST", body: JSON.stringify({ nodeId: state.data.weakPoints[0]?.id || 1 }) }),
                    request("/api/ai-learning/adaptive/test", { method: "POST", body: JSON.stringify({ options: { count: 5 } }) }),
                    request("/api/ai-learning/learning-path", { method: "POST", body: JSON.stringify({ goal: `掌握 ${topic}` }) }),
                    request("/api/ai-learning/forgetting-curve"),
                    request("/api/ai-learning/forgetting-curve/message")
                ]);
                state.data.reportAiLearning = { dashboard, bkt, adaptiveTest, learningPath, reviewSchedule, reviewMessage };
        ...
        document.addEventListener("click", async (e) => {
            ...
            if (e.target.closest("[data-canvas-create]")) {
                ...
                if (!name) return;
                try {
                    const json = await request("/api/concept-canvas", { method: "POST", body: JSON.stringify({ name, data: "[]" }) });
            ...
            if (e.target.closest("[data-canvas-add-node]")) {
                ...
                state.data._activeCanvas.data.push({ id: Date.now(), name, x: 100 + count * 30, y: 100 + count * 30, type: '自定义' });
                render();
                return;
            }
            // 删除画布上的节点
            ...
            if (e.target.closest("[data-obsidian-sync]")) {
                try { await request("/api/obsidian/sync-rag", { method: "POST" }); toast("已同步 Obsidian 到 RAG 知识库"); } catch(err) { toast("同步失败: " + err.message); }
                return;
            }
            // 刷新索引
            if (e.target.closest("[data-obsidian-refresh]")) {
                state.data.obsidianKB = null; await loadObsidianKB(true); toast("索引已刷新"); render();
                return;
            }
            // 切换标签
            const obsTab = e.target.closest("[data-obsidian-tab]");
            if (obsTab) { state.data.obsidianTab = obsTab.dataset.obsidianTab; state.data.obsidianActiveNote = null; render(); return; }
            // 切换文件夹
            const obsFolder = e.target.closest("[data-obsidian-folder]");
            if (obsFolder) { state.data.obsidianActiveFolder = obsFolder.dataset.obsidianFolder; render(); return; }
            // 打开笔记
            const obsNote = e.target.closest("[data-obsidian-note]");
            if (obsNote) { loadObsidianNote(obsNote.dataset.obsidianNote); return; }
            // 关闭笔记详情
            if (e.target.closest("[data-obsidian-close-note]")) { state.data.obsidianActiveNote = null; render(); return; }
            // 搜索输入
            const obsSearch = e.target.closest("[data-obsidian-search]");
            if (obsSearch) { return; } // handled by input event
            // 加载题库
            if (e.target.closest("[data-obsidian-load-questions]")) {
                try {
                    ...
                    if (container) {
                        ...
                        if (d.sample && d.sample.length > 0) {
                            container.innerHTML += `<div style="margin-top:16px;text-align:left;">${d.sample.map((q, i) => `
                ...
                } catch(e) { toast("加载题库失败: " + e.message); }
                return;
            }

            // ========== RAG 检索事件 ==========
            // 发送查询
            if (e.target.closest("[data-rag-send]")) { sendRagQuery(); return; }
            // 建议问题
            const ragSug = e.target.closest("[data-rag-suggestion]");
            if (ragSug) { state.data.ragSearchQuery = ragSug.dataset.ragSuggestion; sendRagQuery(); return; }
            // 加入学习列表
            const ragAddLearn = e.target.closest("[data-rag-add-learning]");
            if (ragAddLearn) { addRagToLearning(ragAddLearn.dataset.ragAddLearning, state.data.ragSearchQuery || ""); return; }

            // ========== Agent 学习中心事件 ==========
            // 初始化学习画像
            if (e.target.closest("[data-agent-init]")) { loadAgentStatus(); return; }
            // 生成学习计划
            if (e.target.closest("[data-agent-plan]")) { generateAgentPlan(); return; }
            // 清除记录
            if (e.target.closest("[data-agent-clear]")) { state.data.agentStatus = null; state.data.agentPlan = null; state.data.agentReasoningLog = []; render(); return; }

            // ========== 智能诊断事件 ==========
            // CAT自适应测试启动
            if (e.target.closest("[data-cat-start]")) { startCatTest(); return; }
            // CAT提交答案
            if (e.target.closest("[data-cat-submit]")) { submitCatAnswer(); return; }
            // CAT跳过题目
            if (e.target.closest("[data-cat-skip]")) { skipCatQuestion(); return; }
            // VARK选项选择
            const varkOpt = e.target.closest("[data-vark-option]");
            if (varkOpt) { selectVarkOption(varkOpt.dataset.question, varkOpt.dataset.value); return; }
            // VARK下一页
            if (e.target.closest("[data-vark-next]")) { nextVarkSection(); return; }
            // VARK上一页
            if (e.target.closest("[data-vark-prev]")) { prevVarkSection(); return; }
            // 加载智能报告
            if (e.target.closest("[data-load-smart-report]")) { loadSmartReport(); return; }
            // 加载知识追踪
            if (e.target.closest("[data-load-knowledge]")) { loadKnowledgeMastery(); return; }
            // 加载认知画像
            if (e.target.closest("[data-load-cognitive]")) { loadCognitiveProfile(); return; }
            // 加载误区检测
            if (e.target.closest("[data-load-misconceptions]")) { loadMisconceptions(); return; }
        });
        /* 概念画布 - 搜索（事件委托） */
        document.addEventListener("input", async (e) => {
            ...
            if (q.length < 2) { state.data._canvasSearchResults = []; render(); return; }
            try {
        ...
        document.addEventListener("drop", async (e) => {
            ...
            if (!dragItem) return;
            let itemData;
            try { itemData = JSON.parse(dragItem.dataset.canvasDragItem); } catch(e) { return; }
            if (!itemData) return;
    ...
    function teamCodeCodexView() {
        ...
        const review = state.data.teamCodeReview;
        const activeRole = state.data.teamCodeActiveRole || state.data.teamCodeModule || "frontend";
        const activeFile = state.data.teamCodeActiveFile || files.find(file => file.module_key === activeRole) || files[0];
        ...
        const mainPanelHtml = (() => {
            if (codexPanel === "repo") return `<section class="team-main-panel">
                ...
                <div class="team-main-tree">
                    ...
                    ${repoRows.length ? repoRows.map(row => row.type === "folder"
                    ...
                    ).join("") : `<p>暂无文件，上传代码后显示仓库目录。</p>`}
                </div>
            </section>`;
            if (codexPanel === "code") return `<section class="team-main-panel">
            ...
            if (codexPanel === "review") return `<section class="team-main-panel">
            ...
            if (codexPanel === "docs") return `<section class="team-main-panel">
            ...
            if (codexPanel === "tools") return `<section class="team-main-panel">
                ...
                <div class="team-tool-grid">
                    ${externalTools.map(([key, name, ic]) => `<button data-team-ide="${key}">
            ...
            if (codexPanel === "logs") return `<section class="team-main-panel">
                <div class="team-main-head"><h2>${icon("history", 20)}过程日志</h2><small>${commits.length + events.length} 条记录</small></div>
                <div class="team-main-logs">${(events.length ? events : commits).slice(0, 20).map(item => `<div><b>${escapeHtml(item.title || item.message || "代码同步")}</b><span>${escapeHtml(item.detail || item.path || "")}</span><small>${formatDate(item.created_at)}</small></div>`).join("") || "<p>暂无日志。</p>"}</div>
                <pre class="team-main-pre">${escapeHtml(toolLog)}</pre>
            </section>`;
            if (codexPanel === "people") return `<section class="team-main-panel">
            ...
            if (codexPanel === "requirements") return `<section class="team-main-panel">
                ...
                <div class="team-requirement-board">
                    ${requirements.length ? requirements.map(item => `<article class="${escapeHtml(item.status)}">
                    ...
                    </article>`).join("") : `<p>暂无需求卡片。上传需求分析说明书后，会按前端、后端、测试、部署拆分功能需求。</p>`}
                </div>
            </section>`;
            if (codexPanel === "modules") return `<section class="team-main-panel">
                <div class="team-main-head"><h2>${icon("layers", 20)}模块开发卡片</h2><small>按开发颗粒度查看进度</small></div>
                <div class="team-module-board">
                    ${roleKeys.map(key => {
                        const meta = roleMeta[key];
                        const stat = moduleStats.find(item => item.roleKey === key) || {};
                        const percent = Math.min(100, 24 + Number(stat.fileCount || 0) * 18 + Number(stat.commitCount || 0) * 10);
            ...
            if (codexPanel === "collab") return `<section class="team-main-panel">
    ...
    async function render() {
        ...
        if (location.pathname.toLowerCase().includes("rag-search") || location.pathname.toLowerCase().includes("ragsearch")) {
        ...
        if (state.view === "intelligence") { await loadIntelligence(); await loadXfyunCapabilities(); }
        if (state.view === "studyPlan") await loadStudyPlan();
        if (state.view === "aiAssistant") { await loadAiAssistant(); await loadAiConfig(); await loadIntelligence(); }
        ...
        if (state.view === "knowledgeGraph" && !state.data.knowledgeGraph) {
            try { const json = await request("/api/knowledge-graph"); state.data.knowledgeGraph = json.data || { nodes: [], edges: [] }; } catch(e) {}
        }
        // 为知识图谱准备笔记下拉列表
        if (state.view === "knowledgeGraph" && state.data.knowledgeGraph) {
        ...
        if (state.view === "diagnostic") {
            ...
            if (state.data.diagnosticMode === "subject" && !state.data.diagnosticSubjects) await loadDiagnosticSubjects();
        }
        ...
        if (state.view === "teacherWorkbench") {
            ...
            if (teacherTab === "notes") { await loadTeacherSubjects(); await loadTeacherNotes(); }
            if (teacherTab === "paths") { await loadTeacherSubjects(); await loadTeacherPaths(); }
            if (teacherTab === "knowledge-graph") { await loadTeacherKnowledgeGraph(); }
        }
        if (["resources", "tutorials", "problems", "videos"].includes(state.view)) {
            if (state.view === "resources") { await loadResources(state.data.resourceFilter); await loadResourceCategories(); }
            if (state.view === "problems") { await loadProblems(state.data.problemFilter); }
        }
        if (state.view === "obsidian") await loadObsidianKB();
        if (state.view === "ragSearch") await loadRagOverview();
        const views = { home: homeView, studyPlan: studyPlanView, profile: profileView, profileCognitive: profileCognitiveView, profileKnowledge: profileKnowledgeView, profileMisconceptions: profileMisconceptionsView, diagnostic: diagnosticView, diagnosticCat: diagnosticCatView, diagnosticVark: diagnosticVarkView, diagnosticReport: diagnosticReportView, report: reportView, exam: examView, practice: () => assessmentView("practice"), test: () => assessmentView("test"), onlineExam: () => assessmentView("onlineExam"), smartNotes: smartNotesView, knowledgeGraph: knowledgeGraphView, conceptCanvas: conceptCanvasView, account: accountView, teacherWorkbench: teacherWorkbenchView, path: pathView, asset: assetView, course: courseView, intelligence: intelligenceView, aiAssistant: aiAssistantView, agentResearch: agentResearchView, codeLab: codeLabView, teamCode: teamCodeCodexView, resources: resourcesView, tutorials: tutorialsView, problems: problemsView, videos: videosView, studentPath: studentPathView, obsidian: obsidianView, ragSearch: ragSearchView, agentCenter: agentCenterView };
        ...
        const activeAssessmentSet = activeAssessmentMode ? state.data.questionSets[`${activeAssessmentMode}:${activeAssessmentSubject}`] || {} : {};
        const focusAssessment = activeAssessmentMode && state.data.assessmentStarted[activeAssessmentMode] && !activeAssessmentSet.result;
            ...
            sections: [
                {
                    title: "学习方式偏好 - 视觉",
                    questions: [
                        { id: "v1", text: "在学习新内容时，你更喜欢：", options: ["看图表和图片", "听讲解", "阅读文字", "动手实践"] },
                        { id: "v2", text: "记忆时，你会：", options: ["在脑海中形成画面", "重复念出声音", "写下来", "动手演练"] }
                    ]
                },
                {
                    title: "学习方式偏好 - 听觉",
                    questions: [
                        { id: "a1", text: "在课堂上，你更喜欢：", options: ["看板书和投影", "听老师讲解", "自己阅读课本", "参与实验和讨论"] },
                        { id: "a2", text: "学习复杂内容时，你会：", options: ["画思维导图", "与人讨论", "记笔记", "动手尝试"] }
                    ]
                },
                {
                    title: "学习方式偏好 - 读写",
                    questions: [
                        { id: "r1", text: "你获取信息的主要方式是：", options: ["看视频", "听播客", "阅读文章", "实践操作"] },
                        { id: "r2", text: "表达想法时，你会：", options: ["画图或演示", "口头说明", "写下来", "实际操作"] }
                    ]
                },
                {
                    title: "学习方式偏好 - 动觉",
                    questions: [
                        { id: "k1", text: "学习新技能时，你会：", options: ["看教程视频", "听指导", "阅读手册", "直接动手尝试"] },
                        { id: "k2", text: "在小组学习中，你更愿意：", options: ["负责画示意图", "负责讲解", "负责记录", "负责实际操作"] }
                    ]
                }
            ]
        };
    ...
    function getMockSmartReport() {
        return {
            summary: {
                overallScore: 78,
                grade: "B+ (良好)",
                description: "你在逻辑思维方面表现优秀，适合从事编程相关的学习。建议加强算法练习，提升解决复杂问题的能力。",
    ...
    function getMockCognitiveProfile() {
        return {
            cognitiveAttributes: {
                ...
                "空间想象": { mastery: 65, confidence: 0.70, state: "developing" },
    ...
    function getMockKnowledgeMastery() {
        const knowledgeNodes = {
            "js-basics": { id: "js-basics", title: "JavaScript基础", subject: "JavaScript", mastery: 0.85, state: "mastered" },
            "js-async": { id: "js-async", title: "异步编程", subject: "JavaScript", mastery: 0.65, state: "learning" },
            "js-dom": { id: "js-dom", title: "DOM操作", subject: "JavaScript", mastery: 0.80, state: "mastered" },
            ...
            "sorting": { id: "sorting", title: "排序算法", subject: "算法", mastery: 0.60, state: "learning" },
            "searching": { id: "searching", title: "查找算法", subject: "算法", mastery: 0.70, state: "learning" },
            "html": { id: "html", title: "HTML标签", subject: "HTML", mastery: 0.90, state: "mastered" },
            ...
            misconceptions: [
                {
                    ...
                    sampleQuestions: [
                        ...
                        { content: "arr.length - 1 表示什么？", userAnswer: "数组长度", correctAnswer: "最后一个元素的索引" }
                    ]
                },
                {
                    category: "变量作用域",
                    ...
                    sampleQuestions: [
                        { content: "for 循环中 var 声明的变量会有什么问题？", userAnswer: "没有问题", correctAnswer: "变量提升导致的闭包问题" }
                    ]
                },
                {
                    category: "异步编程",
                    ...
                    sampleQuestions: [
                        { content: "Promise.then() 中的代码什么时候执行？", userAnswer: "立即执行", correctAnswer: "在当前同步代码执行完毕后" }
                    ]
                },
                {
                    category: "类型转换",
