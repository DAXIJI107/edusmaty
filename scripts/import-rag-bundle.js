...
async function main() {
    ...
    await pool.query(
        'INSERT INTO rag_import_runs (bundle_hash, sources, documents, chunks) VALUES (?, ?, ?, ?)',
        [hash, sourceCount, documentCount, chunkCount]
    );

    console.log(`RAG bundle imported: sources=${sourceCount}, documents=${documentCount}, chunks=${chunkCount}, hash=${hash}`);
}
