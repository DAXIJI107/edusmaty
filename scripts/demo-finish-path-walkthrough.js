...
async function main() {
    ...
    await evalJs(
        `localStorage.removeItem('edusmart_user'); document.cookie='token=; Max-Age=0; path=/'; location.href='/'`
    );
    await sleep(1200);
    await waitFor("#login-form");
    await evalJs(
        `document.querySelector('input[name="username"]').value='teacher'; document.querySelector('input[name="password"]').value='123456'; document.querySelector('#login-form').requestSubmit()`
    );
    await sleep(1800);
    ...
    await caption(
        "教师端：路径详情监控。这里能看到已部署学生、进行中人数、完成数和平均进度。",
        ".teacher-path-monitor-grid"
    );
    ...
    await waitFor("#login-form");
    await evalJs(
        `document.querySelector('input[name="username"]').value='zhangsan'; document.querySelector('input[name="password"]').value='123456'; document.querySelector('#login-form').requestSubmit()`
    );
    await sleep(2200);
    ...
    await evalJs(
        `document.querySelector('[data-path-exercise-answer]').value='哈希冲突是不同键映射到同一位置，可以用链地址法或开放定址法处理。'; document.querySelector('[data-path-exercise-answer]').dispatchEvent(new Event('input', { bubbles:true }));`
    );
