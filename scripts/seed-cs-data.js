...
const knowledgeNodes = [
    // Data Structures
    { name: '数组', description: '数组是一种线性数据结构，将相同类型的元素存储在连续的内存空间中，支持通过索引进行O(1)时间复杂度的随机访问。', difficulty: 'easy', type: 'data_structure', subject: '数据结构' },
    { name: '链表', description: '链表由一系列节点组成，每个节点包含数据和指向下一个节点的指针。链表支持高效的插入和删除操作，但不支持随机访问。', difficulty: 'easy', type: 'data_structure', subject: '数据结构' },
    { name: '栈', description: '栈是一种遵循后进先出（LIFO）原则的线性数据结构，只允许在栈顶进行插入（入栈）和删除（出栈）操作，常用于函数调用、表达式求值等场景。', difficulty: 'easy', type: 'data_structure', subject: '数据结构' },
    { name: '队列', description: '队列是一种遵循先进先出（FIFO）原则的线性数据结构，元素从队尾入队、从队头出队，广泛应用于任务调度、消息队列等场景。', difficulty: 'easy', type: 'data_structure', subject: '数据结构' },
    { name: '二叉树', description: '二叉树是每个节点最多有两个子节点（左子节点和右子节点）的树形数据结构，是二叉搜索树、堆、平衡树等高级数据结构的基础。', difficulty: 'medium', type: 'data_structure', subject: '数据结构' },
    { name: '二叉搜索树', description: '二叉搜索树是一种特殊的二叉树，满足左子树所有节点值小于根节点、右子树所有节点值大于根节点的性质，平均查找、插入、删除时间复杂度为O(log n)。', difficulty: 'medium', type: 'data_structure', subject: '数据结构' },
    { name: '堆', description: '堆是一种特殊的完全二叉树，分为最大堆（父节点值大于等于子节点）和最小堆（父节点值小于等于子节点），常用于实现优先队列和堆排序。', difficulty: 'medium', type: 'data_structure', subject: '数据结构' },
    { name: '哈希表', description: '哈希表通过哈希函数将键映射到存储位置，实现平均O(1)时间复杂度的查找、插入和删除操作，是大多数编程语言中字典/映射类型的底层实现。', difficulty: 'medium', type: 'data_structure', subject: '数据结构' },
    { name: '图', description: '图由顶点和边组成，用于表示对象之间的复杂关系。图可以是有向或无向的、带权或不带权的，广泛应用于社交网络、路径规划、网络拓扑等领域。', difficulty: 'hard', type: 'data_structure', subject: '数据结构' },
    { name: '字典树', description: '字典树（Trie）是一种用于高效存储和检索字符串的树形结构，每个节点代表一个字符，从根到某个节点的路径构成一个前缀或完整字符串，常用于自动补全和拼写检查。', difficulty: 'medium', type: 'data_structure', subject: '数据结构' },

    // Algorithms
    { name: '冒泡排序', description: '冒泡排序通过反复遍历数组，比较相邻元素并交换顺序错误的元素对，每次遍历将最大（或最小）元素"冒泡"到数组末尾，时间复杂度为O(n²)。', difficulty: 'easy', type: 'algorithm', subject: '算法' },
    { name: '快速排序', description: '快速排序采用分治策略，选择一个基准元素将数组分为两部分（小于基准和大于基准），递归地对两部分排序，平均时间复杂度为O(n log n)，是实际应用中最常用的排序算法之一。', difficulty: 'medium', type: 'algorithm', subject: '算法' },
    { name: '归并排序', description: '归并排序也是分治算法，将数组递归地分成两半，分别排序后再合并两个有序子数组，时间复杂度稳定为O(n log n)，需要额外的O(n)空间。', difficulty: 'medium', type: 'algorithm', subject: '算法' },
    { name: '二分查找', description: '二分查找在有序数组中通过不断将搜索范围缩小一半来查找目标值，时间复杂度为O(log n)，是一种经典的高效搜索算法。', difficulty: 'easy', type: 'algorithm', subject: '算法' },
    { name: '广度优先搜索', description: '广度优先搜索（BFS）从起始节点开始，逐层遍历图中所有可达节点，使用队列辅助实现，常用于最短路径查找、层级遍历等场景。', difficulty: 'medium', type: 'algorithm', subject: '算法' },
    { name: '深度优先搜索', description: '深度优先搜索（DFS）沿着一条路径尽可能深入地探索图，当无法继续时回溯到上一个分支点，使用栈或递归实现，常用于拓扑排序、连通分量检测等。', difficulty: 'medium', type: 'algorithm', subject: '算法' },
    { name: '动态规划', description: '动态规划通过将复杂问题分解为重叠子问题，并存储子问题的解来避免重复计算，核心要素包括最优子结构和状态转移方程，是解决最优化问题的强大方法。', difficulty: 'hard', type: 'algorithm', subject: '算法' },
    { name: '贪心算法', description: '贪心算法在每一步选择中都采取当前最优的选择，希望通过局部最优达到全局最优。贪心策略不一定总是能得到全局最优解，需要满足贪心选择性质。', difficulty: 'medium', type: 'algorithm', subject: '算法' },
    { name: '双指针', description: '双指针技术使用两个指针在数组或链表上以不同速度或方向移动，高效解决两数之和、链表环检测、有序数组合并等问题，时间复杂度通常为O(n)。', difficulty: 'easy', type: 'algorithm', subject: '算法' },
    { name: '滑动窗口', description: '滑动窗口技术在数组或字符串上维护一个可变大小的窗口，通过移动窗口的左右边界来高效解决子数组/子串问题，如最长无重复子串、最小覆盖子串等。', difficulty: 'medium', type: 'algorithm', subject: '算法' },

    // Database
    { name: 'SQL基础', description: 'SQL（结构化查询语言）是关系数据库的标准查询语言，包括数据定义（DDL）、数据操作（DML）、数据查询（DQL）和数据控制（DCL）四大类语句。', difficulty: 'easy', type: 'database', subject: '数据库' },
    { name: '连接操作', description: 'JOIN操作用于根据相关列将两个或多个表中的行组合在一起，包括INNER JOIN、LEFT JOIN、RIGHT JOIN和FULL OUTER JOIN等类型，是关系数据库的核心操作。', difficulty: 'medium', type: 'database', subject: '数据库' },
    { name: '索引', description: '数据库索引是一种加速数据检索的数据结构（通常使用B+树），类似于书籍的目录。合理使用索引可以大幅提升查询性能，但不恰当的索引会增加写入开销。', difficulty: 'medium', type: 'database', subject: '数据库' },
    { name: '数据库范式', description: '数据库范式是关系数据库设计中减少数据冗余和避免更新异常的一系列规范，主要包括第一范式（1NF）到第三范式（3NF）以及BCNF等，是数据库设计的重要理论基础。', difficulty: 'medium', type: 'database', subject: '数据库' },
    { name: '事务', description: '事务是一组被视为单一工作单元的操作，要么全部执行成功（提交），要么全部撤销（回滚），确保数据库在并发操作和系统故障情况下保持一致性。', difficulty: 'medium', type: 'database', subject: '数据库' },
    { name: 'ACID', description: 'ACID是事务的四个基本特性：原子性（Atomicity）、一致性（Consistency）、隔离性（Isolation）和持久性（Durability），是关系数据库保证数据可靠性的核心机制。', difficulty: 'medium', type: 'database', subject: '数据库' },

    // Networks
    { name: 'OSI七层模型', description: 'OSI（开放系统互连）模型将网络通信分为七层：物理层、数据链路层、网络层、传输层、会话层、表示层和应用层，是理解网络协议栈的理论框架。', difficulty: 'medium', type: 'network', subject: '计算机网络' },
    { name: 'TCP/IP协议', description: 'TCP/IP是互联网的核心协议族，TCP提供可靠、面向连接的传输服务，IP负责数据包的路由和寻址，两者共同构成了互联网通信的基础。', difficulty: 'medium', type: 'network', subject: '计算机网络' },
    { name: 'HTTP/HTTPS', description: 'HTTP（超文本传输协议）是Web应用的基础协议，采用请求-响应模型。HTTPS在HTTP基础上增加TLS/SSL加密层，确保数据传输的安全性和完整性。', difficulty: 'easy', type: 'network', subject: '计算机网络' },
    { name: 'DNS', description: 'DNS（域名系统）将人类可读的域名（如www.example.com）转换为机器可读的IP地址，是互联网的"电话簿"，采用分层分布式架构实现高效的域名解析。', difficulty: 'medium', type: 'network', subject: '计算机网络' },
    { name: 'REST API', description: 'REST（表述性状态转移）是一种基于HTTP协议的API设计风格，使用GET、POST、PUT、DELETE等方法操作资源，以JSON或XML格式传输数据，具有无状态、统一接口等特点。', difficulty: 'easy', type: 'network', subject: '计算机网络' },

    // Operating Systems
    { name: '进程', description: '进程是正在执行的程序的实例，拥有独立的地址空间、文件描述符等资源。操作系统通过进程调度和上下文切换实现多任务并发执行。', difficulty: 'medium', type: 'os', subject: '操作系统' },
    { name: '线程', description: '线程是进程内的执行单元，同一进程的多个线程共享地址空间和资源，线程的创建和切换开销比进程更小，是多核编程和并发处理的基本单位。', difficulty: 'medium', type: 'os', subject: '操作系统' },
    { name: '死锁', description: '死锁是指两个或多个进程（线程）互相等待对方释放资源而陷入无限等待的状态。死锁的四个必要条件是互斥、持有并等待、不可抢占和循环等待。', difficulty: 'hard', type: 'os', subject: '操作系统' },
    { name: '内存管理', description: '内存管理负责分配和回收内存资源，包括虚拟内存、分页、分段等核心技术。虚拟内存使程序可以使用超过物理内存大小的地址空间，提高系统利用率。', difficulty: 'hard', type: 'os', subject: '操作系统' },
    { name: '文件系统', description: '文件系统负责组织和管理磁盘上的数据，提供文件的创建、读写、删除等操作接口。常见文件系统包括NTFS、ext4、APFS等，涉及inode、目录结构、权限管理等概念。', difficulty: 'medium', type: 'os', subject: '操作系统' }
];
...
const questionData = [
    ...
    },
    {
        ...
        options: ['A. 进程拥有独立的地址空间', 'B. 同一进程的线程共享地址空间', 'C. 线程切换的开销通常小于进程切换', 'D. 一个进程只能包含一个线程'],
    ...
    },
    {
        ...
        options: ['A. 读未提交（Read Uncommitted）', 'B. 读已提交（Read Committed）', 'C. 可重复读（Repeatable Read）', 'D. 可串行化（Serializable）'],
...
async function seedQuestions() {
    ...
    if (useLegacySchema) {
        ...
        const values = questionData.map(q => [
            q.content, q.type, JSON.stringify(q.options), q.answer,
            q.difficulty, 5, q.subject, q.knowledge_name, 1
        ]);
        const [result] = await pool.query(sql, [values]);
        ...
        sandbox_data: JSON.stringify({
            demo: {
                ...
                messages: [
                    ...
                    { role: 'agent', content: '好的！我来帮你完成这个任务。\n\n第一步：查询深圳天气...\n查询结果：今天深圳多云转晴，气温22-28°C，微风，适合户外活动。\n\n第二步：基于天气信息，我建议你可以：\n1. 去深圳湾公园散步或骑行\n2. 爬莲花山，俯瞰城市风景\n3. 去大鹏半岛海边游玩\n\n需要我帮你规划具体路线吗？' }
                ]
            }
        }),
        quiz_data: JSON.stringify({
            questions: [
                { question: 'AI Agent 的核心特征是什么？', options: ['被动响应', '自主感知与决策', '仅执行固定脚本', '不需要环境信息'], answer: 1 },
                { question: '以下哪项不是 AI Agent 的组成部分？', options: ['感知模块', '决策模块', '执行模块', '财务报表模块'], answer: 3 }
            ]
        }),
    ...
    },
    {
        ...
        sandbox_data: JSON.stringify({
            demo: {
                ...
                steps: [
                    { thought: '思考：这是一个简单的加法问题，我需要计算 5 - 2 + 3', action: 'calculate', observation: '计算结果：6' },
                    { thought: '思考：验证一下计算过程', action: 'verify', observation: '验证通过：5 - 2 = 3, 3 + 3 = 6' }
                ],
        ...
        quiz_data: JSON.stringify({
            questions: [
                { question: 'ReAct 范式中的 Act 指的是什么？', options: ['创建新任务', '执行具体的行动', '开始新的对话', '关闭系统'], answer: 1 },
                { question: 'ReAct 循环的正确顺序是什么？', options: ['Act→Think→Observe', 'Think→Act→Observe', 'Observe→Act→Think', 'Think→Observe→Act'], answer: 1 },
                { question: 'Observation 步骤的主要作用是什么？', options: ['制定下一步计划', '输出最终结果', '获取行动结果作为反馈', '重启推理过程'], answer: 2 }
            ]
        }),
        content: `# ReAct 思考范式
...
- 相比纯推理（Chain-of-Thought），ReAct 更依赖外部信息`
    },
    {
            ...
            demo: {
                ...
                scenario: {
                    ...
                    calls: [
                        ...
                        { tool: 'search_web', params: { query: '上海室内活动推荐' }, result: ['博物馆', '室内攀岩', '密室逃脱'] }
                    ]
                }
            }
        }),
        quiz_data: JSON.stringify({
            questions: [
                { question: 'Function Calling 的核心作用是什么？', options: ['美化输出格式', '让AI调用外部工具和API', '加速模型推理', '加密用户数据'], answer: 1 },
                { question: '工具描述（Tool Description）的作用是什么？', options: ['没有实际作用', '帮助AI理解何时以及如何使用该工具', '仅用于展示给用户', '替代工具的代码实现'], answer: 1 }
            ]
        }),
...
- 工具结果会反馈给 AI 用于后续推理和决策`
    },
    {
                ...
                conversations: [
                    {
                        ...
                        memory_updated: { short_term: '用户名叫小明，正在学习数据结构', long_term: '用户：小明，兴趣：计算机科学' }
                    },
                    {
                        round: 2,
        ...
        quiz_data: JSON.stringify({
            questions: [
                { question: 'AI Agent 的记忆系统主要包括哪几种类型？', options: ['只有短期记忆', '短期记忆和长期记忆', '只需要长期记忆', '不需要记忆'], answer: 1 },
                { question: '短期记忆的主要作用是什么？', options: ['永久保存所有信息', '维护当前对话的上下文', '替代数据库', '训练模型'], answer: 1 },
                { question: '长期记忆通常存储哪些内容？', options: ['当前对话中的临时变量', '用户偏好、历史对话摘要、重要事实', '只存储时间戳', '不需要长期记忆'], answer: 1 }
            ]
        }),
...
- 好的记忆系统让 AI 越用越"懂你"`
    },
    {
        ...
        quiz_data: JSON.stringify({
            questions: [
                { question: '多智能体协作的主要优势是什么？', options: ['让系统更复杂', '分工协作，每个Agent专注特定任务', '降低系统可靠性', '减少系统功能'], answer: 1 },
                { question: '在多智能体系统中，协调Agent的主要作用是什么？', options: ['替代所有其他Agent', '整合各Agent的输出，生成最终结果', '只负责记录日志', '不需要协调Agent'], answer: 1 },
                { question: '以下哪个是多智能体系统的典型架构？', options: ['单一巨型模型处理所有任务', '多个专用Agent分工协作', '单线程顺序执行', '不需要架构设计'], answer: 1 }
            ]
        }),
...
async function main() {
    ...
    let nodeCount = 0, questionCount = 0, courseCount = 0, noteCount = 0;
    ...
    try {
        ...
        console.log('========================================');
        console.log(`Seed complete: ${nodeCount} knowledge nodes, ${questionCount} questions, ${courseCount} courses, ${noteCount} notes inserted`);
        console.log('========================================');
    } catch (error) {
