...
async function main() {
    ...
    try {
        ...
        if (users.length === 0) {
            ...
            INFO(
                `掌握状态: ${status.averageMastery}% 平均, ${status.masteredCount}/${status.totalNodes} 已掌握, ${status.weakCount} 薄弱`
            );
            OK("状态摘要获取成功");
        }
    } catch (e) {
    ...
    try {
        ...
        OK(
            `批量生成: ${questions.length} 道题 (${questions.filter(q => q.generatedBy === "llm").length} LLM, ${questions.filter(q => q.generatedBy === "fallback").length} fallback)`
        );
