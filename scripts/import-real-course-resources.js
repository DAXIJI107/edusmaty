...
async function upsertKnowledgePoint(db, course) {
    const [[existing]] = await db.query("SELECT id FROM knowledge_points WHERE title = ? OR source_url = ? LIMIT 1", [
        course.title,
        course.source_url
    ]);
    ...
    const [result] = await db.query(
        `INSERT INTO knowledge_points (title, subject, summary, mastery, source_name, source_url)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [
            course.title.slice(0, 160),
            course.subject || "综合",
            course.description || "",
            48,
            course.provider || "公开课程",
            course.source_url
        ]
    );
...
async function upsertCourse(db, course) {
    const [[existing]] = await db.query("SELECT id, progress FROM courses WHERE source_url = ? OR title = ? LIMIT 1", [
        course.source_url,
        course.title
    ]);
    if (existing) {
