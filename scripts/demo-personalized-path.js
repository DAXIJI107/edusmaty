...
async function run() {
    ...
    async function login(username) {
        ...
        await caption(
            `登录${username === "teacher" ? "教师端" : "学生端"}：${username} / 123456`,
            "#login-form button"
        );
    ...
    await evalJs(
        `document.querySelector('#resource-type-filter').value='knowledge'; document.querySelector('#resource-type-filter').dispatchEvent(new Event('change', { bubbles:true }))`
    );
    await sleep(1200);
    ...
    await evalJs(
        `document.querySelector('.path-step-edit:nth-child(2) .path-step-type').value='exercise'; document.querySelector('.path-step-edit:nth-child(2) .path-step-type').dispatchEvent(new Event('change', { bubbles:true }))`
    );
    await fill(
        ".path-step-edit:nth-child(2) .path-step-content",
        "请用自己的话解释这个知识点，并写出一个容易出错的例子。"
    );
